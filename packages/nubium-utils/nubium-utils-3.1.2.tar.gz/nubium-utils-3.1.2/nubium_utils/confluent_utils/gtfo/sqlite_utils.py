...
class SqliteGtfo:
    def __init__(self, table_name, table_path=None, auto_init=True, max_pending_writes_count=None, min_cache_count=None, max_cache_count=None):
        if not max_pending_writes_count:
            max_pending_writes_count = int(env_vars()['NU_CONSUMER_DEFAULT_BATCH_CONSUME_MAX_COUNT']) * 5
        if not min_cache_count:
            min_cache_count = int(env_vars()['NU_CONSUMER_DEFAULT_BATCH_CONSUME_MAX_COUNT']) * 20
        if not max_cache_count:
            max_cache_count = int(env_vars()['NU_CONSUMER_DEFAULT_BATCH_CONSUME_MAX_COUNT']) * 50
    ...
    def _get_offset(self):
        try:
            return int(self._read_db('offset'))
        except:
            return 0

    def _init_db(self):
        LOGGER.debug(f'initing table {self.table_name}')
        db = SqliteDict(self.full_db_path.as_posix(), tablename='gtfo', autocommit=False)
        self.db = db
        sleep(.1)
        self.db['init'] = 'init'
    ...
    def _confirm_previous_read_in_cache(self):
        """
        Adds a key to the db cache if it was read but never updated
        """
        old_key = self._current_read[0]
        if old_key and old_key not in self.write_cache:
    ...
    def _read_all(self, key):
        ...
        """
        for cache in [self.write_cache, self.db_cache]:
            try:
                return key, cache.pop(key)
            except KeyError:
                pass
