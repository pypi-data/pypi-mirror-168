...
def gru_lstm_network(dl_inputs, model_name, train_dc, valid_dc=False, test_dc=False):
    ...
    cprint(
        ...
        bc.magenta,
        "******************************".format(
            dl_inputs["gru_lstm"]["main_architecture"].upper()
        ),
    )
    cprint(
        "[INFO]",
    ...
    try:
        ...
        cprint(
            "[WARNING]",
            bc.dred,
            "DEPRECATED (gru_dropout): use rnn_dropout in the input file instead.",
        )
    ...
    if dl_inputs["gru_lstm"]["create_tensor_board"]:
        tboard_path = os.path.join(
            dl_inputs["general"]["models_dir"],
            model_name,
    ...
    if dl_inputs["gru_lstm"]["pooling_mode"] in ["hstates_cosine"]:
        ...
        input_loss = nn.NLLLoss(
            weight=torch.tensor(
                [1, 1], dtype=torch.float32, device=dl_inputs["general"]["device"]
            ),
            reduction="mean",
    ...
    model_path = os.path.join(
        dl_inputs["general"]["models_dir"], model_name, model_name + ".model"
    )
...
def fine_tuning(
    ...
    pretrained_model = torch.load(
        pretrained_model_path, map_location=torch.device(device)
    )
    ...
    if tboard_path:
        try:
            ...
            cprint(
                "[WARNING]",
                bc.dred,
                "SummaryWriter could not be imported! Continue without creating a tensorboard.",
            )
            tboard_writer = False
    else:
        tboard_writer = False
    ...
    for epoch in tnrange(epochs):
        if train_dl:
            ...
            for x1, len1, x2, len2, y, train_indxs in t_train:
                ...
                pred = model(
                    x1, len1, x2, len2, pooling_mode=pooling_mode, device=device
                )
                if print_summary:
...
def test_model(
    ...
    if pooling_mode in ["hstates_cosine"]:
        ...
        loss_fn = nn.NLLLoss(
            weight=torch.tensor([1, 1], dtype=torch.float32, device=device),
            reduction="mean",
        )  # The negative log likelihood loss
    ...
    for x1, len1, x2, len2, y, indxs in t_test:
        if output_state_vectors:
            output_par_dir = os.path.abspath(
                os.path.join(output_state_vectors, os.pardir)
            )
        ...
        with torch.no_grad():
            ...
            if output_preds:
                ...
                pred_results = np.vstack(
                    [
                        test_dl.dataset.df.loc[indxs]["s1_unicode"].to_numpy(),
...
class two_parallel_rnns(nn.Module):
    def __init__(
        ...
        self.attn_step1 = nn.Linear(
            self.rnn_hidden_dim * self.num_directions, self.embedding_dim
        )
        self.attn_step2 = nn.Linear(self.embedding_dim, 1)

        self.fc1 = nn.Linear(
            self.rnn_hidden_dim * fc1_multiplier * self.num_directions,
            self.fc1_out_features,
        )
    ...
    def forward(
        ...
        if output_state_vectors:
            ...
            torch.save(
                h1_reshape[output_h_layer, 0],
                f"{output_state_vectors}_fwd_{self.file_id}",
            )
            if self.bidirectional:
                torch.save(
                    h1_reshape[output_h_layer, 1],
                    f"{output_state_vectors}_bwd_{self.file_id}",
                )
        ...
        if pooling_mode in ["attention"]:
            ...
            for i_nhs in range(np.shape(rnn_out_1)[0]):
                attn_weight = F.relu(
                    self.attn_step1(F.dropout(rnn_out_1[i_nhs], self.att1_dropout))
                )
                ...
                if not attn_weight_flag:
                    ...
                    attn_weight_array = torch.cat(
                        (attn_weight_array, attn_weight), dim=1
                    )
            attn_weight_array = F.softmax(attn_weight_array, dim=1)
            attn_vect_1 = torch.squeeze(
                torch.bmm(
                    rnn_out_1.permute(1, 2, 0), torch.unsqueeze(attn_weight_array, 2)
                )
            )
        elif pooling_mode in ["average"]:
            pool_1 = F.adaptive_avg_pool1d(rnn_out_1.permute(1, 2, 0), 1).view(
                x1_seq.size(1), -1
            )
        elif pooling_mode in ["max", "maximum"]:
            pool_1 = F.adaptive_max_pool1d(rnn_out_1.permute(1, 2, 0), 1).view(
                x1_seq.size(1), -1
            )
        elif pooling_mode in ["hstates"]:
            hstates_1_fwd_bwd = self.h1.view(
                self.rnn_n_layers,
                self.num_directions,
                rnn_out_1.shape[1],
                self.rnn_hidden_dim,
            ...
            if self.bidirectional:
                hstates_1 = torch.cat(
                    (hstates_1, hstates_1_fwd_bwd[self.rnn_n_layers - 1, 1]), dim=1
                )
        elif pooling_mode in [
            "hstates_layers",
            ...
            if self.bidirectional:
                ...
                for rlayer in range(1, self.rnn_n_layers):
                    hstates_1_bwd = torch.cat(
                        (hstates_1_bwd, hstates_1_fwd_bwd[rlayer, 1]), dim=1
                    )
        ...
        if pooling_mode in ["attention"]:
            ...
            for i_nhs in range(np.shape(rnn_out_2)[0]):
                attn_weight = F.relu(
                    self.attn_step1(F.dropout(rnn_out_2[i_nhs], self.att1_dropout))
                )
                attn_weight = self.attn_step2(F.dropout(attn_weight, self.att2_dropout))
                if not attn_weight_flag:
                    attn_weight_array = attn_weight
            ...
            attn_weight_array = F.softmax(attn_weight_array, dim=1)
            attn_vect_2 = torch.squeeze(
                torch.bmm(
                    rnn_out_2.permute(1, 2, 0), torch.unsqueeze(attn_weight_array, 2)
                )
            )
        elif pooling_mode in ["average"]:
            pool_2 = F.adaptive_avg_pool1d(rnn_out_2.permute(1, 2, 0), 1).view(
                x2_seq.size(1), -1
            )
        elif pooling_mode in ["max", "maximum"]:
            pool_2 = F.adaptive_max_pool1d(rnn_out_2.permute(1, 2, 0), 1).view(
                x2_seq.size(1), -1
            )
        elif pooling_mode in ["hstates"]:
            hstates_2_fwd_bwd = self.h2.view(
                self.rnn_n_layers,
                self.num_directions,
                rnn_out_2.shape[1],
                self.rnn_hidden_dim,
            ...
            if self.bidirectional:
                hstates_2 = torch.cat(
                    (hstates_2, hstates_2_fwd_bwd[self.rnn_n_layers - 1, 1]), dim=1
                )
        elif pooling_mode in [
            "hstates_layers",
            "hstates_layers_simple",
            ...
            if self.bidirectional:
                ...
                for rlayer in range(1, self.rnn_n_layers):
                    hstates_2_bwd = torch.cat(
                        (hstates_2_bwd, hstates_2_fwd_bwd[rlayer, 1]), dim=1
                    )
        ...
        if pooling_mode in ["attention"]:
            ...
            output_combined = torch.cat(
                (attn_vec_cat, attn_vec_mul, attn_vec_dif), dim=1
            )
            ...
            output_combined = torch.cat(
                (pool_rnn_cat, pool_rnn_mul, pool_rnn_dif), dim=1
            )
            ...
            output_combined = torch.cat(
                (hstates_rnn_cat, hstates_rnn_mul, hstates_rnn_dif), dim=1
            )
            ...
            hstates_cosine_sim = (
                nn.CosineSimilarity(dim=1, eps=1e-10)(hstates_1, hstates_2) + 1
            ) / 2.0
            ...
            return torch.tensor(
                torch.stack([1 - hstates_cosine_sim, hstates_cosine_sim])
            ).T
    ...
    def init_hidden(self, batch_size, device):
        ...
        return (
            Variable(
                torch.zeros((first_dim, batch_size, self.rnn_hidden_dim)).to(device)
            ),
            Variable(
                torch.zeros((first_dim, batch_size, self.rnn_hidden_dim)).to(device)
            ),
...
def inference(
    ...
    if inference_mode in ["test"]:
        ...
        output_state_vectors = os.path.join(
            scenario_path, f"embed_{query_candidate_mode}", "rnn"
        )
        ...
        log_message(
            input_command_line + "\n",
            mode="a",
            filename=os.path.join(scenario_path, "log.txt"),
        )
