...
class Tortoise:
    ...
    @classmethod
    def describe_model(cls, model: Type["Model"], serializable: bool = True) -> dict:  # pragma: nocoverage
        """
        ...
        def get_related_model(related_app_name: str, related_model_name: str) -> Type["Model"]:
            ...
            try:
                ...
                raise ConfigurationError(
                    f"No model with name '{related_model_name}' registered in" f" app '{related_app_name}'."
                )
                    ...
                    if fk_object.to_field:
                        ...
                        if related_field:
                            if related_field.unique:
                                ...
                                raise ConfigurationError(
                                    f'field "{fk_object.to_field}" in model' f' "{related_model_name}" is not unique'
                                )
                        else:
                            raise ConfigurationError(
                    ...
                    if o2o_object.to_field:
                        ...
                        if related_field:
                            if related_field.unique:
                                ...
                                raise ConfigurationError(
                                    f'field "{o2o_object.to_field}" in model' f' "{related_model_name}" is not unique'
                                )
                        else:
                            raise ConfigurationError(
                                f'there is no field named "{o2o_object.to_field}"' f' in model "{related_model_name}"'
                            )
                    else:
    ...
    @classmethod
    def _discover_models(cls, models_path: Union[ModuleType, str], app_label: str) -> List[Type["Model"]]:
    ...
    def _init_apps(cls, apps_config: dict) -> None:
        for name, info in apps_config.items():
            try:
                ...
                raise ConfigurationError(
                    'Unknown connection "{}" for app "{}"'.format(info.get("default_connection", "default"), name)
                )
    ...
    def _build_initial_querysets(cls) -> None:
        for app in cls.apps.values():
            for model in app.values():
                ...
                model._meta.basequery_all_fields = model._meta.basequery.select(*model._meta.db_fields)
    @classmethod
    async def init(
        cls,
