...
def dataset_list_filter(ctx, args, incomplete):
    ...
    if args[len(args) - 1] == "tag":
        options = dataset_tag_labels(ctx, args, incomplete)
...
def dataset_tag_labels(ctx, args, incomplete):
    """Prepare a list of dataset tag labels."""
    ...
    for tag, count in counts.items():
        options.append((tag, f"Used on {count} datasets."))
    return _finalize_completion(
        candidates=options,
        incomplete=incomplete
    )
...
def query_status_filter(ctx, args, incomplete):
    ...
    filter_names = [
        ...
        (
            "regex",
            "List only queries which matches the regex expression "
    ...
    filter_status = [
        (
            "running",
            "List only queries which are currently running or timeout "
            "(see help text)."
        ),
        (
            "finished",
