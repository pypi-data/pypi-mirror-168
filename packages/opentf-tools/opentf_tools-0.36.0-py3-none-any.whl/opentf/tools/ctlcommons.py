...
def _get_value(prefix):
    ...
    _get_value('--foo=')  -> yada if sys.argv contains `--foo yada`
                             None otherwise
    _get_value('-o')      -> yada if sys.argv contains `-o yada`, None
                             otherwise
    ```
    ...
    is_single = len(prefix) == 2 and prefix[0] == '-'
    for index, item in enumerate(sys.argv[1:], start=1):
        if prefix[-1] == '=' and item.replace('_', '-').startswith(prefix):
            return item[len(prefix) :]
        if prefix[-1] == '=' and item == prefix[:-1] and index < max_index:
            return sys.argv[index + 1]
        if is_single and item == prefix and index < max_index:
            return sys.argv[index + 1]
    return None
