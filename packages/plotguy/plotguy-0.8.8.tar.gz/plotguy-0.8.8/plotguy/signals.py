...
class Signals:
    ...
    def __new__(self, filename, output_folder, start_date, end_date, para_dict, result_df, generate_filepath, settings):
        ...
        radioitems_div = components.radioitems(para_dict, result_df)


        checkbox_div = []
        checkbox_div.append(dcc.RadioItems(
        id={'type': 'para-checklist', 'index': 1},
        options=[{'label': k, 'value': k} for k in all_options.keys()],
    ),)
        checkbox_div.append(dcc.RadioItems(
            id={'type': 'para-checklist', 'index': 2},
            options=[{'label': k, 'value': k} for k in all_options.keys()],
        ), )
        # checkbox_div.append(dcc.Checklist(_options, _options, inline=True,
        #                                        id={'type': 'para-checklist', 'index': 1},
        ...
        ], style={'width':'950px','margin':'auto','padding':'0px','color':'white'})

        @app.callback(
            Output('line_chart', 'figure'),
            Input({'type': 'para-checklist', 'index': ALL}, 'value'),
        )
        def display_output(para_checklist):
