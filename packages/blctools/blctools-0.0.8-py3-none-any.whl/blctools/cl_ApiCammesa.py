...
class ApiCammesa():
    ...
    def __init__(self):
        ...
        self._filtro = None
        self._filtro_val_pos_default = ['iniciales','ultimos']
    ...
    def check_filtro(self,val):
            ...
            print(f'Los valores totales posibles de los filtros son: {valores_posibles_totales}')

            if isinstance(val,str):
                val = val.lower()
                print(f'Filtro vale actualmente: {val}, y es un string')
    ...
    def consultar(self,fecha_i, fecha_f, nemo_rpt,exportar_consulta=False,dir_consulta=None,filtro=None):
        ...
        if self.filtro is None:
            pass
            if filtro == False or filtro is None:
                print('Sin parámetro ni atributo detectado de "filtro". Descargando los últimos DTEs disponibles.')
                self.filtro = 'ultimos'
            else:
                self.filtro = filtro
        else:
            #Si no es none, tiene que haber "aprobado" la función check_filtro
            if filtro == False or filtro is None:
                pass
            else:
                self.filtro = filtro
