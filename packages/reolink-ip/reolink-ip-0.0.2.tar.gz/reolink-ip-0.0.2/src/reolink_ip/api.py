...
class Host:
    ...
    def whiteled_schedule(self, channel: int) -> Optional[dict]:
        ...
        if self._whiteled_settings is not None and channel in self._whiteled_settings:
            return self._whiteled_settings[channel]["value"]["WhiteLed"]["LightingSchedule"]
        else:
            return None
    ...
    async def get_switchable_capabilities(self, channel: int) -> list[str]:
        ...
        if self._ftp_enabled is not None and channel in self._ftp_enabled and self._ftp_enabled[channel]:
            capabilities.append("ftp")

        if self._push_enabled is not None and channel in self._push_enabled and self._push_enabled[channel]:
            capabilities.append("push")

        if self._ir_enabled is not None and channel in self._ir_enabled and self._ir_enabled[channel]:
            capabilities.append("irLights")

        if self._whiteled_enabled is not None and channel in self._whiteled_enabled and self._whiteled_enabled[channel]:
            capabilities.append("spotlight")

        if self._audio_alarm_enabled is not None and channel in self._audio_alarm_enabled and self._audio_alarm_enabled[channel]:
            capabilities.append("siren")

        if self._recording_enabled is not None and channel in self._recording_enabled and self._recording_enabled[channel]:
            capabilities.append("recording")

        if self._email_enabled is not None and channel in self._email_enabled and self._email_enabled[channel]:
            capabilities.append("email")

        if self._audio_enabled is not None and channel in self._audio_enabled and self._audio_enabled[channel]:
    ...
    async def get_state(self, cmd: str) -> bool:
        for c in self._channels:
            ...
            channel_level_body = None

            if cmd == "GetFtp":
                channel_level_body = [{"cmd": "GetFtp", "action": 1, "param": channels_param}]
            elif cmd == "GetEnc":
                channel_level_body = [{"cmd": "GetEnc", "action": 1, "param": channels_param}]
            elif cmd == "GetEmail":
                channel_level_body = [{"cmd": "GetEmail", "action": 1, "param": channels_param}]
            elif cmd == "GetIsp":
                channel_level_body = [{"cmd": "GetIsp", "action": 1, "param": channels_param}]
            elif cmd == "GetIrLights":
                channel_level_body = [{"cmd": "GetIrLights", "action": 1, "param": channels_param}]
            elif cmd == "GetWhiteLed":
                channel_level_body = [{"cmd": "GetWhiteLed", "action": 0, "param": channels_param}]
            elif cmd == "GetPtzPreset":
                channel_level_body = [{"cmd": "GetPtzPreset", "action": 1, "param": channels_param}]
            elif cmd == "GetAutoFocus":
                channel_level_body = [{"cmd": "GetAutoFocus", "action": 0, "param": channels_param}]
            elif cmd == "GetZoomFocus":
                channel_level_body = [{"cmd": "GetZoomFocus", "action": 0, "param": channels_param}]
            elif cmd == "GetOsd":
                channel_level_body = [{"cmd": "GetOsd", "action": 1, "param": channels_param}]
            elif cmd == "GetAlarm":
                channel_level_body = [
                    {
                        "cmd": "GetAlarm",
                ...
                if self._api_version_getpush == 0:
                    channel_level_body = [{"cmd": "GetPush", "action": 1, "param": channels_param}]
                else:
                    channel_level_body = [{"cmd": "GetPushV20", "action": 1, "param": channels_param}]
            elif cmd == "GetFtp" or cmd == "GetFtpV20":
                if self._api_version_getftp == 0:
                    channel_level_body = [{"cmd": "GetFtp", "action": 1, "param": channels_param}]
                else:
                    channel_level_body = [{"cmd": "GetFtpV20", "action": 1, "param": channels_param}]
            elif cmd == "GetRec" or cmd == "GetRecV20":
                if self._api_version_getrec == 0:
                    channel_level_body = [{"cmd": "GetRec", "action": 1, "param": channels_param}]
                else:
                    channel_level_body = [{"cmd": "GetRecV20", "action": 1, "param": channels_param}]
            elif cmd == "GetAudioAlarm" or cmd == "GetAudioAlarmV20":
                if self._api_version_getalarm == 0:
                    channel_level_body = [{"cmd": "GetAudioAlarm", "action": 1, "param": channels_param}]
                else:
                    channel_level_body = [{"cmd": "GetAudioAlarmV20", "action": 1, "param": channels_param}]

            if channel_level_body is not None:
    ...
    async def get_states(self) -> bool:
        for c in self._channels:
            ...
            channel_level_body = [
                {"cmd": "GetFtp", "action": 1, "param": channels_param},
                {"cmd": "GetEnc", "action": 1, "param": channels_param},
                {"cmd": "GetEmail", "action": 1, "param": channels_param},
    ...
    async def get_host_data(self) -> bool:
        ...
        response = await self.send(host_level_body)
        if response is None:
    ...
    async def get_vod_source(self, channel: int, filename: str) -> tuple[Optional[str], Optional[str]]:
        ...
        if self._is_nvr:
            # NVR VoDs are only available over Adobe flv
    ...
    def map_host_json_response(self, json_data):
        ...
        for data in json_data:
            try:
                ...
                _LOGGER.error("Host %s:%s failed mapping JSON data: %s", self._host, self._port, e)
        ...
        for data in json_data:
            try:
                ...
                if data["cmd"] == "GetMdState":
                    ...
                    self._push_settings[channel] = data
                    ...
                    self._recording_settings[channel] = data
                    ...
                    self._ptz_presets_settings[channel] = data
                    ...
                    self._alarm_settings[channel] = data
                    ...
                    self._audio_alarm_settings[channel] = data
                    ...
                    self._auto_focus_settings[channel] = data

                elif data["cmd"] == "GetZoomFocus":
                    self._zoom_focus_settings[channel] = data

            except Exception as e:
                _LOGGER.error("Host %s:%s (channel %s) failed mapping JSON data: %s", self._host, self._port, channel, e)
                _LOGGER.error(traceback.format_exc())
                continue
    ...
    async def set_autofocus(self, channel: int, enable: bool) -> bool:
        ...
        body = [{"cmd": "SetAutoFocus", "action": 0, "param": self._auto_focus_settings[channel]["value"]}]
    ...
    def get_focus(self, channel: int):
        ...
        return self._zoom_focus_settings[channel]["value"]["ZoomFocus"]["focus"]["pos"]
    #endof get_focus()
    ...
    def get_zoom(self, channel: int):
        ...
        return self._zoom_focus_settings[channel]["value"]["ZoomFocus"]["zoom"]["pos"]
    #enfof get_zoom()
    ...
    async def set_osd(self, channel: int, namePos = None, datePos = None, enableWaterMark = None) -> bool:
        ...
        body = [{"cmd": "SetOsd", "action": 0, "param": self._osd_settings[channel]["value"]}]
    ...
    async def set_push(self, channel: int, enable: bool) -> bool:
        ...
        if self._api_version_getpush == 0:
            body = [{"cmd": "SetPush", "action": 0, "param": self._push_settings[channel]["value"]}]
            body[0]["param"]["Push"]["schedule"]["enable"] = 1 if enable else 0
        else:
            body = [{"cmd": "SetPushV20", "action": 0, "param": self._push_settings[channel]["value"]}]
    ...
    async def set_ftp(self, channel: int, enable: bool) -> bool:
        ...
        if self._api_version_getftp[channel] == 0:
            body = [{"cmd": "SetFtp", "action": 0, "param": self._ftp_settings[channel]["value"]}]
            body[0]["param"]["Ftp"]["schedule"]["enable"] = 1 if enable else 0
        else:
            body = [{"cmd": "SetFtpV20", "action": 0, "param": self._ftp_settings[channel]["value"]}]
    ...
    async def set_audio(self, channel: int, enable: bool) -> bool:
        ...
        body = [{"cmd": "SetEnc", "action": 0, "param": self._enc_settings[channel]["value"]}]
    ...
    async def set_email(self, channel: int, enable: bool) -> bool:
        if channel not in self._channels:
            return False
        ...
        body = [{"cmd": "SetEmail", "action": 0, "param": self._email_settings[channel]["value"]}]
    ...
    async def set_daynight(self, channel: int, value: str) -> bool:
        ...
        body = [{"cmd": "SetIsp", "action": 0, "param": self._isp_settings[channel]["value"]}]
    ...
    async def set_recording(self, channel: int, enable: bool) -> bool:
        ...
        if self._api_version_getrec[channel] == 0:
            body = [{"cmd": "SetRec", "action": 0, "param": self._recording_settings[channel]["value"]}]
            body[0]["param"]["Rec"]["schedule"]["enable"] = 1 if enable else 0

        else:
            body = [{"cmd": "SetRecV20", "action": 0, "param": self._recording_settings[channel]["value"]}]
    ...
    async def set_motion_detection(self, channel: int, enable: bool) -> bool:
        ...
        body = [{"cmd": "SetAlarm", "action": 0, "param": self._alarm_settings[channel]["value"]}]
        ...
        body = [
            {
                ...
                "param": {
                    "Alarm": {
                        "channel": channel,
                        "type": "md",
                        "sens": self._alarm_settings[channel]["value"]["Alarm"]["sens"],
                    }
                }
            }
        ]
            ...
            if json_data[0]["code"] == 0:
                ...
                if status_only or "File" not in search_result:
                    if "Status" in search_result:
                        ...
                        _LOGGER.warning("Host: %s:%s: No \"Status\" in the result of Search command.", self._host, self._port)
                else:
                    return search_result["Status"], search_result["File"]
            else:
                _LOGGER.warning("Host: %s:%s: The Search command returned error code %s.", self._host, self._port, json_data[0]["code"])
        else:
            self.clear_token()
            _LOGGER.warning("Host: %s:%s: Failed to get results for Search command, JSON data was empty.", self._host, self._port)
    ...
    async def send(self, body, param = None, expected_content_type: Optional[str] = None) -> Optional[list]:
        ...
        try:
            ...
            if body is None:
                ...
                async with session.post(url = self._url, json = body, params = param, allow_redirects = False) as response:
                    _LOGGER.debug("%s/%s:%s::send() HTTP Request params =\n%s\n", self.nvr_name, self._host, self._port, str(param).replace(self._password, "<password>"))
            ...
            self.clear_token()
            _LOGGER.error("Host %s:%s: Unknown exception %s occurred: %s", self._host, self._port, e, traceback.format_exc())
            raise
    #endof send()
    ...
    def renewtimer(self) -> int:
        ...
        _LOGGER.debug("Host %s should renew in: %i seconds...", self._host, diff)
    ...
    async def subscription_send(self, headers, data) -> Optional[str]:
        ...
        try:
            async with aiohttp.ClientSession(timeout = self._timeout, connector = aiohttp.TCPConnector(verify_ssl = False)) as session:
                ...
                async with session.post(url = self._subscribe_url, data = data, headers = headers, allow_redirects = False) as response:
            ...
            _LOGGER.debug('Host %s:%s: connection error %s.', self._host, self._port, str(e))
        except asyncio.TimeoutError:
            _LOGGER.debug('Host %s:%s: connection timeout exception.', self._host, self._port)
        except:
            _LOGGER.debug('Host %s:%s: unknown exception occurred.', self._host, self._port)
    ...
    async def unsubscribe_all(self):
        ...
        if self._nvr_model == "RLN8-410" or self._nvr_model == "RLN16-410":
            _LOGGER.debug("Attempting to unsubscribe previous (dead) session's notifications...")
