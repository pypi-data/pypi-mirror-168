...
from .tools import nearestRes,wrap180,wrap,maxe
...
def plotRes(sm, addlines=True,incb=0,**kwargs):
	...
	if numres > 1:
		...
		axs.plot(time,res_ang[0,:], 'k',marker='.',ls='')
		axs.set_ylabel(ang_names[0],weight='bold')
		if addlines:
			axs.plot(time,np.ones(len(time))*np.median(res_ang),'b-',lw=2)
			axs.plot(time,np.ones(len(time))*(np.median(res_ang)+2*np.std(res_ang)),'b--',lw=1)
			axs.plot(time,np.ones(len(time))*(np.median(res_ang)-2*np.std(res_ang)),'b--',lw=1)
		axs.set_xlabel('Time (yr)',weight='bold')

		axs.set_yticks([-180,0,180])
		axs.set_yticks(np.linspace(-180,180,9),minor=True)
		axs.set_yticklabels(['']*9,minor=True)
		axs.set(ylim=[-180,180])
