...
async function openTerminal(eid, app) {
    ...
    if (terminal) {
        ...
        app.shell.add(terminal, 'main', { mode: 'split-top' });
...
async function handleClick(kernel, app) {
    if (kernel) {
        ...
        if (spec === null || spec === void 0 ? void 0 : spec.name.endsWith('-genv')) {
            ...
            if (await _dialogs__WEBPACK_IMPORTED_MODULE_3__.Dialogs.notSupportedKernel()) {
                await app.commands.execute('notebook:change-kernel');
            }
        }
    }
    else {
        if (await _dialogs__WEBPACK_IMPORTED_MODULE_3__.Dialogs.noKernel()) {
            await app.commands.execute('notebook:change-kernel');
        }
    }
}
...
//# sourceMappingURL=lib_index_js.aad60d06f23f06885186.js.map