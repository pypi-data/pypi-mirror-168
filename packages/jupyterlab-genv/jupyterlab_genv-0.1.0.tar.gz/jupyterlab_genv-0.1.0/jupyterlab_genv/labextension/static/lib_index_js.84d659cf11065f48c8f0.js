...
(function (Dialogs) {
    ...
    async function activate(envs, kernel_id) {
        ...
        function desc(env) {
            if (env.name) {
                return `${env.name} (${env.eid})`;
            }
            return env.eid;
        }
        const values = [
            { value: placeholder, label: kernel_id },
            ...envs.map(env => {
                return { value: env.eid, label: desc(env) };
            })
        ];
        const { value } = await _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.InputDialog.getItem({
            title: 'Activate GPU Environment',
            items: values.map(value => value.label),
            okLabel: 'Next'
        });
        return value;
    }
...
/* harmony import */ var _dialogs__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./dialogs */ "./lib/dialogs.js");
...
async function handleClick(kernel, app) {
    if (kernel) {
        ...
        if (spec === null || spec === void 0 ? void 0 : spec.name.endsWith('-genv')) {
            let eid = await _handler__WEBPACK_IMPORTED_MODULE_4__.Handler.find(kernel.id);
            if (!eid) {
                const envs = await _handler__WEBPACK_IMPORTED_MODULE_4__.Handler.envs();
                eid = await _dialogs__WEBPACK_IMPORTED_MODULE_5__.Dialogs.activate(envs, kernel.id);
                if (eid) {
                    await _handler__WEBPACK_IMPORTED_MODULE_4__.Handler.activate(kernel.id, eid);
                }
            }
            if (eid) {
                if (await _dialogs__WEBPACK_IMPORTED_MODULE_5__.Dialogs.configure(eid)) {
                    await openTerminal(eid, app);
                }
            }
        }
        else {
            if (await _dialogs__WEBPACK_IMPORTED_MODULE_5__.Dialogs.notSupportedKernel()) {
                await app.commands.execute('notebook:change-kernel');
            }
        }
    }
    else {
        if (await _dialogs__WEBPACK_IMPORTED_MODULE_5__.Dialogs.noKernel()) {
            await app.commands.execute('notebook:change-kernel');
        }
    }
}
...
class DevicesWidget extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_2__.Widget {
    async onUpdateRequest(_msg) {
        const devices = await _handler__WEBPACK_IMPORTED_MODULE_4__.Handler.devices();
        if (this.div) {
...
const plugin = {
    ...
    activate: async (app, palette) => {
        ...
        const envsInfos = await _handler__WEBPACK_IMPORTED_MODULE_4__.Handler.envs();
        const envsWidget = new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.MainAreaWidget({
            content: _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.ReactWidget.create(react__WEBPACK_IMPORTED_MODULE_3___default().createElement((react__WEBPACK_IMPORTED_MODULE_3___default().Fragment), null, envsInfos.map((env) => (react__WEBPACK_IMPORTED_MODULE_3___default().createElement("div", null,
                `${env['eid']} ${env['user']}`,
...
//# sourceMappingURL=lib_index_js.84d659cf11065f48c8f0.js.map