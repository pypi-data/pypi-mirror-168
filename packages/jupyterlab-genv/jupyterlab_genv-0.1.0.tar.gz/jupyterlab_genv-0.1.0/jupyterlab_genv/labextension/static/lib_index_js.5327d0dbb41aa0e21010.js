...
(function (Dialogs) {
    async function noKernel() {
        await (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.showDialog)({
            ...
            buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog.warnButton()]
        });
    }
...
async function handleClick(kernel, app) {
    if (kernel) {
        ...
        if (spec === null || spec === void 0 ? void 0 : spec.name.endsWith('-genv')) {
            let eid = await _handler__WEBPACK_IMPORTED_MODULE_2__.Handler.find(kernel.id);
            if (!eid) {
                const envs = await _handler__WEBPACK_IMPORTED_MODULE_2__.Handler.envs();
                eid = await _dialogs__WEBPACK_IMPORTED_MODULE_3__.Dialogs.activate(envs, kernel.id);
                if (eid) {
                    await _handler__WEBPACK_IMPORTED_MODULE_2__.Handler.activate(kernel.id, eid);
                }
            }
            if (eid) {
                if (await _dialogs__WEBPACK_IMPORTED_MODULE_3__.Dialogs.configure(eid)) {
                    await openTerminal(eid, app);
                }
            }
        }
        else {
            await _dialogs__WEBPACK_IMPORTED_MODULE_3__.Dialogs.notSupportedKernel();
            await app.commands.execute('console:change-kernel');
            await app.commands.execute('kernelmenu:change');
            await app.commands.execute('notebook:change-kernel');
        }
    }
    else {
        await _dialogs__WEBPACK_IMPORTED_MODULE_3__.Dialogs.noKernel();
        await app.commands.execute('console:change-kernel');
        await app.commands.execute('kernelmenu:change');
        await app.commands.execute('notebook:change-kernel');
    }
...
const plugin = {
    ...
    activate: async (app, palette) => {
        ...
        const devicesInfos = await _handler__WEBPACK_IMPORTED_MODULE_2__.Handler.devices();
        const devicesWidget = new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.MainAreaWidget({
            content: _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.ReactWidget.create(react__WEBPACK_IMPORTED_MODULE_1___default().createElement((react__WEBPACK_IMPORTED_MODULE_1___default().Fragment), null, devicesInfos.map((device, i) => (react__WEBPACK_IMPORTED_MODULE_1___default().createElement("div", null,
                "GPU ",
                i,
                ":",
                ' ',
                device.eid === '' ? (react__WEBPACK_IMPORTED_MODULE_1___default().createElement("span", { style: { color: 'green' } }, "available")) : (react__WEBPACK_IMPORTED_MODULE_1___default().createElement("span", null,
                    "used by enviornment ",
                    device.eid)))))))
        });
        ...
        // https://jupyterlab.readthedocs.io/en/stable/extension/extension_tutorial.html#show-a-new-image-on-demand
        // https://github.com/jupyterlab/extension-examples/blob/master/react-widget/src/widget.tsx
        // devicesWidget.toolbar.insertItem(
        //   0,
        //   'refresh',
        //   new ToolbarButton({ label: 'Refresh' })
        // );
        ...
        const envsInfos = await _handler__WEBPACK_IMPORTED_MODULE_2__.Handler.envs();
        const envsWidget = new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.MainAreaWidget({
            content: _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.ReactWidget.create(react__WEBPACK_IMPORTED_MODULE_1___default().createElement((react__WEBPACK_IMPORTED_MODULE_1___default().Fragment), null, envsInfos.map((env) => (react__WEBPACK_IMPORTED_MODULE_1___default().createElement("div", null,
                `${env['eid']} ${env['user']}`,
...
//# sourceMappingURL=lib_index_js.5327d0dbb41aa0e21010.js.map