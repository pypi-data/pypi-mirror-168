...
for (h5, cell_num) in zip(h5_list, tqdm(cell_list)):
    ...
    assert np.all(frame_nums == w_frame_nums), "frame nums don't match this should never happend"
...
def foo_plot(c, W, data_key, color_in, smooth_by=1, mult_by=1, leg=''):
    y = np.nanmean(c[data_key], axis=0)
    sd = np.nanstd(c[data_key], axis=0)
    se = sd/np.sqrt(c[data_key].shape[0])
    ci = 1.96*se
    plt.plot(W['meta']['x'], mult_by*utils.smooth(np.nanmean(c[data_key], axis=0), smooth_by), color=color_in+[1], label=leg)
...
smooth_by = 5
bd = '/Users/phil/Dropbox/HIRES_LAB/WhACC PAPER/figures/misc/PSTH_few_cells/'
for ii, cell_num in enumerate(to_loop_cells):
    ...
    plt.xlim(-50, 250)
    plt.legend()
    # plt.savefig(bd+os.sep+c['title1'] + '.pdf', transparent=True, format='pdf')
    # plt.close()
