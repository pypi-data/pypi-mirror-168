...
def Uarray_cell_labels_to_H5_file(c, h5):
    ...
    assert len(full_file_names) == len(np.unique(full_file_names)), 'your h5 file has duplicate video files that it processed, you will have to create a new H5 file and start over, sorry'
    ...
    ind_human_to_whacc_trials = intersect_data_with_nans(h_trials, w_trials, data=np.arange(len(w_trials)))
...
def batch_process_videos_on_colab(bd, local_temp_dir, video_batch_size=2, RESNET_MODEL=None, copy_image_files_to_final_h5=False,
                                  copy_feature_data_to_final_h5=False):
...
def combine_final_h5s(h5_file_list_to_combine, delete_extra_files=False):
    ...
    keys = ['file_name_nums', 'frame_nums', 'full_file_names', 'in_range', 'labels',
            'locations_x_y', 'max_val_stack', 'CNN_pred']
...
def update_whacc():
    ...
    out = os.popen(x).read()
    print(out)
    print('please refer to teh open terminal window for further details')
