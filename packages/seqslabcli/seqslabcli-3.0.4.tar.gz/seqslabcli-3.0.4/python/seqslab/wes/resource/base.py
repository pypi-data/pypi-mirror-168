...
class BaseResource(ABC):
    ...
    WES_RESOURCES_URL = f"https://{API_HOSTNAME}/wes/{__version__}/service-info/workspaces/{{" \
    ...
    WES_RUNTIME_OPTIONS_URL = f"{WES_BASE_URL}runtime-options/{{name}}?backend={{backend}}"
    ...
    def get_run_status(self, run_id) -> dict:
        ...
        backend = ctx.args.backend
        token = BaseAuth.get_token().get('tokens').get('access')
        with requests.get(url=f"{self.WES_RUNS_STATUS_URL.format(id=run_id,backend=backend)}",
                          headers={"Authorization": f"Bearer {token}"}) as response:
            if response.status_code not in [requests.codes.ok]:
                raise requests.HTTPError()
            return json.loads(response.content)

    def sync_run_jobs(self, data, headers, run_request_id, run_name, retries=5) -> requests.Response:
        try:
            ctx = context.get_context()
            backend = ctx.args.backend
            if not headers.get("Authorization"):
            ...
            if run_request_id:
                if run_name:
                    url = f'{self.WES_RUNS_URL.format(backend=backend)}&request_id={run_request_id}&run_name={run_name}'
                else:
                    url = f'{self.WES_RUNS_URL.format(backend=backend)}&request_id={run_request_id}'
            else:
                if run_name:
                    url = f'{self.WES_RUNS_URL.format(backend=backend)}&run_name={run_name}'
                else:
                    url = self.WES_RUNS_URL.format(backend=backend)
    ...
    def wes_files(self, run_id) -> requests.Response:
        try:
            ...
            backend = ctx.args.backend
            token = BaseAuth.get_token().get('tokens').get('access')
            with requests.get(url=f"{self.WES_RUNS_FILE_URL.format(id=run_id,backend=backend)}",
                              headers={"Authorization": f"Bearer {token}"}) as response:
                if response.status_code not in [requests.codes.ok]:
    ...
    def get_runtime_setting(self, runtime_name: str) -> dict:
        ...
        backend = ctx.args.backend
        token = BaseAuth.get_token().get('tokens').get('access')
        with requests.get(url=f"{self.WES_RUNTIME_OPTIONS_URL.format(name=runtime_name, backend=backend)}",
                          headers={"Authorization": f"Bearer {token}"}) as response:
            if response.status_code not in [requests.codes.ok]:
                raise requests.HTTPError(f"{response.status_code}: {repr(response.text)}")
            return response.json()
    ...
    def get_run_id(self, run_id) -> dict:
        token = BaseAuth.get_token().get('tokens').get('access')
        with requests.get(url=f"{self.WES_BASE_URL}runs/{run_id}/",
                          headers={"Authorization": f"Bearer {token}"}) as response:
            if response.status_code not in [requests.codes.ok]:
                raise requests.HTTPError(response.content)
            return json.loads(response.content)
