...
from enoceanx.protocol import crc8
from enoceanx.protocol.eep import EEP
from enoceanx.protocol.constants import PACKET, RORG, PARSE_RESULT, DB0, DB2, DB3, DB4, DB6
...
class Packet(object):
    ...
    def _bit_data(self):
        ...
        return enoceanx.utils.to_bitarray(self.data[1:len(self.data) - 5], (len(self.data) - 6) * 8)
    ...
    def _bit_data(self, value):
        ...
        for byte in range(len(self.data) - 6):
            self.data[byte+1] = enoceanx.utils.from_bitarray(value[byte * 8:(byte + 1) * 8])
    ...
    def _bit_status(self):
        return enoceanx.utils.to_bitarray(self.status)

    @_bit_status.setter
    def _bit_status(self, value):
        self.status = enoceanx.utils.from_bitarray(value)
    ...
    def parse(self):
        ...
        if self.rorg in [RORG.RPS, RORG.BS1, RORG.BS4]:
            ...
            self.repeater_count = enoceanx.utils.from_bitarray(self._bit_status[4:])
        return self.parsed
...
class RadioPacket(Packet):
    ...
    @property
    def sender_int(self):
        return enoceanx.utils.combine_hex(self.sender)

    @property
    def sender_hex(self):
        return enoceanx.utils.to_hex_string(self.sender)

    @property
    def destination_int(self):
        return enoceanx.utils.combine_hex(self.destination)

    @property
    def destination_hex(self):
        return enoceanx.utils.to_hex_string(self.destination)
        ...
        if self.rorg == RORG.BS4:
            ...
            if self.learn:
                ...
                if self.contains_eep:
                    ...
                    self.rorg_func = enoceanx.utils.from_bitarray(self._bit_data[DB3.BIT_7:DB3.BIT_1])
                    self.rorg_type = enoceanx.utils.from_bitarray(self._bit_data[DB3.BIT_1:DB2.BIT_2])
                    self.rorg_manufacturer = enoceanx.utils.from_bitarray(self._bit_data[DB2.BIT_2:DB0.BIT_7])
...
class UTETeachInPacket(RadioPacket):
    ...
    def parse(self):
        ...
        self.request_type = enoceanx.utils.from_bitarray(self._bit_data[DB6.BIT_5:DB6.BIT_3])
        self.rorg_manufacturer = enoceanx.utils.from_bitarray(self._bit_data[DB3.BIT_2:DB2.BIT_7] + self._bit_data[DB4.BIT_7:DB3.BIT_7])  # noqa: E501
    ...
    def create_response_packet(self, sender_id, response=TEACHIN_ACCEPTED):
        ...
        data = [self.rorg] + \
               [enoceanx.utils.from_bitarray([True, False] + response + [False, False, False, True])] + \
               self.data[2:8] + \
               sender_id + [0]
