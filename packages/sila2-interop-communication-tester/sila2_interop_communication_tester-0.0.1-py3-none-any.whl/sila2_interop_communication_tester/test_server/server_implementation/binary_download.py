...
from sila2_interop_communication_tester.test_server.helpers.raise_error import raise_binary_download_failed_error
...
class BinaryDownloadImpl(BinaryDownloadServicer):
    ...
    def GetBinaryInfo(
        ...
        if not string_is_uuid(binary_id):
            raise_binary_download_failed_error(context, f"Given id is not a valid UUID: {request.binaryTransferUUID}")

        binary_uuid = uuid.UUID(binary_id)
        if binary_uuid not in self.binaries:
            raise_binary_download_failed_error(context, f"No binary exists with UUID {request.binaryTransferUUID}")
    ...
    def GetChunk(
        ...
        for request in request_iterator:
            ...
            if request.offset + request.length > len(binary):
                raise_binary_download_failed_error(
                    context,
                    f"Requested range is out of range for binary {request.binaryTransferUUID} of length {len(binary)} (offset: {request.offset}, length: {request.length})",
                )

            yield SiLABinaryTransfer_pb2.GetChunkResponse(
                binaryTransferUUID=request.binaryTransferUUID,
                offset=request.offset,
                payload=binary[request.offset : request.offset + request.length],
            )
