...
def test_requested_server_type_requested(server_calls):
    calls = server_calls["SiLAService.Get_ServerType"]
    assert any(call.successful for call in calls)


def test_requested_server_uuid_requested(server_calls):
    calls = server_calls["SiLAService.Get_ServerUUID"]
    assert any(call.successful for call in calls)


def test_requested_server_description_requested(server_calls):
    calls = server_calls["SiLAService.Get_ServerDescription"]
    assert any(call.successful for call in calls)


def test_requested_server_version_requested(server_calls):
    calls = server_calls["SiLAService.Get_ServerVersion"]
    assert any(call.successful for call in calls)


def test_requested_server_vendor_url_requested(server_calls):
    calls = server_calls["SiLAService.Get_ServerVendorURL"]
    assert any(call.successful for call in calls)


def test_requested_implemented_features_requested(server_calls):
