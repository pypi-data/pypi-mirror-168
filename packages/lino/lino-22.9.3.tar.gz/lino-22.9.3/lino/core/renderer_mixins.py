...
class JsCacheRenderer():
    ...
    def build_site_cache(self, force=False, from_commandline=False):
        """
        ...
        if not from_commandline and settings.SITE.never_build_site_cache:
            logger.debug(
                "Not building site cache because `settings.SITE.never_build_site_cache` is True")
            return
        if not os.path.isdir(settings.SITE.site_cache_root):
            try:
                os.mkdir(settings.SITE.site_cache_root)
            except FileNotFoundError:
                logger.debug(
                    "Not building site cache because " +
                    "parent directory '%s' of (settings.SITE.site_cache_root) does not exist.",
                    settings.SITE.site_cache_root.parent)
                return
