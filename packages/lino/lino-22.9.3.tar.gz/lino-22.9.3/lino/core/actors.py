...
class Actor(with_metaclass(ActorMetaClass, type('NewBase', (actions.Parametrizable, Permittable), {}))):
    ...
    def class_init(cls):
        ...
        if hasattr(cls, 'display_mode'):
            raise ChangedAPI(
                "{0} must convert `display_mode` to `display_mode_main` or `display_mode_slave`".format(cls))
    ...
    def slave_as_html(cls, master, ar):
        """
        Creates and returns the method to be used when :attr:`display_mode_main`
        is `html`.
        """
        ar = cls.request(
    ...
    def get_table_summary(cls, obj, ar):
        ...
        detail form when :attr:`display_mode_main` is `summary` or (2)
        in a grid.
