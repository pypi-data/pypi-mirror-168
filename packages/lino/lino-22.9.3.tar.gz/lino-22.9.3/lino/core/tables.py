...
class AbstractTable(actors.Actor):
    ...
    display_mode_main = constants.DISPLAY_MODE_TABLE
    """
    The default display_mode when the Table is the `main` layout element.
    """

    _display_mode_slave = None

    @classproperty
    def display_mode_slave(cls):
        """

        The display_mode when the Table is something other then the `main`
        layout element.

        """
        if cls._display_mode_slave is None:
            return cls.display_mode_main
        return cls._display_mode_slave

    @display_mode_slave.setter
    def display_mode_slave(cls, value):
        cls._display_mode_slave = value
