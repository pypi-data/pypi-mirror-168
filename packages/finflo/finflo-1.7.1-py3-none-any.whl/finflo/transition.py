...
class FinFlotransition():
    ...
    def __init__(self, action :str  , type : str , t_id : str , state :str , source : str , target : str , *args, **kwargs) -> None :
        self.action = action
        self.type = type 
    ...
    def Record_datas_get(self):
        ...
        try:
            ...
            for iter in work_model:
                ...
                print(overall_model[0].workflowitems.transitionmanager.t_id)
                print(gets_model)
                try:
                    datas = gets_model.objects.filter(id = overall_model[0].workflowitems.transitionmanager.t_id).first()
                    models_dict = model_to_dict(datas) 
                    if datas is not None:
                        break
                    continue
                except:
                    continue
            return {"values" : str(models_dict)}
        except:
            return {"values" : None}
    ...
    # SPECIAL DECORATOR FUNCTION  SOURCE AND TARGET FITTING

    def manualtransitions(self):
        overall_model = FinFlotransition.gets_all_models(self)
        try:
            # work_model = settings.FINFLO['WORK_MODEL']  
            gets_model =  apps.get_model(type)
            gets_model.objects.filter(t_id = self.t_id , type = self.type)
            # now set the source and target 
            gets_wf = self.gets_wf_item(self.t_id)
            wf = workflowitems.objects.update_or_create( transitionmanager=overall_model[0] or overall_model[0].id, defaults= {"initial_state" :  self.source , "interim_state" : None,
                "final_state" : self.target, "action" : self.action, "subaction" : self.action , "next_available_transitions" : None, "model_type" : self.type.upper(), "event_user" : get_current_user() , "current_from_party" : None , "current_to_party" : None})
            workevents.objects.create(workflowitems=overall_model[3] , event_user=get_current_user(),  initial_state=self.source  , record_datas = self.Record_datas_get() , 
                interim_state = None , final_state=self.target , action= self.action, subaction = self.action ,type=self.type.upper(), from_party = None , to_party = None)
        except:
            raise ModelNotFound()
        # return None
        # return self.manualtransitions()  -> max recursion depth  
    ...
    def return_transition(self):
        ...
        if self.action == self.gets_default_return_action().description.upper():
            ...
            workevents.objects.create(workflowitems=overall_model[3] , event_user=get_current_user(),  initial_state=overall_model[3].initial_state,final_value = True , record_datas = self.Record_datas_get() , 
        ...
        if overall_model[0] is not None :
            def Transition_Handler():
                    ...
                    # MAKER TRANSITION 
                        ...
                        workevents.objects.create(workflowitems=overall_model[3], event_user=get_current_user(),  initial_state=overall_model[1].from_state.description,
                                                  interim_state = overall_model[4][1 + overall_model[0].sub_sign], record_datas = self.Record_datas_get() , final_state=overall_model[1].to_state.description, action= self.action, subaction= self.action, type= self.type.upper(), from_party = overall_model[1].from_party , to_party = overall_model[1].from_party)
                    ...
                    #  TRANSITION 
                        ...
                        try:
                            ...
                            workevents.objects.create(workflowitems=overall_model[3], event_user=get_current_user(),  initial_state=overall_model[1].from_state.description,
                                                  interim_state = overall_model[1].to_state.description,record_datas = self.Record_datas_get() ,  final_state=overall_model[1].to_state.description, action= self.action, subaction= self.action, type = self.type.upper(), from_party = overall_model[1].from_party , to_party = overall_model[1].to_party , final_value = True)
                            overall_model[0].sub_sign = 0
                            overall_model[0].save()
...
# MANUAL TRANSITION STATEMACHINE
