...
from .models import Action , TransitionManager , workevents , workflowitems
...
class TransitionApiView(APIView):
    ...
    def post(self,request):
        ...
        if type and t_id  is not None:
            transitions = FinFlotransition(action = action , type = type , t_id = t_id , source = source , target = target )
...
class ActionListApi(ListCreateAPIView):
    ...
    def list(self, request):
        queryset = Action.objects.all()
        datas  = TransitionManager.objects.filter(t_id = 5).first()
        cores = model_to_dict(datas)
        print(cores)
        values = json.dumps(dict(cores) , cls = DjangoJSONEncoder )
        print("the type is " , type(values))
        serializer = Actionseriaizer(queryset, many=True)
        return Response({"status": "success" , "values" : cores , "data": serializer.data}, status=status.HTTP_200_OK)
