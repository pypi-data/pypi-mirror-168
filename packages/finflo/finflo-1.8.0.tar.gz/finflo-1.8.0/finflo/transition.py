...
class FinFlotransition():
    ...
    def __init__(self, action :str  , type : str , t_id : str , source : str , target : str , *args, **kwargs) -> None :
    ...
    def manualtransitions(self):
        try:
            ...
            wf = workflowitems.objects.update_or_create( transitionmanager= queryset , defaults= {"initial_state" : self.source, "interim_state" : self.source ,
                    "final_state" : self.target, "model_type" : self.type.upper(), "event_user" : get_current_user() })
            wf.save()
            workevents.objects.create(workflowitems = wf , event_user=get_current_user(),  initial_state=self.source  , 
                    interim_state = None , final_state=self.target ,type=self.type.upper())
        except:
            return None  
