...
class ThreadOtherListServices:
    ...
    def thread_extend_field_list(params):
        ...
        extend_field_obj = ThreadExtendField.objects.annotate(field_label=F("value")).annotate(category_value=F("category__value"))
        extend_field_obj = extend_field_obj.filter(**params)
        obj_list = extend_field_obj.values('category_value', "field", 'field_label', 'category_id', 'type', 'unit', 'config')
