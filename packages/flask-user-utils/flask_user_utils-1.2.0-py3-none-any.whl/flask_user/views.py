...
class SuperUserLoginView(MethodView):

    def get(self) -> Response:
        return render_template('user/superuser_login.html', form=UserForm(),
                               form_action=url_for('.make_superuser_login'))


class MakeSuperUserLoginView(MakeLoginView):

    def post(self):
        self.validator.superuser_login = True
        return super().post()
