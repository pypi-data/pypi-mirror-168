...
def set_launcher_icons(project_main, launchers_path):
    ...
    fh.copy_dir_with_permission_denied(launchers_path, project_main)
...
def manipulate(xml_path, xml, place_holder_map, on_backup, on_pre_build):
    ...
    if create_alias.lower() == 'true':
        ...
        fh.copy_dir_with_permission_denied(project_path, alias_path)
    ...
    if on_backup is not None:
        on_backup(project_path, old_package_name, package_name)
