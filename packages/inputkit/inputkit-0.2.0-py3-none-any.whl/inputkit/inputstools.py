...
__all__ = ["keys_","input_wait_for","check_input","limit_input","ignore_input","subprocess_input","system_input","urlparse_input","urlparse_input_","yn_input","regex_input","json_input"]
...
def limit_input(prompt:str="",length:int=None,end:str="\n"): # Test input
	...
	while len(string) != int(length):
			...
			if str(char) == "\r":
				...
				raise InputExit("Interrupt key have been pressed {key}".format(key=key),key=key,input=string)
			elif key == b"\x08":
				string=string[:-1]
				if string.strip():
					sys.stdout.write('\x0D'+ prompt + (string) + " ")
...
def json_input(prompt:str=""):
	""" take an input as json return dict object """
	import json
	def check(cls,input):
		try:
