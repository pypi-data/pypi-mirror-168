...
def password_input_(prompt='',*,star="*",space=" ",refesh="",end="\n",newline=True,encoding='cp1256'):
    ...
    while True:
        ...
        if c == b'\r':
            ...
            try:
                if repr(c)[2:-1].startswith("\\x"): password += c.decode(encoding);raise UnicodeDecodeError(encoding,c,len(password)-1,len(password),"invild input")
...
def password_input(*args,**kw):
    return password_input_(*args,**kw)
##########################
