...
async def async_annotate_match(
    ...
    if hgvs_batch or all(type(d) == str for d in data):
        csra_batch = list(data)
    ...
    for record in calls:
        if record["__type__"] != "HEADER":
            if record.get("mutation_type") not in SV_MUTATION_TYPES:
                msg = (
                    f"unknown SVTYPE of "
                    f"'{record.get('mutation_type', None)}'\n"
...
def add_annotation_hgvs_match(
    annotations_list: AnnotationMatchList, expected_bundle_version, call: str
):
    annotations = {}
    if annotations_list:
        annotations = (
            obj_to_dict(None, annotations_list.get_annotation(call)) or {}
        )
