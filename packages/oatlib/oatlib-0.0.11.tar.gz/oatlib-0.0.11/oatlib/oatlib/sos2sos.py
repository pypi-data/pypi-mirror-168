...
def istSOS2istSOS(from_istSOS, to_istSOS, to_sensors, gen_settings = {}, verbose=False, minlog=False):
    ...
    for dest_sens, obs in list(to_sensors.items()):
        ...
        for key, dest_proc in list(obs.items()):
            ...
            if verbose:
                print("|")
                print(f"|----> aggregating {key.split(':')[-1]} with settings: {settings}")
            ...
            if 'origin_endposition' in dest_proc and dest_proc['origin_endposition']:
                ...
                if verbose:
                    print(f"|----- last_origin_obs from {dest_proc['origin_endposition']}")
            ...
            if verbose:
                print(f"|----- last_origin_obs/last_dest_obs: {last_origin_obs}/{last_dest_obs}")
            ...
            if 'origin_agg_method' in dest_proc and 'origin_agg_freq' in dest_proc:
                ...
                aggr = ou.sensorAggergate(
                            ...
                            period = f"{last_dest_obs.isoformat()}/{last_origin_obs.isoformat()}" if settings['full_gap'] else None
                        )
