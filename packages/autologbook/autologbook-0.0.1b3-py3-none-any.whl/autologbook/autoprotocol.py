...
class HTMLObject(object):
    ...
    def print_html(self, indentation=True):
        """
    ...
    def save_html_to_file(self, filename, indentation=False):
        """
...
class Attachment(HTMLObject):
    ...
    def __init__(self, path=None):
        ...
        path : str | path-like, optional
            The full path to the attchament.
            The default is None.
    ...
    def generate_html_code(self, yaml_dict):
        """
        ...
        with self.tag('tr'):
            ...
            with self.tag('td'):
                with self.tag('b'):
                    self.text('Size: ')
                self.text(f'{autotools.pretty_filesize(self.params["size"])}')
            with self.tag('td'):
                with self.tag('b'):
...
class AttachmentDict(ResettableDict):
    ...
    def add(self, attachment):
        ...
        Raises
        ------
        TypeError: if attachment is not of one of the acceptable types.
...
class Protocol(HTMLObject):
    ...
    def remove_attachment(self, attachment_path):
        """
    ...
    def remove_sample(self, sample_full_name):
        """
        Remove one sample.
    ...
    def generate_custom_html(self, main_key):
        """
    ...
    def generate_html_subsamplelist_recursively(self, sample_list):
        """
    ...
    def _check_subsamples(self, sample_list, parent):
                    ...
                    with self.tag('tbody'):
                        for key, image in sample.images.items():
                            ...
                            if image.getID() in self.yamlDict:
                                ...
                                ydict = {}
                            image.generate_html_code(ydict)
                self.doc.stag('br')
...
class ELOGProtocol(Protocol):
    ...
    def initialize_yaml(self):
        ...
        self.yamlFilename = Path(self.path) / Path(f'protocol-{self.protocol}.yaml')
    ...
    def post_elog_message(self, skip_attachments=False):
        """
        ...
        for ID in protocol_ids:
            ...
            message, attributes, attachments = self.elog_instance.read(ID)
...
class QuattroELOGProtocol(ELOGProtocol):
    ...
    def generate_html_navigation_camera_images(self):
        ...
        if len(self.navcamimages) == 0:
            ...
            with self.tag('table', align='left', border='1', cellpadding='1', cellspacing='1',
                          style='float:none;width:%spx; border-collapse:collapse'
                                % autoconfig.IMAGE_NAVIGATION_MAX_WIDTH):
                with self.tag('tbody'):
    ...
    def add_navigation_camera_image(self, path):
        """
    ...
    def remove_navigation_camera_image(self, path):
        """
...
class Sample(HTMLObject):
    ...
    def add_microscope_picture(self, micro_pic):
        """
        ...
        """
        self.images[micro_pic.params['path']] = micro_pic
    ...
    def remove_microscope_picture_path(self, path):
        """
    ...
    def remove_microscope_picture(self, micro_pic):
        """
        ...
        """
        if micro_pic.params['path'] in self.images:
            ...
            self.emit_removed(ElementType.MICROSCOPE_PIC,
                              micro_pic.params['path'], self.full_name)
            del (self.images[micro_pic.params['path']])
        else:
            log.warning('Attempt to remove %s from sample %s, but it was not there' % (
                micro_pic.params['fileName'], self.full_name))

    def add_subsample(self, child_sample_name):
        """
    ...
    def remove_subsample(self, child_sample_name):
...
class MicroscopePicture(HTMLObject):
    ...
    def __init__(self, path=None, pic_type=None):
        """
    ...
    def generate_png(self, max_width: int = 400, subfolder: str = 'png-thumb', force_regen: bool = False) -> str:
        ...
        if not force_regen and exists(thumbfile):
            ...
            if max_width == 0:
                ...
                log.debug('Generating thumbnail of {} with max_width {}'.format(
...
class FEIPicture(MicroscopePicture):
    ...
    def __init__(self, path=None, pic_type=PictureType.FEI_MICROSCOPE_PICTURE):
    ...
    def crop_databar(self, force_regen=False):
        """
...
class QuattroFEIPicture(FEIPicture):
    ...
    def __init__(self, path=None, pic_type=PictureType.QUATTRO_MICROSCOPE_PICTURE):
...
class VersaFEIPicture(FEIPicture):
    ...
    def __init__(self, path=None, pic_type=PictureType.VERSA_MICROSCOPE_PICTURE):
        FEIPicture.__init__(self, path, pic_type)
        if path is not None:
