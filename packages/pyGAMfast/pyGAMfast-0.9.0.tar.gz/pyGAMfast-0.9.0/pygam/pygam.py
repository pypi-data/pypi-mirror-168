...
class GAM(Core, MetaTermMixin):
    ...
    def _linear_predictor(self, X=None, modelmat=None, b=None, term=-1, use_sparse: bool = True, ignore_checks: bool = False):
        """linear predictor
        ...
        """
        if modelmat is None:
            modelmat = self._modelmat(X, term=term, use_sparse=use_sparse, ignore_checks=ignore_checks)
        if b is None:
            b = self.coef_[self.terms.get_coef_indices(term)]
        return modelmat.dot(b).flatten()

    def predict_mu(self, X, use_sparse: bool = True, ignore_checks: bool = False):
        """
        ...
        lp = self._linear_predictor(X, use_sparse=use_sparse, ignore_checks=ignore_checks)
    ...
    def _modelmat(self, X, term=-1, use_sparse: bool = True, ignore_checks: bool = False):
        """
        ...
        return self.terms.build_columns(X, term=term, return_sparse=use_sparse)
...
class LogisticGAM(GAM):
    ...
    def predict(self, X, use_sparse: bool = True, ignore_checks: bool = False):
        """
        ...
        """
        return self.predict_mu(X, use_sparse=use_sparse, ignore_checks=ignore_checks) > 0.5

    def predict_proba(self, X, use_sparse: bool = True, ignore_checks: bool = False):
        """
        ...
        y : np.array of shape (n_samples,)
            containing expected values under the model
        """
        return self.predict_mu(X, use_sparse=use_sparse, ignore_checks=ignore_checks)
