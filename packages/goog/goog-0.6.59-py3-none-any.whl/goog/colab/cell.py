...
def webkit_GUI_hook(sc_port, _screenshot=None):
    def sc():
...
'''.replace("<mouse_uuid>", mouse_uuid)
    ...
    for _ in [
        (update_uuid, screenshot_cb),
        (mouse_uuid, mouse_cb),
        (wheel_uuid, wheel_cb),
        (keyboard_uuid, keyboard_cb),
        (get_tabs_uuid, get_tabs_cb),
        (set_tab_uuid, set_tab_cb),
        (close_tab_uuid, close_tab_cb),
    ]:
