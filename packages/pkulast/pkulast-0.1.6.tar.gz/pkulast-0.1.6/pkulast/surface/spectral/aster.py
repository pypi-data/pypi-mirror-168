...
def read_aster_file(filename):
    ...
    measurementLineNum = 0
    descriptionLineNum = 0
    ...
    for line in fin.readlines():
        ...
        if nItems == 1:
            ...
            LOG.info('more than 2 values on signature line,', filename)
            continue
        try:
            x = float(pair[0])
        except:
            x = 0
            LOG.info('corrupt signature line,', filename)
        if x == 0:
...
class AsterDatabase(SpectralDatabase):
    ...
    def get_sample(self, sampleID):
        import array
        query = '''SELECT Samples.SampleID, Samples.Name, SpectrumID,  XData, YData
                FROM Samples, Spectra WHERE Samples.SampleID = Spectra.SampleID
                AND Spectra.SampleID = ?'''
    ...
    def get_name(self, SampleID):
        query = '''SELECT Name FROM Samples WHERE SampleID = ?'''
        result = self.cursor.execute(query, (SampleID,))
        rows = result.fetchall()
        if len(rows) < 1:
            raise 'Measurement record not found'
