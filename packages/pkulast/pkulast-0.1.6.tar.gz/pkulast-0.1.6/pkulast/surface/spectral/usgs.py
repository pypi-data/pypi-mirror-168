...
from pkulast.surface.spectral.spectral_database import SpectralDatabase
...
class USGSDatabase(SpectralDatabase):
    ...
    def get_description(self, SampleID):
        query = '''SELECT Description FROM Samples WHERE SampleID = ?'''
        result = self.cursor.execute(query, (SampleID,))
        rows = result.fetchall()
        if len(rows) < 1:
    ...
    def get_sample(self, SampleID):
        sample_list = []
        query = '''SELECT ValuesArray, AssumedWLSpmeterDataID, LibName, Record, Description, Spectrometer, Purity, MeasurementTypeDescription FROM Samples WHERE SampleID = ?'''
        result = self.cursor.execute(query, (SampleID, ))
        rows = result.fetchall()
        if len(rows) < 1:
            raise Exception('Measurement record not found.')
        for row in rows:
            y = array_from_blob(row[0])
            assumedWLSpmeterDataID = row[1]
            info = []              
            info.append(row[2:])
            query = '''SELECT ValuesArray FROM SpectrometerData WHERE SpectrometerDataID = ?'''
            result = self.cursor.execute(query, (assumedWLSpmeterDataID, ))
            spec_rows = result.fetchall()
            if len(spec_rows) < 1:
                raise Exception('Measurement (wavelengths) record not found.')
            x = array_from_blob(spec_rows[0][0])
            info.append((x, y))
            sample_list.append(info)
        return sample_list

    def get_name(self, SampleID):
        query = '''SELECT Description FROM Samples WHERE SampleID = ?'''
        result = self.cursor.execute(query, (SampleID,))
        rows = result.fetchall()
        if len(rows) < 1:
    ...
    def create_envi_spectral_library(self, spectrumIDs, bandInfo):
        ...
        '''
        from pkulast.surface.spectral.resampling import BandResampler
        from pkulast.surface.spectral.envi import SpectralLibrary
        import numpy
        import unicodedata
