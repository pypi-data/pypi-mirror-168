...
import tempfile
import subprocess
from pkulast.raster.imagewriter import DEFAULTDRIVERNAME
from pkulast.raster.imagewriter import DEFAULTCREATIONOPTIONS
from pkulast.raster import cuiprogress
from pkulast.raster.imagereader import ImageReader
from pkulast.raster.imageio import NumpyTypeToGDALType
...
class Vector(object):
    ...
    def __init__(self, filename, inputlayer=0, burnvalue=DEFAULTBURNVALUE,
        ...
        if self.ds is None:
            raise ImageOpenException("Unable to open OGR dataset: %s" % filename)
        self.layer = self.ds.GetLayer(inputlayer)
        if self.layer is None:
            raise VectorLayerException("Unable to find layer: %s" % inputlayer)
        ...
        if attribute is not None:
            ...
            if fieldidx == -1:
                raise VectorAttributeException("Attribute does not exist in file: %s" % attribute)
    ...
    def reproject(self, proj):
        ...
        if proc.returncode != 0:
            ...
            raise VectorProjectionException(msg)
    ...
    def rasterizeSingle(info, vector, progress):
        ...
        try:
            if info.isFirstBlock():
                ...
                if outds is None:
                    raise ImageOpenException("Unable to create temporary file %s" % vector.temp_image)
                ...
                if err != gdal.CE_None:
                    raise VectorRasterizationException("Rasterization failed")
