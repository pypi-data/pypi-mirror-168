...
class ImageInfo(object):
    ...
    def __init__(self, filename, omitPerBand=False):
        ...
        if not is_HDF_EOS_subdataset and not os.path.exists(filename):
            raise FileOpenException("Unable to open file %s"%filename)
    ...
    def layerNameFromNumber(self, layerNumber):
        ...
        if ndx >= 0 and ndx < self.rasterCount:
            ...
            raise GDALLayerNumberException("Layer number %s is outside range 1-%d"%
...
class VectorFileInfo(object):
    ...
    def __init__(self, filename):
        ...
        if ds is None:
            raise VectorLayerException("Unable to open vector dataset '%s'"%filename)
...
class VectorLayerInfo(object):
    ...
    def __init__(self, ds, i):
        lyr = ds.GetLayer(i)
        if lyr is None:
            raise VectorLayerException("Unable to open layer %s in dataset '%s'"%(i, ds.GetName()))
...
class ColumnStats(object):
    ...
    def __init__(self, band, columnName, includeImageNull=False, histogramweighted=True):
        ...
        if columnNdx == -1:
            raise AttributeTableColumnException("Cannot find RAT column '%s'"%columnName)
