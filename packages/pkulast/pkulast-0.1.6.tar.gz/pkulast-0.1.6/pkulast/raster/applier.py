...
from pkulast.raster import imagereader
from pkulast.raster import imagewriter
from pkulast.raster import imageio
from pkulast.raster import vectorreader
from pkulast.raster import calcstats
from pkulast.raster import rat
from pkulast.raster.parallel import jobmanager
        ...
        if name not in writerdict:
            ...
            if isinstance(outfileName, list):
                ...
                if len(outblock) != numFiles:
                    raise MismatchedListLengthsException(("Output '%s' writes %d files, "+
...
def separateVectors(infiles):
    ...
    for name in nameList:
        ...
        if opensAsRaster(testFilename):
            ...
            raise FileOpenException("Failed to open file '%s' as either raster or vector"%testFilename)
