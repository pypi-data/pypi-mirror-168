...
def _single_user_execute(mode_dir, mode_id):
    ...
    os.chdir(work_dir)
    process = subprocess.run(['./run_mod5.sh'],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            input='',
                            encoding='ascii')
    output =  process.stdout #.decode() #check_output('./run_mod5.sh', shell=False).decode()
    if 'Error' in output:
        # os.chdir(cwd)
        raise Exception(
            output.split(
...
class ModtranWrapper:
    ...
    def correction(self,
        ...
        process = subprocess.run(['./run_mod5.sh'],
                                ...
                                input='',
                                encoding='ascii')
        output = process.stdout #check_output('./run_mod5.sh', shell=False).decode('utf-8')
        if 'Error' in output:
            os.chdir(cwd)
            raise Exception(
                output.split(
        ...
        while not (os.path.exists(UWARD_DIR + '.chn')
            ...
            continue
        os.chdir(cwd)
        # output = check_output(f'start /D "{RTM_MODTRAN_DIR}" ./run_mod5.sh /WAIT',shell=True)
        if self.filter.unit == 'um':
            factor = 1e-3
        elif self.filter.unit == 'nm':
            factor = 1
...
def run_modtran(profiles,
    ...
    #                         input='',
    #                         encoding='ascii')
    process = subprocess.run(['./MOD5_win.exe'],
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             input='',
                             encoding='ascii')
    ...
    for folder in folders:
        if not os.path.exists(os.path.join(work_dir, folder)):
            os.symlink(os.path.join(RTM_MODTRAN_DIR, folder),
                       os.path.join(work_dir, folder))
    os.chdir(work_dir)
    process = subprocess.run([RTM_MODTRAN_DIR + '/MOD5_win.exe'],
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             input='',
                             encoding='ascii')
