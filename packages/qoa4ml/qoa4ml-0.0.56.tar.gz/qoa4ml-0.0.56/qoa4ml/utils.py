...
def get_proc_cpu(pid = None, key=None):
    ...
    if key == "idle":
        cpu_usage = process.cpu_times().idle
        for child in child_list:
            cpu_usage += child.cpu_times().idle
    elif key == "system":
        cpu_usage = process.cpu_times().system
        for child in child_list:
            cpu_usage += child.cpu_times().system
    elif key == "all":
        cpu_usage = process.cpu_times().system + process.cpu_times().idle + process.cpu_times().user
        for child in child_list:
            cpu_usage = cpu_usage + child.cpu_times().system + child.cpu_times().idle + child.cpu_times().user
    else:
