...
def DID_ConBased(peak_dict,core_dict,origin_data):
    peak_value = []
    peak_location = []
    clump_center=[]
    clump_com = []
    ...
    for key in peak_dict.keys():
        ...
        clump_center.append(peak_dict[key])
        ...
        size = np.sqrt((mass_array*(np.array(core_dict[key])**2)).sum(0)/od_mass.sum()-\
                       ((mass_array*np.array(core_dict[key])).sum(0)/od_mass.sum())**2)*2.35
    ...
    detect_infor_dict['peak_value'] = np.around(peak_value,3)
    ...
    detect_infor_dict['clump_sum'] = np.around(clump_sum,3)
