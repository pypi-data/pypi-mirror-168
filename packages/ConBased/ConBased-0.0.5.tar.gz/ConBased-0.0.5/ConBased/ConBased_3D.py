...
def Get_Regions(origin_data, RMS, RegionMin=8, threshold='otsu'):
    regions = []
    kopen_radius = 1
    n_dilation = 1
    ...
    #         open_data[:,:,i] = morphology.opening(origin_data[:,:,i] > threshold,morphology.disk(kopen_radius))
    #         open_data[:,:,i] = ndimage.binary_fill_holes(open_data[:,:,i])
...
def Connectivity(core_dict, i_num, j_num):
    ...
    #     box_data = np.zeros_like(core_data)
    #     box_data[i_region[:,0],i_region[:,1],i_region[:,2]] = 1
    #     box_data[j_region[:,0],j_region[:,1],j_region[:,2]] = 1
    box_label = measure.label(box_data)
    box_region = measure.regionprops(box_label)
...
def Del_CP_Dict(core_data, peak_dict, core_dict, index, ClumpMin, DIntensity, DDistance, RegionMin):
    ...
    if len(pdk_sort) > 0:
        ...
        if connectivity > 1:
            ...
            #             print('lb_area:',lb_area)
            #             print('v_delta:',v_delta)
            #             print('nearest_dis_0:',nearest_dis_0)
            #             print('peak_dict[i_num]:',peak_dict[i_num])
            #             print('pdk_sort[k]:',peak_dict[pdk_sort[k]])
        ...
        if logic:
            #         if peak_delta_0 < peak_delta and nearest_dis_0 < nearest_dist:
            #             peak_dict[order] = peak_dict[i_num]
...
def DID_ConBased(center_dict, core_dict, origin_data):
    ...
    detect_infor_dict['clump_maximum'] = np.array(clump_center).tolist()
...
def Detect_ConBased(RMS, Threshold, RegionMin, ClumpMin, DIntensity, DDistance, origin_data):
    ...
    for i in tqdm(range(len(regions))):
        ...
        #         new_core_dict,new_peak_dict = Rebuild_CP_Dict(core_dict,peak_dict)
        core_dict, peak_dict_center = Update_CP_Dict(peak_dict, core_dict, core_data, ClumpMin, DIntensity, DDistance,
                                                     RegionMin)
        #         core_dict_center = Update_Core_Dict(new_peak_dict,peak_dict_center,new_core_dict)
...
if __name__ == '__main__':
    ...
    RegionMin = [3*3,3]  # [27,64,125]
    ClumpMin = [6*6,6]  # [125,...]
    DIntensity = 2 * RMS  # [-3*RMS,3*RMS]
    DDistance = [6,6]  # [6,16]

    origin_data = 'origin_data'
    did_ConBased = Detect_ConBased(RMS, Threshold, RegionMin, ClumpMin, DIntensity, DDistance, origin_data)