...
#     dist_record = [[],[]]
    ...
    _mdf1, _nomdf1, _mdf2, _nomdf2 = get_MatchFrame(ddf1, ddf2, usecols=['Cen1','Cen2','Cen3'], mindt=2.0, dbias=1)
    match_id_i = list(_mdf1.index)
    match_id_j = list(_mdf2.index)
    for i,j in zip(match_id_i,match_id_j):
        dist = ((origin_center[i][0]-com[j][0])**2 + (origin_center[i][1]-com[j][1])**2 + (origin_center[i][2]-com[j][2])**2)**(1/2)
...
def Get_IOU(infor_dict,com,regions_data):
    match_id_i = []
    match_id_j = []
    intersection_record = []
    union_record = []
    origin_center = infor_dict['clump_com']
...
# def Get_IOU(core_table,com,regions_data):
#     match_id_i = []
#     match_id_j = []
#     intersection_record = []
#     union_record = []
#     origin_center = core_table['infor_dict'].item()['center']
#     origin_regions = core_table['infor_dict'].item()['clump_regions']
#     detect_center = com
#     A = np.stack(origin_center,axis = 0)
#     ddf1 = pd.DataFrame({'Cen1': A[:, 0], 'Cen2': A[:, 1], 'Cen3': A[:, 2]})
#     B = np.stack(detect_center,axis = 0)
...
#     IOU = intersection_record/np.array(union_record)
# #     IOU = IOU.mean()
# #     IOU = IOU[np.where(IOU<IOU.mean()+IOU.std()) and np.where(IOU>IOU.mean()-IOU.std())].mean()
#     return IOU
...
if __name__ == '__main__':
    ...
    recall,precision,F1,dist_record,delta_flux,match_flux = Evaluate(origin_center,did_ConBased['clump_com'],origin_flux,did_ConBased['clump_sum'])
    IOU = Get_IOU(infor_dict,did_ConBased['clump_com'],did_ConBased['regions_data'])
