...
def Generate_Center(n, xres, yres, zres, sigma, ctime, nearest_dist):
    x_center = []
    y_center = []
    z_center = []
    length = 0
...
def Generate(n, xres, yres, zres, nearest_dist, peak_low, peak_high, ctime, sigma_one, sigma_two, sigma_three,
             angle=None):
    peak = []
    peak_value = []
    peak_location = []
    ...
    temp_sigma = np.array([np.array(sigma_one).mean(), np.array(sigma_two).mean(), np.array(sigma_three).mean()],
                          dtype='uint8')
    x_center, y_center, z_center = Generate_Center(5 * n, xres, yres, zres, temp_sigma, ctime, nearest_dist)
    i = 0
    temp_length = 1
    len_peak = 0
    len_region = 1
    while len(peak_value) < n:
        ...
        if len_peak == temp_length:
            ...
            x_size = core_x_max - core_x_min
            y_size = core_y_max - core_y_min
            z_size = core_z_max - core_z_min
            clump_length.append([x_size, y_size, z_size])
            clump_size.append(list(2 * math.sqrt(2 * math.log(2)) * sigma_t))
...
if __name__ == '__main__':
    ...
    start = time.time()
    origin_data, infor_dict = Generate(n, xres, yres, zres, nearest_dist, peak_low, peak_high, ctime, sigma_one,
                                                     sigma_two, sigma_three, angle)
