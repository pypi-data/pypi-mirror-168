...
class FieldParser:
    ...
    def parse(self):
        ...
        for field in self.fields:
            ...
            if in_function_parameters.__contains__(","):
                params = []
            ...
            elif field.__contains__(".") and not in_function_parameters.__contains__(","):
                table_name = field_split[-2] if field_split.__len__() in [2, 3] else table_name

                schema_name = field_split[-3] if field_split.__len__() in [3] else schema_name

                schema_name = f"{self.table_quote}{schema_name}{self.table_quote}."
                table_name = f"{self.table_quote}{table_name}{self.table_quote}."
                field = schema_name + table_name + f"{self.field_quote}{field}{self.field_quote}"

            elif not in_function_parameters.__contains__(","):
                field = field_split.strip()
