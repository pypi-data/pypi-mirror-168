...
)
{
    ...
    udt_VariableType*   new_varType = NULL;     //参数类型
    udt_VariableType*   tmp_varType = NULL;     //临时变量
    udint4              size;
    int                 is_udt;
    sdint2              param_type;
    ...
    {       
        ...
        if (new_var == NULL)
        {
                ...
                param_type == DSQL_PARAM_INPUT)
            {
                ...
                //sql_type是服务器推荐类型，如果是varchar，而绑定数据是int，则绑定类型用int
                if (bindParamDesc->sql_type == DSQL_VARCHAR && new_varType == &vt_Integer)
                {
                    bindParamDesc->sql_type = DSQL_INT;

                    bindParamDesc->prec     = 10;
                    bindParamDesc->scale    = 0;
                }
...
static PyMemberDef g_CursorMembers[] = {
    ...
    { "rowcount",       T_INT,          offsetof(udt_Cursor, totalRows),        READONLY },  /** 结果集总行数 **/
    { "rownumber",      T_INT,          offsetof(udt_Cursor, rowCount),         READONLY },   /** 游标所在当前位置0-based **/
    { "with_rows",      T_BOOL,         offsetof(udt_Cursor, with_rows),        READONLY },   /** 游标所在当前位置0-based **/
