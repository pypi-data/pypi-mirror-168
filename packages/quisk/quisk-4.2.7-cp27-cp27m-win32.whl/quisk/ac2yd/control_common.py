...
#   self.remote_radio_sound_port = self.remote_ctl_base_port + 2	# UDP port for radio sound to the speakers
#   self.remote_mic_sound_port = self.remote_ctl_base_port + 3		# UDP port for microphone sent to the remote radio
#
  ...
  def RemoteCtlRead(self):
    ...
    while '\n' in self.received:	# A complete response ending with newline is available
      ...
      if reply[0] in 'Qq':
        ...
        QS.start_control_head_remote_sound(self.remote_radio_ip, self.remote_radio_sound_port,
                 self.remote_mic_sound_port, self.graph_data_port)
