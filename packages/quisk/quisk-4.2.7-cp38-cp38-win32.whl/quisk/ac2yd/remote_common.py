...
#   self.remote_radio_sound_port = self.remote_ctl_base_port + 2	# UDP port for radio sound from the remote to the speakers
        ...
        if command == "TOKEN":
          ...
          if hmac.compare_digest(H.hexdigest(), args[1]):
            ...
            QS.start_remote_radio_remote_sound(self.control_head_ip, self.remote_radio_sound_port,
                       self.remote_mic_sound_port, self.graph_data_port, self.control_head_data_width)
          else:
            time.sleep(1)
