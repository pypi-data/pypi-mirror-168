...
from typing import cast, Type, TypeVar, Any, Generic, Generator, Callable, Optional
...
def avro_schema(class_: Type[ETPModel]) -> str:
    ...
    return cast(str, getattr(mod, "avro_schema"))  # cast is not a runtime check !
...
class Strict(Generic[T]):
    ...
    def validate(cls, value: Any) -> T:
        if not isinstancex(value, cls.__typelike__):
            raise TypeError(f"{type(value)} : -- {value!r} is not of valid type")
        return value

    @classmethod
    def __modify_schema__(cls, field_schema, field: Optional[ModelField] = None):
