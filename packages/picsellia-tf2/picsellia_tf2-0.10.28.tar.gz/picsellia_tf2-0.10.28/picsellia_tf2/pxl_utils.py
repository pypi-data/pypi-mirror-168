...
def train(ckpt_dir='', config_dir='', train_steps=None, use_tpu=False, checkpoint_every_n=100,
          record_summaries=True, log_real_time=None):
...
def get_confusion_matrix(input_tfrecord_path, model, labelmap, iou_thresh=0.5, confidence_thresh=0.5, draw_option=False, draw_save_path=""):
    """
    ...
    filename_matrix = [[[] for i in range(len(categories)+1)] for j in range(len(categories)+1)]
            ...
            if decoded_dict:
                ...
                for i in range(len(groundtruth_boxes)):
                    if matches.shape[0] > 0 and matches[matches[:,0] == i].shape[0] == 1:
                        ...
                        if filename not in filename_matrix[groundtruth_classes[i] - 1][int(detection_classes[int(matches[matches[:,0] == i, 1][0])] - 1)]:
                            filename_matrix[groundtruth_classes[i] - 1][int(detection_classes[int(matches[matches[:,0] == i, 1][0])] - 1)].append(filename)
                        if int(detection_classes[int(matches[matches[:,0] == i, 1][0])] - 1) == groundtruth_classes[i] - 1:
                            TP[groundtruth_classes[i]-1] += 1
                        else:
                            FP[int(detection_classes[int(matches[matches[:,0] == i, 1][0])] - 1)] += 1
                    else:
                        confusion_matrix[groundtruth_classes[i] - 1][confusion_matrix.shape[1] - 1] += 1
                        if filename not in filename_matrix[groundtruth_classes[i] - 1][confusion_matrix.shape[1] - 1]:
                            filename_matrix[groundtruth_classes[i] - 1][confusion_matrix.shape[1] - 1].append(filename)
                ...
                for i in range(len(detection_boxes)):
                    if matches.shape[0] > 0 and matches[matches[:,1] == i].shape[0] == 0:
                        ...
                        if filename not in filename_matrix[confusion_matrix.shape[0] - 1][int(detection_classes[i] - 1)]:
                            filename_matrix[confusion_matrix.shape[0] - 1][int(detection_classes[i] - 1)].append(filename)
