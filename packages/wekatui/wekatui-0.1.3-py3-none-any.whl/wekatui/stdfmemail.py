...
class EmailTreeLine(wekatui.TreeLine):
...
class EmailTree(wekatui.MultiLineTreeNew):
    ...
    def h_select(self, ch):
        ...
        self.editing = False
        self.how_exited=wekatui.wgwidget.EXITED_UP
        self.hidden  = True
    ...
    def h_save_message_part(self, ch):
        ...
        wekatui.notify_wait("Message part saved to your downloads folder: \n %s" % self.parent.DOWNLOAD_DIR)


class EmailPager(wekatui.Pager):
    def set_up_handlers(self):
...
class EmailViewFm(wekatui.SplitFormWithMenus):
    ...
    def create(self):
        ...
        self.nextrely = 1
        self.wSubject = self.add(wekatui.TitleText, begin_entry_at=10, editable=False,
                                 use_two_lines=False, name = "Subject:")
        self.wFrom    = self.add(wekatui.TitleText, begin_entry_at=10,
                                 editable=False, name = "From:", ) #max_width=-8)
        self.wDate    = self.add(wekatui.TitleText, begin_entry_at=10,
        ...
        self.nextrely      += 1
        self.wStatusLine    = self.add(wekatui.FixedText,
                                       editable=False,
                                       use_max_space=True,
                                       color='STANDOUT',
    ...
    def _parse_email_tree(self, this_email):
        ...
        self._this_email_tree = wekatui.NPSTreeData(content=this_email, ignoreRoot=False)
    ...
    def viewShortHeaders(self,):
        ...
        wekatui.notify_confirm(s_header_list, wide=True, wrap=False)
    ...
    def viewAllHeaders(self,):
        ...
        for headers in list(self.this_email.keys()):
            ...
            if these_headers:
                for h in these_headers:
                    s_header_list.append(str(headers).capitalize() + ": " + h.strip())
        wekatui.notify_confirm(s_header_list, wide=True, wrap=True)
    ...
    def viewMessageSource(self,):
        wekatui.notify_confirm(self.this_email.as_string(), wide=True)