...
pub fn find_intersecting_boxes(raw_boxes: &[Box])->Vec<Vec<u32>>{
    ...
    assert!((boxes.len() as u64) < ((1 as u64) << 32), "Only supports 4 billion boxes");
    let mut sorted_boxes: Vec<(CalcBox,u32)> = boxes.iter().enumerate().map(|(idx,b)|(*b,idx as u32)).collect();
    sorted_boxes.sort_by_key(|(b,_)|b.x1);

    // only stores boxes to the left ot itself
    let nodes_to_left:Vec<Vec<u32>> = sorted_boxes.par_iter().enumerate().map(|(sidx,(b1, _)) |
        sorted_boxes[sidx+1..].iter()
...
mod tests {
    ...
    fn test_boxes_perf(){
        ...
        .collect();
        let test_result = find_intersecting_boxes(&boxes);
        let num_adj:usize = test_result.iter().map(|l|l.len()).sum();
        println!("{}",num_adj);
        assert_eq!(num_adj, 109310);
    }
}