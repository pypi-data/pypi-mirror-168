...
def tags(config, team: str, artifact, url, output, limit):
    ...
    for row in rows:
        if url:
            row["more_details"] = "pierone describe --url {url} {team} {artifact} {tag}".format(url=url, **row)
        else:
            row["more_details"] = "pierone describe {team} {artifact} {tag}".format_map(row)

    with OutputFormat(output):
        titles = {
            "created_time": "Created",
            "created_by": "By",
            "more_details": "See more details with"
        }
        print_table(
            [
                "team",
...
def describe(config, team, artifact, tag, url):
    ...
    compliance_checker_details = compliance.get("checker", {}).get("details", "")
    status_details = markdown_2_cli(compliance_checker_details or "None")
