...
def log_system_information(logger_method=print, data_directory="/opt/qetl/users"):
    logger_method(
        f'SYS STAT: CPU usage={psutil.cpu_percent(interval=0.5)} %, '
        f'frequency='
        f'{get_cpu_frequency()} MHz, '
        f'count='
        f'{get_cpu_count()}'
    )
    logger_method(
        ...
        f'{get_ram_usage_pct()} %, '
        f'total='
        f'{get_ram_size_total_in_mb():,.0f} MB, '
        f'used='
        f'{get_ram_size_usage_in_mb():,.0f} MB, '
        f'available='
        f'{get_ram_size_available_in_mb():,.0f} MB '
    )
    logger_method(
        f'SYS STAT: SWAP usage='
        f'{get_swap_usage_pct()} %, '
        f'total='
        f'{get_swap_total_in_mb():,.0f} MB, '
        f'used='
        f'{get_swap_usage_in_mb():,.0f} MB, '
        f'available='
        f'{get_swap_available_in_mb():,.0f} MB '
    )

    try:
        logger_method(
            f'SYS STAT: DISK usage='
            f'{get_disk_size_pct(data_dir=data_directory)} %, '
            f'total='
            f'{get_disk_size_total(data_dir=data_directory):,.0f} MB, '
            f'used='
            f'{get_disk_size_usage(data_dir=data_directory):,.0f} MB, '
            f'available='
            f'{get_disk_size_remaining(data_dir=data_directory):,.0f} MB, '
            f'path='
            f'{data_directory}, '
        )
    except Exception as e:
...
def if_disk_space_usage_greater_than_90_log_warning(logger_method=print, data_directory="/opt/qetl/users"):
    try:
        disk_size_usage_pct = int(get_disk_size_pct(data_dir=data_directory))
...
def get_disk_size_pct(data_dir="/opt/qetl/users"):
    return psutil.disk_usage(data_dir).percent


def get_disk_size_total(data_dir="/opt/qetl/users"):
    return psutil.disk_usage(data_dir).total / 1024 / 1024


def get_disk_size_usage(data_dir="/opt/qetl/users"):
    return psutil.disk_usage(data_dir).used / 1024 / 1024


def get_disk_size_remaining(data_dir="/opt/qetl/users"):
