...
def test_preliminary_voevent(client, create_obj, obj_type):
    ...
    response = client.create_voevent(obj_id, 'PR', **PRELIMINARY_VOEVENT_DATA)
...
def test_initial_voevent(client, create_obj, obj_type):
    ...
    # Create VOEvent and check results
    response = client.create_voevent(obj_id, 'IN', **INITIAL_VOEVENT_DATA)
    assert response.status_code == 201
    data = response.json()
...
def test_update_voevent(client, create_obj, obj_type):
    ...
    # Create VOEvent and check results
    response = client.create_voevent(obj_id, 'UP', **INITIAL_VOEVENT_DATA)
    assert response.status_code == 201
    data = response.json()
...
def test_retraction_voevent(client, create_obj, obj_type):
    ...
    response = client.create_voevent(obj_id, 'RE')
    assert response.status_code == 201
