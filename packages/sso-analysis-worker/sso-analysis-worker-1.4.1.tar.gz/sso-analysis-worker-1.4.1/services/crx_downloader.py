...
def download(url, file_path=None) -> Tuple[str, str, str]:
    ...
    return download_stream(CRX_URL.format(version=CHROME_VERSION, ext_id=chrome_app_id, nacl_arch="x86-64"),
                           file_path if file_path else file_name), chrome_app_id, file_name
...
def download_stream(download_url, file_name):
    try:
        ...
        if content_length:
            logger.info("Downloading %s. File Size %s " % (file_name, byte_to_human(content_length)))
        else:
