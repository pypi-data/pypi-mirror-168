...
def parseOUTCAR(OUTCAR_in,foutlog):
    ...
    l1=fin.readline()
    while l1:
        if len(elems)==0 and "POTCAR:" in l1:
            while len(l1.strip())>4:
                l2=l1.split()
                e2=l2[2].split("_")[0]
                elems.append(e2)
                l1=fin.readline()
...
def calcEdefCorrectionVASP():
    ...
    if len(def0_elems)==0:
        ...
        foutlog.write("    Please try to adjust rtol_vac-parameter in the input file")
        sys.exit()
    if len(def0_elems)>1:
        ...
        foutlog.write("    Position_direct("+def0_elems[0]+") = "+str(def0_posi[0][0])+" "+str(def0_posi[0][1])+" "+str(def0_posi[0][2])+"\n")
        averaged_posi=[0.0,0.0,0.0] 
        p1=def0_posi[0]
        for id2 in range(1,len(def0_elems)):
            p2=def0_posi[id2]
            foutlog.write("    Position_direct("+def0_elems[id2]+") = "+str(p2[0])+" "+str(p2[1])+" "+str(p2[2])+"\n")
            pdiff1=[p2[0]-p1[0],p2[1]-p1[1],p2[2]-p1[2]]
            rdiff2min=1e10
            for s0 in [-1,0,1]:
                for s1 in [-1,0,1]:
        ...
        if len(def0_elems)>1:
            foutlog.write("    Position_direct(ave.) = "+str(averaged_posi[0])+" "+str(averaged_posi[1])+" "+str(averaged_posi[2])+"\n")
    ...
    foutlog.write("    Position_direct(adopted) = "+str(posi_def0[0])+" "+str(posi_def0[1])+" "+str(posi_def0[2])+"\n")
    foutlog.flush()
    ##### rdist from defect
    rdist=[]
    rmax=0.0
