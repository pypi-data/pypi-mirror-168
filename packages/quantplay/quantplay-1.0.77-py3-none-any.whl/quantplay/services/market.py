...
class Market:
    ...
    def initialize_broker(self):
        ...
        if "DEFAULT" in config and "preferred_broker" in config["DEFAULT"]:
            preferred_broker = config["DEFAULT"]["preferred_broker"]
            Constants.logger.info("Using {} as preferred broker".format(preferred_broker))
            if preferred_broker == "Zerodha":
                self.broker = Zerodha()
            elif preferred_broker == "AngelOne":
                self.broker = AngelOne()
            else:
                print("use 'quantplay broker generate-token' to setup broker'")
                raise Exception("Unable to create market instance because {} is not supported".format(preferred_broker))
