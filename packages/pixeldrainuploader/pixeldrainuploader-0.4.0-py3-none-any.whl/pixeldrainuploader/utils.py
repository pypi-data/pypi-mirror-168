...
class PathTranslator(PathTranslatorBase):
    def get(self):
        try:
            self.cache = pickle.load(open(self.identifier()["name"], "rb"))
        except:
            self.cache = {}
        return self.cache
    ...
    def update(self):
        pickle.dump(self.cache, open(self.identifier()["name"], "wb"))
        return True

    def purge(self):
        os.remove(self.identifier()["name"])
        self.cache = {}
        return True
