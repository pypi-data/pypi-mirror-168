...
def get_reponse_67985726() -> bytes:
    """Get response of VAT number 67985726."""
    return get_file_bytes(os.path.join(DATA_DIR_PATH, 'ares_std_response_67985726.xml'))
        ...
        with self.assertLogs(self.logger_name, level='DEBUG') as logs:
            with responses.RequestsMock() as rsps:
                rsps.add(
                    ...
                    body=get_reponse_67985726()
                )
                content = get_xml_content('67985726')
        data = get_reponse_67985726()
