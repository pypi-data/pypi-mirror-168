  ...
  constructor(
    ...
    this.tracker.activeCellChanged.connect(() => {
      if (this.tracker.activeCell?.model?.type === 'code') {
        const code_mirror_editor = this.tracker.activeCell
        ...
        const debounced_on_change = _.debounce(() => {
          ...
          const line = code_mirror_editor
            ...
            ?.trim();
          const isSqlMagic = line?.startsWith('%%sql');
          if (
            // if the first line is sql magic and mode is not sql, set mode to sql
            isSqlMagic &&
            code_mirror_editor.editor.getOption('mode') !== 'text/x-sql'
          ) {
            code_mirror_editor.editor.setOption('mode', 'text/x-sql');
          } else if (
            // if the first line is not sql magic but mode is sql, set it back to python
            !isSqlMagic &&
            code_mirror_editor.editor.getOption('mode') === 'text/x-sql'
          ) {
            code_mirror_editor.editor.setOption('mode', 'text/ipython');
          }
        }, 300);
