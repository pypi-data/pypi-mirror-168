...
def generate_input_func(args):
    ...
    # Read in extra coordinates if specified
    # These MUST have molcas extended xyz format
    if args.basis_xyz:
        extra_labels, extra_coords = xyzp.load_xyz(
            args.basis_xyz, capitalise=False
        )
        if len(args.xyz_output):
            xyz_out = args.xyz_output
        else:
            xyz_out = "coordinates.xyz"
    else:
        extra_labels = []
        extra_coords = []
        xyz_out = args.xyz_output
...
def read_args(arg_list=None):
    ...
    gen_inp.add_argument(
        ...
        type=str,
        default="",
        help='Name of the xyz-file containing coordinates and'
             + ' basis set labels in extended MOLCAS format.\n'
             + 'These MUST be in same coordinate system as input xyz\n'
             + 'n.b. if used this will output coordinates to file'
             + ' coordinates.xyz,\n(or other name given by --xyz_output'
             + ' argument) rather than molcas input'
    )
    ...
    gen_inp.add_argument(
        ...
        default=False,
        action='store_true',
        help='If True, atoms are not reordered in input file'
    )
