...
algorithms_available = algorithms_available.union(hashlib.algorithms_available)
...
def method_load(method):
    ...
    if isinstance(method, (BYTES_TYPE, STR_TYPE)):
        ...
        if loader:
            ...
            return lambda : hashlib.new(method)
    else:
        return method
...
def register_pbkdf2_password_hash(method, prefix, seperator, class_name_suffix=""):
    ...
    globals()[class_name] = type(class_name, (Pbkdf2PasswordHashBase,), {
...
def register_simple_password_hash(method, prefix, class_name_suffix=""):
    ...
    globals()[class_name] = type(class_name, (SimplePasswordHashBase,), {
        "method": method,
        "prefix": prefix,
    })
    return globals()[class_name]
...
def register_simple_salt_password_hash(method, prefix, hash_size=0, salt_length=8, class_name_suffix=""):
    ...
    globals()[class_name] = type(class_name, (SimpleSaltPasswordHashBase,), {
        "method": method,
        "prefix": prefix,
        "hash_size": hash_size,
...
def register_hexlify_password_hash(method, length=0, class_name_suffix=""):
    ...
    globals()[class_name] = type(class_name, (HexlifyPasswordHashBase,), {
        "method": method,
        "length": length,
    })
    return globals()[class_name]
