...
def get_project_names(
    ...
    return list(lookup.keys())  # type: ignore[union-attr]
...
def get_project_asset(
    ...
    try:
        project_assets = lookup[project_name]  # type: ignore[index]
    except KeyError:
        raise NPTKError("Invalid project_name=%s" % project_name)

    try:
        asset_path = project_assets[asset_name]
    except KeyError:
        raise NPTKError("Invalid asset_name=%s" % asset_name)

    if asset_path.endswith(".yml"):
        ...
        raise NPTKError("Unsupported data-manifestor file format: %s" % asset_path)
...
def put_project_asset(
    ...
    asset: Any,
    lookup: Optional[Dict[str, Dict[str, str]]] = None,
    project_asset_dir: Optional[str] = None,
    allen_config_path: Optional[str] = None,
) -> None:
    try:
        project_assets = lookup[project_name]  # type: ignore[index]
    except KeyError:
    ...
    if not isinstance(asset, dict) and not isinstance(asset, list):
        raise NPTKError("Invalid asset type. type=%s" % type(asset))

    # store each asset in it's own file to reduce project json size
    asset_id = uuid4().hex
    asset_path = os.path.join(
        project_asset_dir,  # type: ignore[arg-type]
        f"{asset_id}.json",
    )
    ...
    lookup[project_name][asset_name] = asset_path  # type: ignore[index]

    if allen_config_path:
        write_to_allen_config("projects", lookup, allen_config_path)
    else:
        write_to_allen_config("projects", lookup)
