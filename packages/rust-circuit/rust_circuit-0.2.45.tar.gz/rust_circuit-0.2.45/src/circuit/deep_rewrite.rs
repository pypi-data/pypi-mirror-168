...
use super::{
    ...
    visit_circuit, Add, Concat, GeneralFunction, HashBytes, Index,
};
...
pub fn compiler_simp(circ: &Circuit, opt_context: &mut OptimizationContext) -> CircuitRc {
    ...
    fn simplify_changed_descendants(
        circ: &Circuit,
        simplified: &mut HashMap<HashBytes, CircuitRc>,
    ) -> Option<CircuitRc> {
        ...
        circ.map_children_op(&mut |x: &Circuit| {
            if simplified.contains_key(&x.info().hash) {
                None
            } else {
                Some(fully_simplify(x.clone().rc(), simplified))
            }
        })
        .map(|c| c.rc())
    }
    ...
    fn fully_simplify(
        ...
        if let Some(result) = simplified.get(&circ.info().hash) {
            return result.clone();
        }
        let mut result: CircuitRc = circ
            .map_children_unwrap(&mut |x: &Circuit| fully_simplify(x.clone().rc(), simplified))
            .rc();
        for iter_count in 0.. {
            match compiler_simp_step(&result) {
                Some(r) => result = simplify_changed_descendants(&r, simplified).unwrap_or(r),
                None => break,
            }
            if iter_count > 50 {
                result.compiler_print();
        ...
        simplified.insert(circ.info().hash, result.clone());
        result
    }
    fully_simplify(circ.clone().rc(), &mut opt_context.cache.simplified)
}
...
pub fn deep_pull_concat(circuit: CircuitRc, min_size: Option<usize>) -> CircuitRc {
    ...
    apply_fn_until_same(&pulled, |x: &CircuitRc| {
        deep_push_down_index(compiler_simp(x, &mut cache), min_size)
    })
}
