...
impl RearrangeSpec {
    ...
    pub fn flatten(rank: usize) -> Self {
        ...
        RearrangeSpec {
            ...
            int_sizes: vec![OpSize::from(None); rank],
        }
    }
    ...
    pub fn is_valid(&self) -> bool {
        ...
        let result = is_unique(&self.ints_in_inp_it().collect::<Vec<_>>())
    ...
    /// ```
    /// # use rust_circuit::rearrange_spec::{OpSize, RearrangeSpec};
    /// let spec = RearrangeSpec::new(
    ///     vec![vec![0, 1], vec![2, 3]],
    ///     vec![vec![3, 2, 1, 0]],
    ///     vec![OpSize(3), OpSize(1), OpSize(1), OpSize(1)],
    /// );
    ...
    ///     vec![vec![1, 2], vec![5], vec![3, 4], vec![0]],
    ///     vec![
    ///         OpSize(2),
    ///         OpSize(2),
    ///         OpSize(2),
    ...
    ///     vec![vec![1], vec![3], vec![2], vec![0]],
    ///     vec![OpSize(2), OpSize(4), OpSize(4), OpSize(2)],
    /// );
    /// assert_eq!(spec.canonicalize(true), canonicalized_spec);
    /// ```
