...
use super::{
    computational_node::EinsumArgs, deep_map_unwrap, evaluate_fn, prelude::*, Add, Concat, Einsum,
    GeneralFunction, HashBytes, Index, Rearrange, ScalarConstant, Scatter, Symbol,
};
use crate::{
    filter_by_variant,
    opt_einsum::optimize_einsum_spec_cached,
    py_types::PY_CIRC_COMPILER_UTIL,
    ...
    util::{
        ...
        unique_to_appearance, AsOp, EinInt,
    },
};
...
#[pyfunction]
pub fn einsum_nest_optimize(einsum: &Einsum) -> Einsum {
    ...
    einsum_nest_path(einsum, path)
}

#[pyfunction]
pub fn deep_optimize_einsums(circ: CircuitRc) -> CircuitRc {
    deep_map_unwrap(&circ, |x| x.map_or_clone(einsum_nest_optimize))
}
...
pub fn remove_axes(circuit: &CircuitRc, axes: &HashSet<usize>) -> Option<CircuitRc> {
    ...
    match &***circuit {
        ...
        Circuit::Rearrange(rearrange) => {
            ...
            Some(
                rearrange
                    .evolve(None, Some(spec.filter_out_axes_unsafe(axes)))
                    .rc(),
            )
        }
        _ => {
...
pub fn add_pull_removable_axes(add: &Add, remove_non_common_axes: bool) -> Option<CircuitRc> {
    let removable_axes: Vec<HashSet<usize>> = add
        ...
        .iter()
        .map(|x| {
            let rank_difference = add.info().rank() - x.info().rank();
    ...
    let new_operands = zip(&add.nodes, removable_axes)
        .map(|(node, removable_here_base)| {
            ...
            if !remove_non_common_axes {
                ...
                Rearrange::nrc(raw_removed, broadcast_rspec, None)
            }
        })
        .collect();
    let add_circuit = Add::nrc(new_operands, add.name_cloned());
    Some(Rearrange::nrc(add_circuit, post_rearrange_spec, None))
}
...
pub fn push_down_index(node: &Index) -> Option<CircuitRc> {
    match &**node.node {
        ...
        Circuit::Concat(inner) => {
            ...
            let shape_non_axis = filter_out_idx(&inner.info().shape, &HashSet::from([inner.axis]))
                .into_iter()
                .collect();
            if index_non_axis.is_identity(&shape_non_axis) {
                return None;
            }
        ...
        Circuit::Rearrange(inner) => {
            ...
            python_result.map(|(index, spec)| {
                ...
                Rearrange::nrc(new_index, spec, inner.name_cloned())
            })
        }
