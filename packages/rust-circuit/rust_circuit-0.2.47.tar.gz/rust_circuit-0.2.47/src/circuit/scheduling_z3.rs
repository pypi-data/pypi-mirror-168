    ...
    opt.get_model()
        .map(|model| {
            let mut start_steps: Vec<(u64, usize)> = (0..dag.node_costs.len())
                ...
                .collect();
            let end_steps: Vec<(u64, usize)> = (0..dag.node_costs.len())
                .map(|i| {
                    (
                        model.eval(&end_times[i], true).unwrap().as_u64().unwrap(),
                        i,
                    )
                })
                .collect();
            if verbose >= 3 {
                println!(
                    "start steps {:?} end steps {:?}",
                    start_steps.iter().map(|x| x.1).collect::<Vec<usize>>(),
                    end_steps.iter().map(|x| x.1).collect::<Vec<usize>>()
                );
            }
            start_steps.sort();
