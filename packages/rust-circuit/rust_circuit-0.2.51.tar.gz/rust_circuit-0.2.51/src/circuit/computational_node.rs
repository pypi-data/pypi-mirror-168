...
use crate::sv;
use crate::{
    all_imports::TorchDeviceDtypeOp,
    new_rc_unwrap,
    py_types::{PY_EINSUM, PY_MAKE_DIAGONAL},
    pyo3_prelude::*,
};
...
use crate::{
    ...
    py_types::{
        ExtraPySelfOps, PyEinsumAxes, Tensor, PY_CIRC_COMPILER_UTIL, PY_EINOPS_REPEAT,
        PY_SCALAR_TO_TENSOR, PY_TORCH,
    },
    ...
    fn eval_tensors(
        ...
        Python::with_gil(|py| {
            if self.args.is_empty() {
                return Ok(PY_SCALAR_TO_TENSOR
                    .call(py, (1.0, Vec::<usize>::new(), device_dtype.clone()), None)
                    .unwrap()
                    .extract::<Tensor>(py)
                    .unwrap());
            }
            let out_axes_deduped: EinsumAxes = self.out_axes.iter().unique().cloned().collect();
            let result_non_diag: Tensor =
                PY_EINSUM
                    .call(
                        py,
                        (
                            zip(tensors.iter().cloned(), self.input_axes().cloned())
                                .collect::<Vec<(Tensor, EinsumAxes)>>(),
            ...
            if out_axes_deduped.len() != self.out_axes.len() {
                Ok(PY_MAKE_DIAGONAL
                    .call(
                        py,
                        (result_non_diag, out_axes_deduped, self.out_axes.clone()),
                        None,
                    )?
                    .extract(py)?)
            } else {
...
impl Einsum {
    ...
    pub fn scalar_mul(node: &CircuitRc, scalar: f64, name: Option<String>) -> Self {
        let name_definite = name.unwrap_or_else(|| "scalar_mul".to_owned());
    ...
    fn eval_tensors(
        ...
        if tensors.is_empty() {
            return Python::with_gil(|py| {
                Ok::<crate::py_types::Tensor, super::TensorEvalError>(
                    PY_SCALAR_TO_TENSOR
                        .call(py, (0.0, Vec::<usize>::new(), device_dtype.clone()), None)
                        .unwrap()
                        .extract::<Tensor>(py)
                        .unwrap(),
                )
    ...
    fn eval_tensors(
        ...
        Python::with_gil(|py| {
            PY_EINOPS_REPEAT
                .call(
                    py,
                    (tensors[0].clone(), self.spec.to_einops_string()),
                    Some(
                        HashMap::<String, u64>::from_iter(self.spec.letter_sizes())
                            .into_py_dict(py),
                    ),
                )
...
impl Index {
    ...
    pub fn try_new(
        ...
        for (idx, l) in zip(&index.0, &node.info().shape) {
            let l_i64 = *l as i64;
            match idx {
                TensorAxisIndex::Single(i) => {
                    if *i < -l_i64 || *i >= l_i64 {
                        return Err(CircuitConstructionError::IndexOutOfBounds {
                            i: idx.clone(),
                            index: index.clone(),
                            shape: node.info().shape.clone(),
                        });
                ...
                TensorAxisIndex::Slice(slice) => {
                    let start = slice.start.map_or(0, |x| if x < 0 { x + l_i64 } else { x });
                    let stop = slice
                        .stop
                        .map_or(l_i64, |x| if x < 0 { x + l_i64 } else { x });

                    if start < 0 || start > l_i64 || stop < 0 || stop > l_i64 {
                        return Err(CircuitConstructionError::IndexOutOfBounds {
                            i: idx.clone(),
                            index: index.clone(),
                            shape: node.info().shape.clone(),
                    ...
                    if start > stop {
                        return Err(CircuitConstructionError::SliceDisordered { s: slice.clone() });
                    }
