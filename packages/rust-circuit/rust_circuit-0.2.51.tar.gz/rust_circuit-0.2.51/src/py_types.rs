...
lazy_static! {
    static ref PYTHON_UTILS: Py<PyModule> = Python::with_gil(|py| {
        PyModule::from_code(
            ...
            "rust_circuit.rust_circuit_type_utils",
        )
...
lazy_static! {
    static ref PYCIRCUIT_MODULE: Py<PyModule> = Python::with_gil(|py| {
        PyModule::from_code(
            py,
            include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/py_circuit_interop.py")),
            concat!(env!("CARGO_MANIFEST_DIR"), "/py_circuit_interop.py"),
            "rust_circuit.py_circuit_interop",
        )
        .unwrap() // TODO: fail more gracefully?
        .into()
    });
