...
use crate::{
    ...
    filter_by_variant,
    py_types::{Tensor, CAST_TENSOR, PY_CAST_INT, PY_TORCH},
    pycall,
...
impl Slice {
    pub fn to_unsigned_loc(x: i64, l: usize) -> usize {
        if x < 0 {
            l.checked_sub((-x) as usize).unwrap()
        } else {
            assert!(x <= l as i64);
            x as usize
        }
...
impl USlice {
    ...
    pub fn shrink_base(&self, new_base: &USlice) -> USlice {
        ...
        assert!(self.start >= new_base.start && new_base.stop >= new_base.stop);
        USlice {
    ...
    pub fn new_tensor_randint_seeded(
        ...
        TensorAxisIndex::Tensor(pyo3::Python::with_gil(|py| {
            ...
            if let Some(dtype) = device_dtype.dtype {
                kwargs.insert("dtype", PY_TORCH.getattr(py, dtype).unwrap());
            }
            ...
            zip(&self.0, shape)
                .map(|(idx, &l)| match idx {
                    TensorAxisIndex::Single(i) => {
                        assert!(
                            *i < l as i64 && *i >= -(l as i64),
                            "single oob idx {} length {}",
                            i,
                            l
                        );
                        TensorAxisIndex::Single((*i + l as i64) % l as i64)
                    }
