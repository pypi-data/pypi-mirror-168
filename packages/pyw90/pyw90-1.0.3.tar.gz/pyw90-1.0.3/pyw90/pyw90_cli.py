...
def auto(args):
    ...
    if args.mode.lower()[0] == 'r':  # run
        ...
        # python_exe = os.path.join(sys.exec_prefix, 'bin/python')
        p = subprocess.Popen(['python', os.path.join(path, 'auto_w90_fit.py'), '--path', args.path],
        # p = subprocess.Popen([python_exe, os.path.join(path, 'auto_w90_fit.py'), '--path', args.path],
                               env={'PYTHONPATH': path},
        ...
        if usr_input.lower()[0] == 'y':
            ...
            config = Config(yaml_file=os.path.join(path, 'auto_w90_input.yaml'))
            jobs = Job(config)
            jobs.cancel()
        else:
            return
