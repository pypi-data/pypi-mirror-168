...
class AutoPipelinesRuns(BaseAutoPipelinesRuns):
    ...
    def list(self) -> 'DataFrame':
        ...
        runs_details = self._wml_engine._wml_client.training.get_details(limit=50, _internal=True)
        ...
        if self.auto_pipeline_optimizer_name is not None:
            values = [
                [timestamp, run['metadata'].get('id', run['metadata'].get('guid')), run['entity']['status']['state'],
                 wml_pipeline_name] for
    ...
    def get_params(self, run_id: str = None) -> dict:
        ...
        if kb_node_number is not None:
            ...
            params = {
                ...
                'retrain_on_holdout': ts_parameters.get('feature_columns', True),
            }
