...
class ContentCLIApi(object):
    ...
    def content_identities_retrieve(self, **kwargs):  # noqa: E501
        ...
        |입력변수 |설명 |필수 |기본값 |  |:-----|:-----|:-----|:-----|  |category             |CM category                                        |X     |            |    |출력변수 |설명 |타입 |  |:-----|:-----|:-----|  |cm_identity_name_list |CM 리스트 |List of string | - 존재하지 않는 카테고리를 조회시 빈 리스트를 반환 - 카테고리 설정 하지 않으면 조회 가능한 cm list 모두를 반환  # noqa: E501
    ...
    def content_model_retrieve(self, **kwargs):  # noqa: E501
        ...
        ## 입력변수  |입력변수 |설명 |필수 |기본값 |  |:-----|:-----|:-----|:-----|  |identity_name        |CM identity name                                        |O     |            |  |end                  |받을 데이터의 마지막 인덱스(날짜)                          |X     |request 시점 | |code_format          |데이터 칼럼 형식: ccid, isin, short_code 중 택1           |X     |ccid        | |tail                 |받을 데이터의 row 개수: 마지막 인덱스부터 tail 값 만큼, 1 이상 365 이하 값으로 제한됨 |X     |20 | ## 출력변수  |출력변수  |설명               |타입                 |  |:--------|:------------------|:--------------------|  |cm       |CM 데이터          |string (json-string) | |meta     |출력값 메타 정보    |dict                 |  ### mete dict 구성  - identity_name  - code_format  - tail  - start_date  - last_date  ### 'cm'으로 받은 CM 문자열 데이터프레임 변환 - pd.read_json(cm, orient='index')  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
