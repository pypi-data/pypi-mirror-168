...
class EmulsionAgent(AbstractAgent):
    ...
    def matches_prototype(self, prototype):
        ...
        prototype. Return True if this agent owns all non-None
        statevars mentioned in the prototype, with matching
        values. Return False otherwise.

        """
        for variable, value in prototype.items():
            # consider 'None' as a wildcard
            if value is None:
                continue
