...
def test_hoofdlocationset(loc_sets):
    ...
    assert isinstance(loc_sets.hoofd_loc.geo_df, pd.DataFrame)
...
def test_hoofdlocs_geom_z_nodata(loc_sets):
    ...
    assert loc_sets.hoofd_loc.geo_df.iloc[0].Z == "2.87"
    assert loc_sets.hoofd_loc.geo_df.iloc[0].geometry.z == 2.87
    assert loc_sets.hoofd_loc.geo_df.iloc[1].Z == "", "empty cells should be left empty, so '' in csv and pd.NA in df"
    assert loc_sets.hoofd_loc.geo_df.iloc[1].geometry.z == loc_sets.hoofd_loc.fews_config.Z_NODATA_VALUE
