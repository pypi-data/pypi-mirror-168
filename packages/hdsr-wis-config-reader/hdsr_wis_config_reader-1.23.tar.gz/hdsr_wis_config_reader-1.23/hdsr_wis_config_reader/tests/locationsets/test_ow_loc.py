...
def test_waterstandlocationset(loc_sets):
    ...
    assert isinstance(loc_sets.waterstand_loc.geo_df, pd.DataFrame)
    assert LocSetSharedCols.z in loc_sets.waterstand_loc.geo_df
    assert not loc_sets.waterstand_loc.geo_df[LocSetSharedCols.z].hasnans
