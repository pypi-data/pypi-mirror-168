...
def lastday(rq,wtype='0'):  #取指定日期的上一工作日
    ...
    while i < 30:
        lastd = datetime.datetime.strptime(rq, "%Y%m%d") - datetime.timedelta(days=1)
        xq = lastd.weekday()
        jg = swdb.jg1("select count(*) from tworkday where d_date='%s' and c_type='%s'" % (lastd.strftime('%Y%m%d'),wtype))
        if (xq >= 5 and jg == 1):
            break
        if (xq < 5 and jg == 0):
            break
...
def nextday(rq,wtype='0'):  #取指定日期的下一工作日
    ...
    while i < 30:
        nextd = datetime.datetime.strptime(rq, "%Y%m%d") + datetime.timedelta(days=1)
        xq = nextd.weekday()
        jg = swdb.jg1("select count(*) from tworkday where d_date='%s' and c_type='%s'" % (nextd.strftime('%Y%m%d'),wtype))
        if (xq >= 5 and jg == 1):
            break
        if (xq < 5 and jg == 0):
            break
...
def isworkday(rq,wtype='0'):    #判断是否工作日
    ...
    jg = swdb.jg1("select count(*) from tworkday where d_date='%s' and c_type='%s'" % (rq,wtype))
    if xq<5:    #周一到周五
        return jg==0
    else:
        return jg>0
