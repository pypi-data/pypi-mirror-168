            ...
            for bin_cnt, df in enumerate(data_df_lst):
                ...
                for clf in self.clf_names:
                    ...
                    if len(bouts_df) > 0:
                        ...
                        video_dict[file_name][bin_cnt][clf]['Event count'] = None
                        video_dict[file_name][bin_cnt][clf]['Total event duration (s)'] = None
                        video_dict[file_name][bin_cnt][clf]['Mean event duration (s)'] = None
                        video_dict[file_name][bin_cnt][clf]['Median event duration (s)'] = None
                    if len(bouts_df) > 1:
...
# test = TimeBinsClf(config_path='/Users/simon/Desktop/troubleshooting/train_model_project/project_folder/project_config.ini', bin_length=15)
