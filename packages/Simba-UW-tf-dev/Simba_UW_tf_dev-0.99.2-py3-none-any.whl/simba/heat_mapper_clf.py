...
class HeatMapperClf(object):
    ...
    '''
    Class for creating heatmaps representing the locations of the classified behavio
    Parameters
    ----------
    config_path: str
    ...
    @staticmethod
    @jit(nopython=True)
    def calculate_cum_array(cum_array: np.array, current_frm: np.array):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            if (self.frame_setting) or (self.video_setting):
                for frm_cnt, cumulative_frm in enumerate(range(self.clf_array.shape[0])):
                    ...
                    cum_array = self.calculate_cum_array(cum_array=cum_array, current_frm=current_frm)
