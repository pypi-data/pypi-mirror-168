...
- Added a new method for opening DataRobot in a browser via :meth:`datarobot.rest.RESTClientObject.open_in_browser`.  It can be invoked via `dr.Client().open_in_browser()`.
...
- Added new convenience methods to :class:`datarobot.models.Project` for dealing with Partition classes. Both should be called before :meth:`Project.analyze_and_model <datarobot.models.Project.analyze_and_model>`.
...
- Added experimental deployment improvements:
  - :class:`RetrainingPolicyRun <datarobot._experimental.models.retraining.RetrainingPolicyRun>` can be used to manage retraining policies run for a retraining policy associated with a deployment.
...
- Added a new Autopilot method to :class:`datarobot.models.Project` :meth:`Project.analyze_and_model <datarobot.models.Project.analyze_and_model>` which allows you to initiate Autopilot or data analysis against data that has been uploaded to DataRobot.
...
Bugfixes
********
- :meth:`Dataset.list <datarobot.models.Dataset.list>` will no longer throw errors when listing datasets with no owner.
...
- The method :meth:`Dataset.create_from_data_source<datarobot.models.Dataset.create_from_data_source>` will now raise ``InvalidUsageError`` if ``username`` and ``password`` are not passed as a pair together.
...
- ``Project.set_target`` has been removed. Please use :meth:`Project.analyze_and_model <datarobot.models.Project.analyze_and_model>` instead.
