...
class RooRealVar:
    ...
    def parse(self, obj:Union[str, Dict, "ROOT.RooRealVar"]):
        if isinstance(obj, str):
            ...
            self.value_range = obj.get("value_range", None)
    ...
    def new(self):
        ...
        elif (self.value is None) and (self.value_range is not None):
            variable = ROOT.RooRealVar(self.name, title,
                                       self.value_range[0], self.value_range[1])
        elif (self.value is not None) and (self.value_range is None):
            variable = ROOT.RooRealVar(self.name, title, self.value)
        else:
