...
-  ``--scan-target-id UUID``: Only list alerts from the specifiedscan
   targets.
-  ``--states [OPEN|ACTIVE|IN_PROGRESS|RISK_ACCEPTED|CLOSED|MITIGATING_CONTROL|FALSE_POSITIVE]``:
...
-  ``--scan-target-id UUID``: Only list alerts from the specifiedscan
   targets.
-  ``--state [OPEN|ACTIVE|IN_PROGRESS|RISK_ACCEPTED|CLOSED|MITIGATING_CONTROL|FALSE_POSITIVE]``:
   Only list alerts in the specified states. [default: OPEN, ACTIVE,
   IN_PROGRESS, RISK_ACCEPTED, MITIGATING_CONTROL, FALSE_POSITIVE]
-  ``--severity [CRITICAL|HIGH|MEDIUM|LOW|INFO]``: Only list alerts with
   the specifiedseverities [default: CRITICAL, HIGH, MEDIUM, LOW, INFO]
