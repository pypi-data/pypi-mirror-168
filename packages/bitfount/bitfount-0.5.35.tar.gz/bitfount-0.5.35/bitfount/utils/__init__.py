...
__all__: List[str] = ["one_hot_encode_list", "seed_all", "ExampleSegmentationData"]
...
class ExampleSegmentationData:
    ...
    This class is used mainly in Tutorial 11.
    """
    ...
    def _plot_img_array(self, img_array: np.ndarray, ncol: int = 3) -> None:
        """Helper function for tutorial 11."""
        nrow = len(img_array) // ncol

        f, plots = plt.subplots(
            nrow, ncol, sharex="all", sharey="all", figsize=(ncol * 4, nrow * 4)  # type: ignore[arg-type] # Reason: see below # noqa: B950
        )
        # From matplotlib documentation:
        # sharex, sharey: bool or {'none', 'all', 'row', 'col'}, default: False
        for i in range(len(img_array)):
            plots[i // ncol, i % ncol]  # type: ignore[index] # reason: see below # noqa: B950
            plots[i // ncol, i % ncol].imshow(img_array[i])  # type: ignore[index] # reason: see below # noqa: B950
        # matplotlib allows indexing of `Axes` objects,
        # not sure why mypy raises this error

    def plot_side_by_side(self, img_arrays: np.ndarray) -> None:
        """Helper function for tutorial 11.

        Plots side by side images and masks.

        Args:
            img_arrays: The array of images and masks pairs to plot.
        """
        flatten_list = reduce(lambda x, y: x + y, zip(*img_arrays))

        self._plot_img_array(np.array(flatten_list), ncol=len(img_arrays))
...
def delegates(
    ...
    def _f(f: Any) -> Any:
        ...
        fdoc = docstring_parser.parse(f.__doc__, style=DOCSTRING_STYLE)
        ...
        arg_names: List[str] = []
        # get all the metadata from the decorated class' docstring
        for meta in fdoc.meta:
            meta_type = type(meta)
        ...
        for (meta_type, meta) in metas.items():
            ...
            arg_names += [arg.args[1] for arg in meta]  # type: ignore[attr-defined] # Reason: see top of function # noqa: B950
        if "kwargs" in sigd:
            k = sigd.pop("kwargs")
        else:
            k = None
        ...
        ancestors = [
            item
            for item in f.__mro__
            if item.__module__.startswith("bitfount") and item != f
        ]
        ...
        for item in ancestors:
            ...
            # show up in the docs.
            to_f_dict = item.__dict__
            # get the abstract methods
            abstract_methods = (
                item.__abstractmethods__ if hasattr(item, "__abstractmethods__") else []
            )

            # Get the public non-abstract methods
            new_dict_items = {
                k: v
                for k, v in to_f_dict.items()
                if not k.startswith("_")
                and k not in f.__dict__
                and k not in abstract_methods
            }
            ...
            for k, v in new_dict_items.items():
                ...
                if (
                    item.__module__.startswith("bitfount")
                    and not str(k).startswith("_")
                    and k not in f.__dict__
                    and callable(to_f_dict[k])
                ):
            ...
            item_doc = docstring_parser.parse(item.__doc__, style=DOCSTRING_STYLE)
            # "metas" keeps track of all args, attributes and errors for one class only.
            metas = {}
            for meta in item_doc.meta:
                ...
                metas.setdefault(meta_type, []).append(meta)
            for (meta_type, meta) in metas.items():
                to_add = [arg for arg in meta if arg.args[1] not in arg_names]  # type: ignore[attr-defined] # Reason: see top of function  # noqa: B950
                arg_names += [arg.args[1] for arg in meta]  # type: ignore[attr-defined] # Reason: see top of function  # noqa: B950
                if meta_type in combined.keys():
                    combined[meta_type] += to_add
                else:
                    combined[meta_type] = to_add
            ...
            for k in combined.keys():
                combined[k] = [
                    item
                    for item in sorted(combined[k], key=lambda d: d.args[1])  # type: ignore[no-any-return] # Reason: see below # noqa: B950
                ]
                # Mypy raises error for the sorted function due to
                # the definition of sorted having changed,
                # see here: https://github.com/python/mypy/issues/9656

            # Now get the signature.
            to_f = item.__init__
            # extract the "self" param from the signature
            self_param = sigd.pop("self")
        ...
        from_f.__signature__ = sig.replace(parameters=sigd.values())  # type: ignore[arg-type] # Reason: see below # noqa: B950
        ...
        fdoc.meta = list(chain(*combined.values()))
        f.__doc__ = docstring_parser.compose(fdoc, style=DOCSTRING_STYLE)
