...
from ..io import PetrelBackend, AliyunOSSBackend, HardDiskBackend, FileIO
...
class DetectionParse:
    ...
    def __init__(self, data_info_file, ann_file, separate_flag, dataset_name=None, config=None):
        if FileIO.is_ceph_path(data_info_file):
            self.file_client = PetrelBackend()
        elif FileIO.is_oss_path(data_info_file):
            assert config, "you must provide the configuration of aliyun oss."
            bucket_name = FileIO.get_bucket_name(data_info_file)
            data_info_file = FileIO.get_oss_rel_path(data_info_file)
            ann_file = FileIO.get_oss_rel_path(ann_file)
            self.file_client = AliyunOSSBackend(bucket_name=bucket_name, **config)
