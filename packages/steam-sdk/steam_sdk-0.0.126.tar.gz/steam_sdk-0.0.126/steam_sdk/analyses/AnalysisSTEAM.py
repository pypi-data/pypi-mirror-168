...
class AnalysisSTEAM:
    ...
    def __init__(self, file_name_analysis: str,
    ...
    def step_make_model(self, step, verbose: bool = False):
        ...
        if step.simulation_number:
            if 'FiQuS' in step.software:
                self.setup_sim_FiQuS(simulation_name=step.simulation_name, simulation_type=step.type, sim_number=step.simulation_number, magnet_name=BM.model_data.GeneralParameters.magnet_name, magnet_type=BM.model_data.GeneralParameters.magnet_type)
    ...
    def step_modify_model(self, step, verbose: bool = False):
        ...
        for v, value in enumerate(step.variable_value):
            ...
            if len_simulation_numbers > 0:
                ...
                if 'FiQuS' in step.software:
                    BM.buildFiQuS()
                    #self.setup_sim_FiQuS(magnet_type=BM.model_data.GeneralParameters.magnet_type, simulation_name=step.simulation_name, model_name=step.model_name, simulation_type=step.type, sim_number=simulation_number)
                    self.setup_sim_FiQuS(simulation_name=step.simulation_name, simulation_type=step.type, sim_number=simulation_number, magnet_name=BM.model_data.GeneralParameters.magnet_name, magnet_type=BM.model_data.GeneralParameters.magnet_type)
                if 'LEDET' in step.software:
    ...
    def setup_sim_FiQuS(self, simulation_name, simulation_type, sim_number, magnet_name=None, magnet_type=None):
        """
        ...
        local_model_folder = os.path.join(self.settings.local_FiQuS_folder, simulation_name + '_' + str(sim_number))
        make_folder_if_not_existing(local_model_folder)

        # Copy simulation file
        file_name_temp = os.path.join(self.output_path, magnet_name)
    ...
    def run_sim(self, software: str, simulation_name: str, sim_number: int, simFileType: str = None, verbose: bool = False):
        ...
        if software == 'FiQuS':
            ...
            dFiQuS.run_FiQuS(sim_file_name=simulation_name, output_directory=local_analysis_folder)
        elif software == 'LEDET':
