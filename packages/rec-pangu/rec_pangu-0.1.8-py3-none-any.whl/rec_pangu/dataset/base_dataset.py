...
class BaseDataset(Dataset):
    ...
    def enc_data(self):
        ...
        for col in self.dense_cols:
            self.enc_data[col] = np.array(self.enc_dense_data(col))
        for col in self.sparse_cols:
            self.enc_data[col] = np.array(self.enc_sparse_data(col))

    def __getitem__(self, index):
        data = defaultdict(np.array)
        for col in self.dense_cols:
            data[col] = torch.Tensor([self.enc_data[col][index]]).squeeze(-1)
        for col in self.sparse_cols:
            data[col] = torch.Tensor([self.enc_data[col][index]])
