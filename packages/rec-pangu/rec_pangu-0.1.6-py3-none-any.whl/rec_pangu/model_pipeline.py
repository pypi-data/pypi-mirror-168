    ...
    if num_task == 1:
        ...
        for idx,data in enumerate(train_loader):
            ...
            if idx % 20 ==0 and device.type != 'cpu':
                logger.info(f'Iter {idx} Loss:{round(float(loss.detach().cpu().numpy()), 4)} AUC:{auc} GPU Mem:{get_gpu_usage(device)}')
            else:
                logger.info(f'Iter {idx} Loss:{round(float(loss.detach().cpu().numpy()), 4)} AUC:{auc}')
        res_dict = dict()
        ...
        for idx,data in enumerate(train_loader):
            ...
            if idx % 20 ==0 and device.type != 'cpu':
                logger.info(f'Iter {idx} Loss:{round(float(loss.detach().cpu().numpy()), 4)} GPU Mem:{get_gpu_usage(device)}')
            else:
                logger.info(f'Iter {idx} Loss:{round(float(loss.detach().cpu().numpy()), 4)}')
