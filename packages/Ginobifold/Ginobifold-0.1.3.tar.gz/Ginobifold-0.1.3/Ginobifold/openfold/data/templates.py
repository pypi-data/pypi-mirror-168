...
from Ginobifold.openfold.data import parsers, mmcif_parsing
from Ginobifold.openfold.data.errors import Error
from Ginobifold.openfold.data.tools import kalign
from Ginobifold.openfold.data.tools.utils import to_date
from Ginobifold.openfold.np import residue_constants
...
def _parse_obsolete(obsolete_file_path: str) -> Mapping[str, str]:
    """Parses the params file from PDB that lists which PDB ids are obsolete."""
...
def _realign_pdb_template_to_query(
    ...
    Args:
        ...
        mmcif_object: A mmcif_object which holds the actual template params.
...
def _extract_template_features(
    ...
    try:
        ...
        raise NoAtomDataInTemplateError(
            "Could not get atom params (%s_%s): %s" % (pdb_id, chain_id, str(ex))
        ) from ex
...
def _process_single_hit(
    ...
    try:
        ...
        # These 3 errors indicate missing mmCIF experimental params rather than a
