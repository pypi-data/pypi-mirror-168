...
from Ginobifold.openfold.data.errors import MultipleChainsError
import Ginobifold.openfold.np.residue_constants as residue_constants
...
def mmcif_loop_to_list(
    ...
    """Extracts loop associated with a prefix from mmCIF params as a list.

    Reference for loop_ in mmCIF:
      http://mmcif.wwpdb.org/docs/tutorials/mechanics/pdbx-mmcif-syntax.html

    Args:
      prefix: Prefix shared by each of the params items in the loop.
        e.g. '_entity_poly_seq.', where the params items are _entity_poly_seq.num,
        _entity_poly_seq.mon_id. Should include the trailing period.
      parsed_info: A dict of parsed mmCIF params, e.g. _mmcif_dict from a Biopython
        parser.
...
def mmcif_loop_to_dict(
    ...
    """Extracts loop associated with a prefix from mmCIF params as a dictionary.

    Args:
      prefix: Prefix shared by each of the params items in the loop.
        e.g. '_entity_poly_seq.', where the params items are _entity_poly_seq.num,
        _entity_poly_seq.mon_id. Should include the trailing period.
      index: Which item of loop params should serve as the key.
      parsed_info: A dict of parsed mmCIF params, e.g. _mmcif_dict from a Biopython
        parser.
...
def _get_atom_site_list(parsed_info: MmCIFDict) -> Sequence[AtomSite]:
    """Returns list of atom sites; contains params not present in the structure."""
    return [
        AtomSite(*site)
...
def _is_set(data: str) -> bool:
    """Returns False if params is a special mmCIF character indicating 'unset'."""
    return data not in (".", "?")
