...
LastEditors: Hexu
LastEditTime: 2022-09-20 18:13:05
FilePath: /pycronjob/pycronjob/pycronjob.py
Description: crontab
'''
import datetime
import types
...
class Crontab:
    def __init__(self, joblist, asynchronous=False, timezone='Asia/Shanghai') -> None:
        self.jobs = joblist
        self.asynchronous = asynchronous
        if self.asynchronous:
            self.sched = BackgroundScheduler(timezone=timezone)
        else:
            self.sched = BlockingScheduler(timezone=timezone)
        self._check()
        self._add_jobs()
