...
class signal_daq():
    ...
    def add_monitoring_singals(self, signal_list):
        ...
        if len(this_add_list) > 0:
            rc, daq_handle = self.client.create_daq(self.env, len(this_add_list), 0)
            if rc.value != 0:
            ...
            for i in range(len(this_add_list)):
                ...
                set_port_result = self.client.daq_set_port(self.env, daq_handle, i, this_add_list[i][0],
                                                   this_add_list[i][1], mrt_port_type, this_add_list[i][3])
                rc = rc + set_port_result.value

            if rc != 0:
                raise Exception("add signal to daq fail")
            self.client.daq_set_trigger_period(self.env, daq_handle, self.daq_period, self.daq_offsiet)

            rc = self.client.start_daq(self.env, daq_handle)
            if rc.value != 0:
                raise Exception("Start daq fail")
            self.daqs[daq_handle] = this_add_list
            for s_added in this_add_list:
                self.full_signals[s_added] = daq_handle
    ...
    def do_fetch(self):
        while self.keep_fetch:
            for q, v in self.daqs.items():
                msg_list = self.client.daq_read(self.env, q, 1, None)
                for msg in msg_list:
                    for item in msg:
