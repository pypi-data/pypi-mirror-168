...
class KFT :
    ...
    def createZone(self, topLeftX = 0, topLeftY = 0, botRightX = 0, botRightY = 0) :
        calcul = [[topLeftX, topLeftY], [botRightX, botRightY], False]
        if self._checkTypeZone(calcul) :
            return calcul
        else :
            self.writeLog(text="createZone(topLeftX = "+str(topLeftX)+", topLeftY = "+str(topLeftY)+", botRightX = "+str(botRightX)+", botRightY = "+str(botRightY)+")", type=self.ERROR, comment=self.ERROR_IMG_NOT_DEFINED + "")
            raise Exception("Impossible to create this zone")
        
    def createPosition(self, x=0,y=0) :
        calcul = [x,y]
        if self._checkTypePosition(calcul) :
            return [x,y]
        else :
            self.writeLog(text="createPosition(x = "+str(x)+", y = "+str(y)+")", type=self.ERROR, comment=self.ERROR_IMG_NOT_DEFINED + "")
            raise Exception("Impossible to create this position")
...
#Helps to delete positions that are within a margin of Error, avoids multiple identical clicks
    ...
    def setProcessTimeout(self, newTimeout):
        ...
        if os.path.exists(self.base_path + "/" + self.TIMEOUT_FILE_NAME)  == True :
            if self.VERBOSE_MODE == True :
                self.writeLog(text="setProcessTimeout("+str(newTimeout)+")", type=self.VERBOSE)
        else :
            if self.VERBOSE_MODE == True :
                self.writeLog(text="setProcessTimeout("+str(newTimeout)+")", type=self.VERBOSE, comment="New process timeout")


        f = open(self.base_path + "/" + self.TIMEOUT_FILE_NAME, "w")
        f.write(str(newTimeout))
        f.close()
