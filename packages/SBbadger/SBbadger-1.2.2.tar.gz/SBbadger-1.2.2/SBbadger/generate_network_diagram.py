...
def reaction_network_fig(net_path, fig_path, layout, source_nodes, sink_nodes, n_species):
    # print(net_path)
    if layout == 'default':
        layout = 'dot'
        ...
        for i, line in enumerate(lines):
            ...
            if i > 0:
                if line:
                    ...
                    for each in coef:
                        if coef[each] < 0:
                            left_side.append(each)
                        if coef[each] > 0:
                            right_side.append(each)
    ...
    for each in range(n_species):
        if each in left_side and each not in right_side:
            source_nodes.append(each)
        if each not in left_side and each in right_side:
            sink_nodes.append(each)
...
def model(verbose_exceptions=False, output_dir='models', group_name='test', overwrite=True, n_species=10,
    ...
    while i < num_existing_models + 1:
        ...
        ant_str, source_nodes, sink_nodes = buildNetworks.get_antimony_script(rl, ic_params, kinetics, rev_prob, add_enzyme, source, sink)
        anti_dir = os.path.join(output_dir, group_name, 'antimony', group_name + '_' + str(i) + '.txt')
        with open(anti_dir, 'w') as f:
            f.write(ant_str)
...
def generate_models(i, n_models, group_name, add_enzyme, n_species, n_reactions, kinetics, in_dist, out_dist,
                    ...
                    edge_ev_in, independent_sampling, source, sink, network_attempts, distribution_attempts):
            ...
            if net_plots == 'edge' and found_pydot:
                ...
                if found_pydot:
                    reaction_network_fig(os.path.join(output_dir, group_name, 'networks', group_name + '_' + str(i)
                                                      + '.csv'), os.path.join(output_dir, group_name, 'net_figs',
                                                                              group_name + '_' + str(i) + '.png'),
                                         net_layout, source_nodes, sink_nodes, n_species)
...
def models(verbose_exceptions=False, output_dir='models', group_name='test', overwrite=True, n_models=1, n_species=10, 
    ...
    args_list = [(i, n_models, group_name, add_enzyme, n_species, n_reactions, kinetics, in_dist, out_dist, output_dir,
                  ...
                  pmf_in, pmf_joint, range_out, range_in, edge_ev_out, edge_ev_in, independent_sampling, source, sink,
                  network_attempts, distribution_attempts)
...
def generate_networks(i, dists_list, directory, group_name, n_reactions, rxn_prob, mod_reg, gma_reg, sc_reg,
    ...
    if net_plots:
        ...
        if net_plots == 'edge' and found_pydot:
            ...
            graph.write_png(os.path.join(directory, group_name, 'net_figs', group_name + '_' + str(i)
                                         + '.png'), prog=net_layout)
        else:
            if found_pydot:
                reaction_network_fig(
                    os.path.join(directory, group_name, 'networks', group_name + '_' + str(i)
...
def generate_rate_laws(i, nets_list, directory, group_name, add_enzyme, kinetics, rev_prob, ic_params, source, sink):
...
def rate_laws(verbose_exceptions=False, directory='models', group_name='test', overwrite=True, kinetics=None, 
              ...
              n_cpus=cpu_count()-1, source=None, sink=None):
    """
    ...
    args_list = [(net[0], nets_list, directory, group_name, add_enzyme, kinetics, rev_prob, ic_params, source, sink)
