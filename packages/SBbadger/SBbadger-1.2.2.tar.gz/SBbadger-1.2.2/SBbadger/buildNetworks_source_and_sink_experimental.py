...
def generate_distributions(n_species, in_dist, out_dist, joint_dist, min_node_deg, in_range, out_range, joint_range,
    ...
    def single_unbounded_pmf(sdist, in_degree=False, out_degree=False):
        """Assumes starting degree of 1 and extends until cutoff found"""

        n_species2 = None
        if in_degree:
            n_species2 = n_species - n_source
        if out_degree:
            n_species2 = n_species - n_sink
        # print(n_species2)
        ...
        while True:
            ...
            distsum = sum(dist)
            dist_n = [x * n_species2 / distsum for x in dist]
    ...
    def single_bounded_pmf(sdist, drange, in_degree=False, out_degree=False):
        ...
        dist = [x * n_species2 for x in pmf0]
    ...
    if input_case == 1:
        ...
        pmf_out, range_out = single_unbounded_pmf(out_dist, in_degree=False, out_degree=True)

    if input_case == 2:

        pmf_out, range_out = single_bounded_pmf(out_dist, out_range, in_degree=False, out_degree=True)
    ...
    if input_case == 5:
        ...
        pmf_in, range_in = single_unbounded_pmf(in_dist, in_degree=True, out_degree=False)

    if input_case == 6:

        pmf_in, range_in = single_bounded_pmf(in_dist, in_range, in_degree=True, out_degree=False)
    ...
    if input_case == 9:
        ...
        print('range_in', range_in)
        print()

        edge_ev_out = find_edges_expected_value(pmf_out, range_out, n_species-n_sink)
        edge_ev_in = find_edges_expected_value(pmf_in, range_in, n_species-n_source)

        print('edge_ev_out', edge_ev_out)
        print('edge_ev_in', edge_ev_in)
        print()
...
def generate_samples(n_species, in_dist, out_dist, joint_dist, input_case, pmf_out, pmf_in, pmf_joint,
                     range_out, range_in, edge_ev_out, edge_ev_in, n_source, n_sink):

    in_samples = []
    out_samples = []
    joint_samples = []

    def sample_single_pmf(pmf0, drange, n_species2=None):

        # if n_species2 is None:
        #     n_species2 = n_species
        ...
        j = 0
        while j < n_species2:
            ind = random.choices(outind, weights=pmf0)[0]
            samplest[ind] += 1
            j += 1
    ...
    def sample_both_pmfs(pmf01, drange1, pmf02, drange2, order):
        ...
        while True:
            ...
            j = 0
            if order == 'io':
                while j < n_species-n_source:
                    ind = random.choices(ind1, pmf01)[0]
                    samples1t[ind] += 1
                    j += 1
            if order == 'oi':
                while j < n_species-n_sink:
                    ind = random.choices(ind1, pmf01)[0]
                    samples1t[ind] += 1
                    j += 1
            ...
            if order == 'io':
                while edges2 < edges1 and nodes < n_species-n_source:
                    ind = random.choices(ind2, pmf02)[0]
                    samples2t[ind] += 1
                    edges2 += drange2[ind]
                    nodes += 1
            if order == 'oi':
                while edges2 < edges1 and nodes < n_species-n_sink:
                    ind = random.choices(ind2, pmf02)[0]
                    samples2t[ind] += 1
                    edges2 += drange2[ind]
                    nodes += 1
    ...
    if input_case == 1:
        ...
        out_samples = sample_single_pmf(pmf_out, range_out, n_species-n_sink)
    ...
    if input_case == 5:
        ...
        in_samples = sample_single_pmf(pmf_in, range_in, n_species-n_source)
    ...
    if input_case == 9:
        ...
        if edge_ev_in < edge_ev_out:
            in_samples, out_samples = sample_both_pmfs(pmf_in, range_in, pmf_out, range_out, 'io')

        if edge_ev_in > edge_ev_out:
            out_samples, in_samples = sample_both_pmfs(pmf_out, range_out, pmf_in, range_in, 'oi')

        if edge_ev_in == edge_ev_out and pmf_out == pmf_in:
            temp_ind = 0
            while True:
                temp_ind += 1
                out_samples = sample_single_pmf(pmf_out, range_out, n_species-n_sink)
...
def generate_reactions(in_samples, out_samples, joint_samples, n_species, n_reactions, rxn_prob, mod_reg, gma_reg, 
    ...
    while len(out_nodes_count) < n_species:
        out_nodes_count.append(0)

    while len(in_nodes_count) < n_species:
    ...
    if not bool(joint_samples):
        while True:
            ...
            for i, each in enumerate(in_nodes_count):
                if in_nodes_count[i] == 0 and out_nodes_count[i] == 0:
                    isolated_node = True
                    break
