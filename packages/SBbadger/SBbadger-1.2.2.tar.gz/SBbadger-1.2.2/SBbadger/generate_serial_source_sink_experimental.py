...
def model(verbose_exceptions=False, output_dir='models', group_name='test', overwrite=True, n_species=10,
    ...
    if source_sink is None and joint_dist is None:
        source_sink = [0.1, 0.1]
    if source_sink is None and joint_dist is not None:
        source_sink = 0.1

    if isinstance(source_sink, list):
        ...
        if source_sink < 0 or source_sink > 1:
            if not verbose_exceptions:
                sys.tracebacklimit = 0
            raise Exception("The proportion of nodes that can act as a source or sink must be between 0 and 1.")
    ...
    while i < num_existing_models + 1:
        ...
        if isinstance(source_sink, list):
            ...
            n_source = n_sink = round(source_sink * n_species)
...
def distributions(verbose_exceptions=False, output_dir='models', group_name='test', overwrite=True, n_models=1,
                  ...
                  joint_range=None, min_freq=1.0, dist_plots=False, source_sink=None):
    """
