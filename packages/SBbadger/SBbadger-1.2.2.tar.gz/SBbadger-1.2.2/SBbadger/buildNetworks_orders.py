...
def generate_reactions(in_samples, out_samples, joint_samples, n_species, n_reactions, rxn_prob, rxn_orders, mod_reg,
                       gma_reg, sc_reg, reg, mass_violating_reactions, edge_type, mass_balanced, connected):
    ...
    if (bool(out_samples) and bool(in_samples)) or bool(joint_samples):
        ...
        while True:
            ...
            if rxn_prob and not rxn_orders:
                rt = _pick_reaction_type(rxn_prob)
            if not rxn_prob and not rxn_orders:
                rt = _pick_reaction_type()
            n_reactants = None
            n_products = None
            if not rxn_prob and rxn_orders:
                n_reactants = random.randint(rxn_orders[0], rxn_orders[1])
                n_products = random.randint(rxn_orders[0], rxn_orders[1])

            reg_num = 0
            if reg:
                reg_num = random.randint(0, reg[0])

            print(n_reactants, n_products, reg_num)
            quit()
