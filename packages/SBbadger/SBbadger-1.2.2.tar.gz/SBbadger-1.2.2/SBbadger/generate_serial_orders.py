...
import sys
from SBbadger import buildNetworks_orders
import os
import shutil
import glob
import antimony
...
def models(verbose_exceptions=False, output_dir='models', group_name='test', overwrite=True, n_models=1, n_species=10, 
           n_reactions=None, in_dist='random', out_dist='random', joint_dist=None, in_range=None, out_range=None, 
           joint_range=None, min_freq=1.0, mass_violating_reactions=True, connected=True, edge_type='generic',
           kinetics=None, add_enzyme=False, mod_reg=None, gma_reg=None, sc_reg=None, reg=None, rxn_prob=None,
           rxn_orders=None, rev_prob=0, ic_params=None, dist_plots=False, net_plots=False, net_layout='dot',
           mass_balanced=False):
    """
    ...
    :param rxn_prob: Describes the reaction probabilities. Defaults to
        [UniUni, BiUni, UniBi, BiBI] = [0.35, 0.3, 0.3, 0.05]
    :param rxn_orders: Min and max number of reactants and products; takes the form
        [min reactants, max reactants, min products, max products]
    :param rev_prob: Describes the probability that a reaction is reversible.
    ...
    if rxn_prob and rxn_orders:
        if not verbose_exceptions:
            sys.tracebacklimit = 0
        raise Exception("You have provided both reaction probabilities and reaction orders. Please review the "
                        "documentation and choose only one.")
    ...
    if rxn_orders:
        if (out_dist != 'random' and rxn_orders[0] > 1) or (in_dist != 'random' and rxn_orders[2] > 1):
            if not verbose_exceptions:
                sys.tracebacklimit = 0
            raise Exception("The minimum reactant/product reaction orders must be 1 if an out/in distribution is "
                            "provided.")
    ...
    input_case, pmf_out, pmf_in, pmf_joint, range_out, range_in, edge_ev_out, edge_ev_in = \
        buildNetworks_orders.generate_distributions(n_species, in_dist, out_dist, joint_dist, min_freq, in_range,
                                                    out_range, joint_range)
    ...
    while i < num_existing_models + n_models:
        ...
        while not rl[0]:
            ...
            in_samples, out_samples, joint_samples = \
                buildNetworks_orders.generate_samples(n_species, in_dist, out_dist, joint_dist, input_case, pmf_out,
                                                      pmf_in, pmf_joint, range_out, range_in, edge_ev_out, edge_ev_in)

            rl, el = buildNetworks_orders.generate_reactions(in_samples, out_samples, joint_samples, n_species,
                                                             n_reactions, rxn_prob, rxn_orders, mod_reg, gma_reg,
                                                             sc_reg, reg, mass_violating_reactions, edge_type,
        ...
        ant_str = buildNetworks_orders.get_antimony_script(rl, ic_params, kinetics, rev_prob, add_enzyme)
        anti_dir = os.path.join(output_dir, group_name, 'antimony', group_name + '_' + str(i) + '.txt')
        with open(anti_dir, 'w') as f:
            f.write(ant_str)
