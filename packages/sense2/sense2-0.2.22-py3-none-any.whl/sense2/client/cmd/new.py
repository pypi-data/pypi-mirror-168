...
from sense2 import Sense2File, parseRequires2Denpens
import os
...
class {package_name}Conan(Sense2File):
    ...
    generators = "cmake"
    requires = ""
    # docker_version = "<dokcer version>"
    # sense_system_deb = "<debs>"
    ...
    deb_control = {{
       ...
       "Priority": "optional",
       "Essential": "no",
       "Depends": parseRequires2Denpens(requires),
       "Homepage": "www.example.com",
    }}
    changelog = "my changelog test"
    
    ### upload cpp package to artifactory
    # user="user"
    # channel="beta"

    ## upload deb setting
    apt_user = "qiuhai"
    apt_sever = "http://10.0.50.109:8081/repository/sense2-system-deb/"
...
#         pass
    ...
    def deb(self):
        ## copy *.deb to deb folder
        package_name = self.name
        package_version = self.version
        deb_file = package_name + "_" + package_version + ".deb"
        self.copy(deb_file, dst=".", keep_path=False)

        if os.path.exists(package_name):
            self.run("rm -rf " + package_name)
        
        self.run("rm -rf *.deb")
        pass
        
    def install_deb(self):
        print("run conanfile install_deb")
        pass
    
    def install_docker(self):
        print("run conanfile install_docker")
        pass

    def install_sense_system(self):
        print("run conanfile install_sense_system")
        pass
"""
