...
class Firewall:
    ...
    def get_managed_image(self) -> str:
        ...
        with self._backend.GRPCErrorHandler():
            with self._backend.get_firewall_stub() as firewall_tester:
                res: ListFirewallsResponse = firewall_tester.ListFirewalls(req)
        managed_image_name = res.firewalls[0].managed_image.name
    ...
    def update_firewall_reference_set_source(
        self,
        rolling_window_seconds: Optional[int] = None,
        reference_set_time_bin: Optional[Tuple[datetime, datetime]] = None,
    ) -> UpdateFirewallResponse:
        """Update firewall reference set source.

        Reference set source may be a rolling window, a fixed time bin, or the default
        (stress test reference set) if both arguments are left empty.

        Args:
            rolling_window_seconds: length of rolling window in seconds
            reference_set_time_bin: start and end times of fix time bin

        Warnings:
            Updating the reference set also resets firewall thresholds and rules.
        """
        req = ListFirewallsRequest(firewall_ids=[self._firewall_id])
        with self._backend.GRPCErrorHandler():
            with self._backend.get_firewall_stub() as firewall_tester:
                res: ListFirewallsResponse = firewall_tester.ListFirewalls(req)
        stress_test_id = res.firewalls[0].stress_test_run_id
        ...
        return self._update_firewall(
            stress_test_run_id=stress_test_id, reference_set_source=reference_set_source
        )
...
def get_reference_set_source(
    ...
    Arguments:
        ...
        reference_set_time_bin: start and end times of fix time bin
    """
    if rolling_window_seconds is not None and reference_set_time_bin is not None:
        raise ValueError(
            "Only one of `rolling_window_seconds` or `reference_set_time_bin` "
            "may be set."
        )
    elif rolling_window_seconds is not None:
        return ReferenceSetSource(rolling_window_seconds=rolling_window_seconds)
