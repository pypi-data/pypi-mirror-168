...
if __name__ == '__main__':
    ...
    # photpy.config(plot=False)  # 全局关闭画图
    ...
    data_x, data_y = photpy.gen_bits(num_symbols * bits_per_symbol)  # 生成两列随机二进制序列

    # QAM调制器
    modem = photpy.Modem(bits_per_symbol)
    ...
    shaper = photpy.PulseShaper(up_sampling_factor=up_sampling_factor, len_filter=128 * up_sampling_factor,
    ...
    noise = photpy.DacNoise(sampling_rate_awg, sampling_rate, dac_resolution_bits)

    signal_x, signal_y = noise.add(signal_x, signal_y)

    """ 加入发射端激光器产生的相位噪声 """
    linewidth_tx = 150e3  # 激光器线宽
    noise = photpy.PhaseNoise(sampling_rate / total_baud, linewidth_tx, total_baud)
    ...
    noise = photpy.GaussianNoise(osnr_db, sampling_rate)

    signal_x, signal_y = noise.add(signal_x, signal_y)

    """ 添加接收端激光器产生的相位噪声 """
    linewidth_rx = 150e3  # 激光器线宽
    noise = photpy.PhaseNoise(sampling_rate / total_baud, linewidth_rx, total_baud)
    ...
    signal_x, signal_y = photpy.add_freq_offset(signal_x, signal_y, frequency_offset, sampling_rate)

    """ 模拟接收机造成的I/Q失衡，主要考虑幅度失衡和相位失衡，这里将两者都加在虚部上 """

    signal_x, signal_y = photpy.add_iq_imbalance(signal_x, signal_y)

    """ 加入ADC的量化噪声 """
    adc_sample_rate = 160e9  # ADC采样率
    adc_resolution_bits = 8  # ADC的bit位数

    signal_x, signal_y = photpy.add_adc_noise(signal_x, signal_y, sampling_rate, adc_sample_rate, adc_resolution_bits)

    """ IQ正交化补偿，就是将之前的I/Q失衡的损伤补偿回来 """

    signal_x, signal_y = photpy.iq_freq_offset_and_compensation(signal_x, signal_y, sampling_rate)

    """ 接收端相应的RRC脉冲整形，具体的参数代码与发射端的RRC滤波器是一致的 """
    signal_x, signal_y = shaper.rx_shape(signal_x, signal_y)

    """ 帧同步，寻找与发射端原始信号头部对应的符号 """
    syncer = photpy.FrameSyncer(up_sampling_factor)
    ...
    equalizer = photpy.AdaptiveEqualizer(num_tap, cma_convergence, ref_power_cma, step_size_cma, step_size_rde,
    ...
    bps = photpy.BPS(num_test_angle, block_size, bits_per_symbol)
    signal_x, signal_y = bps.restore(signal_x, signal_y)

    """ 此处开始计算误码率 """

    ber = photpy.BER(bits_per_symbol)
