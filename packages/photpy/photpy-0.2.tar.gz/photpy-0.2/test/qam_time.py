...
def main():
    ...
    # 首先产生发射端X/Y双偏振信号
    # data_x = photpy.gen_bits((num_symbols * bits_per_symbol, 1), seed=1)  # 固定随机数序列
    # data_y = photpy.gen_bits((num_symbols * bits_per_symbol, 1))
    ...
    upsampled_matrix_x = photpy.upsample(scm_matrix_x, up_sampling_factor)
    upsampled_matrix_y = photpy.upsample(scm_matrix_y, up_sampling_factor)
    ...
    # 对信号进行量化，模拟加入量化噪声
    tx_samples_x_dac = photpy.dac_resolution(tx_samples_x, dac_resolution_bits)
    tx_samples_y_dac = photpy.dac_resolution(tx_samples_y, dac_resolution_bits)
    ...
    phase_noise = photpy.linewidth_induced_noise(len(tx_samples_x_dac), sampling_rate / total_baud, linewidth_tx,
    ...
    noise_x, noise_y, noise_power = photpy.load_awgn(len(tx_samples_x_dac))
    ...
    phase_noise = photpy.linewidth_induced_noise(len(tx_signal_x), sampling_rate / total_baud, linewidth_rx, total_baud)
    ...
    # 对信号量化，添加ADC造成的量化噪声
    re_x = photpy.adc_resolution(re_x, adc_resolution_bits)
    re_y = photpy.adc_resolution(re_y, adc_resolution_bits)
    ...
    rx_xi_tem, rx_xq_tem = photpy.gram_schmidt_orthogonalize(np.real(re_x), np.imag(re_x))
    rx_yi_tem, rx_yq_tem = photpy.gram_schmidt_orthogonalize(np.real(re_y), np.imag(re_y))
    ...
    re_x, re_y, fre_offset = photpy.fre_offset_compensation_fft(re_x, re_y, sampling_rate)
    ...
    start_index_x_1 = photpy.fine_synchronize(re_x[0:10000 * up_sampling_factor:up_sampling_factor].T,
                                            tx_scm_matrix_x[0:2000].T)
    start_index_y_1 = photpy.fine_synchronize(re_y[0:10000 * up_sampling_factor:up_sampling_factor].T,
    ...
    # 对信号采用CMA-RDE进行自适应均衡
    equalization_matrix_x, equalization_matrix_y = photpy.cma_rde(input_x_i, input_x_q, input_y_i, input_y_q, num_tap,
                                                                  cma_convergence, ref_power_cma, step_size_cma,
                                                                  step_size_rde, up_sampling_factor, bits_per_symbol)

    photpy.utils.plot_scatter(equalization_matrix_x, pt_size=1)
    ...
    # 利用FFT-FOE算法对信号的频偏进行估计与补偿
    equalization_matrix_x, equalization_matrix_y, fre_offset = photpy.fre_offset_compensation_fft(equalization_matrix_x,
    ...
    # BPS算法
    equalization_matrix_x, phase_x = photpy.bps_hybrid_qam(np.real(equalization_matrix_x), np.imag(equalization_matrix_x),
                                                           num_test_angle, block_size, bits_per_symbol)
    equalization_matrix_y, phase_y = photpy.bps_hybrid_qam(np.real(equalization_matrix_y), np.imag(equalization_matrix_y),
                                                           num_test_angle, block_size, bits_per_symbol)
    ...
    photpy.utils.plot_scatter(equalization_matrix_x)
    ...
    # 对发射端信号跟均衡后信号进行同步
    start_index_x_1 = photpy.fine_synchronize(tx_scm_matrix_x[:, 0].T, equalization_matrix_x[0:10000, 0].reshape((1, -1)))
    ...
    ber, q_db = photpy.ber_count(rx_bits, tx_bits)  # 比较码元的正确率
