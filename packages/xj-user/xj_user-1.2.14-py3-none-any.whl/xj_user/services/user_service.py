...
class UserService:
    ...
    def user_list(params, need_pagination=True):
        if not need_pagination:
            ...
            res_data = []
            if res_obj:
                res_data = list(res_obj.values("id", "user_name", "full_name", "nickname", "phone", "email", "wechat_openid"))
            for i in res_data:
                id(i)
                i['is_user'] = True
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=["page", "size", "user_id", "email", "full_name", "user_name", "nickname", "id_list", "wechat_openid"],
            alias_dict={
