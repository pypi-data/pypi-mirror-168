...
def make_tarfile(
    ...
    ignorefile_path: Optional[str] = None,
) -> None:
    if ignorefile_path:
        is_file_ignored = gitignorefile.parse(
            path=ignorefile_path, base_path=source_dir
        )
    else:
        # if no ignorefile present assume that every file needs to be added
