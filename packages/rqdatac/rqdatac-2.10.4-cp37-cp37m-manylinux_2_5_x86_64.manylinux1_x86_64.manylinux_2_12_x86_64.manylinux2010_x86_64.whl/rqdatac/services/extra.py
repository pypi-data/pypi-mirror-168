...
def current_freefloat_turnover(order_book_ids):
    ...
    if len(order_book_ids) == 1:
        snapshots = [snapshots]
    total_turnover = pd.Series({t.order_book_id: t.total_turnover for t in snapshots})
    last = pd.Series({t.order_book_id: t.last for t in snapshots})
    turnover = total_turnover / (shares * last)
    return turnover.reindex(order_book_ids)
