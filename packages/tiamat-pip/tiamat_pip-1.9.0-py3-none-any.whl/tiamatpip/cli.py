...
from tiamatpip.utils import patch_pip_internal_req_install_get_default_environment
...
def redirect_to_pip(argv: List[str]) -> None:
    ...
    with debug_print(
        ...
        if cmd in ("install", "uninstall"):
            ...
            patch_pip_internal_req_install_get_default_environment()
            if cmd == "uninstall":
                configure.refresh_pkg_resources_working_set(exclude_builtin=True)
            else:
                if "--no-warn-script-location" not in argv:
