    ...
    if 2 < len(update.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                mdiskpro = MdiskPro(API)
                url = await mdiskpro.convert(f'https://t.me/{temp.Bot_Username}?start=DKBOTZ-DK-BOTZ-{file_id}')
                #resp = requests.get(f'https://api.shareus.in/shortLink?token={API}&format=json&link=https://t.me/{temp.Bot_Username}?start=DKBOTZ-DK-BOTZ-{file_id}').json()
                ...
                if settings["button"]:
                    btn.append([InlineKeyboardButton(f"{filesize} {filename}", url=url)])
                else:                    
                    btn.append([InlineKeyboardButton(f"{filesize}", url=url),
                                InlineKeyboardButton(f"{filename}", url=url)])
        else:
            return
