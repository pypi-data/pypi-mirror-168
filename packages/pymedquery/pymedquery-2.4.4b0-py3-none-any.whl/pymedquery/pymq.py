...
class PyMedQuery:
    ...
    def __set_extract_params(
            self, get_all: bool, no_affines: bool, format_params: Dict[str, Union[str, int, float, bool]],
        ...
        if get_all:
            ...
            sql_file_path: os.PathLike = config.SERIES_MASK_QUERY_DEFAULT if no_affines else config.SERIES_MASK_AFF_QUERY
    ...
    def __pg_extract(
        ...
        # other kinds of data
        for blob_uid in config.BLOB_UIDS:
            if pg_payload[blob_uid]:
                config.all_uids.extend(pg_payload[blob_uid])

        return config.all_uids, pg_payload

    def __fetch_uids(
            self, get_all: bool, no_affines: bool, format_params: dict,
            sql_file_path: str, project_id: str, limit: str,
            include_mask: bool, verbose: bool
    ) -> List[str]:
        ...
        format_params, sql_file_path = self.__set_extract_params(
            get_all=get_all, no_affines=no_affines, format_params=format_params,
            sql_file_path=sql_file_path, project_id=project_id,
            limit=limit, include_mask=include_mask
        )
        all_uids, pg_payload = self.__pg_extract(sql_file_path=sql_file_path, format_params=format_params, verbose=verbose)
        if not all_uids:
    ...
    def extract(
            self, get_all: bool, no_affines: bool = True, project_id: Optional[str] = None, limit: Optional[Union[int, str]] = 'NULL',
            include_mask: Optional[bool] = None, format_params: Optional[dict] = None, sql_file_path: Optional[str] = None,
            bucket_name: Optional[str] = 'multimodal-images', verbose: bool = False
        ...
        no_affines : bool
            Set no_affines to False if you want to include affines in the image extraction. (The default is False)
        ...
        try:
            ...
            all_uids, data_info = self.__fetch_uids(
                get_all=get_all, no_affines=no_affines, sql_file_path=sql_file_path,
            ...
            if isinstance(all_uids, list) and len(all_uids) > 1:
                ...
                all_uids = all_uids if isinstance(all_uids, str) else all_uids[0]
                images: Dict[str, List[np.ndarray]] = self.mh.get_blobs(bucket_name=bucket_name, blob_name=all_uids)
            # merge the two dicts
            final_dict = images | data_info
    ...
    def batch_extract(
            self, get_all: bool, no_affines: bool = True, sql_file_path: Optional[str] = None,
            ...
            bucket_name: Optional[str] = 'multimodal-images', verbose: bool = True
