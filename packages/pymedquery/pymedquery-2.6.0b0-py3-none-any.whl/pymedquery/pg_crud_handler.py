...
class CRUD:
    ...
    def commit(self, reset_connection: bool = True, verbose: bool = False) -> NoReturn:
    ...
    def read(
        ...
        with self.__db() as (conn, cur):
            try:
                if is_file_path:
                    ...
                    if verbose:
                        ...
                        self.log.info(f"""
                                ...
                                """)
                    if format_params:
                        cur.execute(sql.format(**format_params))
                    else:
                        cur.execute(sql)
                    self.log.info('executed a read query')
                if cur:
                ...
                if is_file_path:
                    sys.exit(f'Exiting program for SQL script: {basename(sql)}')
                else:
                    sys.exit(f'Exiting program for SQL query: {sql}')
