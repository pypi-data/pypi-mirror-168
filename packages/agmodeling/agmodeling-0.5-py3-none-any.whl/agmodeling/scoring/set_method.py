...
def get_IPI_score(ref,candidate):
    ...
    header = " %-10s: %-10s: %-10s: %-10s: %-10s: %-10s :: %-10s" \
           % ("Match",'RMSE','Pearson','Kendall', \
              'Spearman','LFE','IPI')
    values = " %-10.6f: %-10.6f: %-10.6f: %-10.6f: %-10.6f: %-10.6f :: %-10.6f" \
...
def compute_score_unique(work, d) :
    ...
    tmp=pd.DataFrame()
    tmp['BINS_CANDIDATE']=pd.cut(work['CANDIDATE'],binsCandidate, labels =[x for x in range(0,d)])
