...
async def handle_dist(
...
) -> Optional[Union[PipPackageInfo, CondaPlaceHolder]]:
    ...
    if installer:
        ...
        if installer == "pip":
            ...
            priority = 100 if strict else priorities.get(name.lower(), 50)
            if priority == -2:
                return {
                    "name": name,
                    "client_version": dist.version,
            ...
            return CondaPlaceHolder(name=name)
        else:
            return
    else:
        return
...
async def create_pip_env_approximation(
    ...
    only: Optional[Set[str]] = None,
    strict: bool = False,
) -> typing.Dict[str, Union[PipPackageInfo, CondaPlaceHolder]]:
    async with aiohttp.ClientSession("https://pypi.org") as client:
        ...
        # packages that are LAST take priority over packages first in the list
        return {
            pkg["name"]: pkg
...
async def create_environment_approximation(
    ...
    return list(pip_env.values()), list(filterd_conda.values())
