...
def login(
    ...
    logging.debug(auth)
    valid = is_valid(host=host, app=app) if host else is_valid(app=app)
    if valid is False or auth["password"]:
        try:
...
def is_licensed(app: Application) -> bool:
    ...
    try:
        ...
        pass  # Need to account for versions of the API with no license endpoint
    return status
