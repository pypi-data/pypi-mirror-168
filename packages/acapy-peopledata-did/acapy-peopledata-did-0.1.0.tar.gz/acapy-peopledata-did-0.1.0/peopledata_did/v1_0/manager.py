...
from peopledata_did.v1_0.messages.problem_report import (
    peopledataDIDProblemReportMessage,
    peopledataDIDProblemReportMessageReason,
)
from peopledata_did.v1_0.messages.read_did import ReadDIDMessage, ReadDIDMessageBody
from peopledata_did.v1_0.messages.read_did_response import (
    ReadDIDResponseMessage,
    ReadDIDResponseMessageSchema,
)
from peopledata_did.v1_0.models.diddoc_model import peopledataDIDDoc, peopledataDIDResponseBody
from peopledata_did.v1_0.utils.did.peopledata_did import DIDpeopledata
...
class ADAManager:
    ...
    # Record for keeping track of DIDs that are registered in the DID registry (peopledata DID registry)
    RECORD_TYPE_peopledata_DID_REGISTRY_DID_INFO = "peopledata_did_registry_did_info"
    ...
    async def process_read_did_message(
        ...
        create_did_message_from_did: DIDpeopledata = DIDpeopledata.from_public_key_b58(
            receipt.sender_verkey, key_type=KeyType.ED25519
        )
        create_did_message_to_did: DIDpeopledata = DIDpeopledata.from_public_key_b58(
        ...
        peopledata_did_registry_did_info_record = None
        try:

            # Fetch DID from wallet
            peopledata_did_registry_did_info_record = await storage.search_records(
                type_filter=ADAManager.RECORD_TYPE_peopledata_DID_REGISTRY_DID_INFO,
            ...
            peopledata_did_problem_report = peopledataDIDProblemReportMessage(
                problem_code=peopledataDIDProblemReportMessageReason.DID_NOT_FOUND.value,
            ...
            # Assign thread id
            peopledata_did_problem_report.assign_thread_id(thid=read_did_message._id)

            if responder:
                await responder.send_reply(peopledata_did_problem_report)
        ...
        read_did_response_message = ReadDIDResponseMessage(
            ...
            to_did=response_message_to_did.did,
            created_time=round(time.time() * 1000),
            body=peopledataDIDResponseBody(
                did_doc=peopledataDIDDoc.from_json(
                    peopledata_did_registry_did_info_record.value
                ),
                version=peopledata_did_registry_did_info_record.tags.get("version"),
                status=peopledata_did_registry_did_info_record.tags.get("status"),
            ),
        )
    ...
    async def resolve_remote_peopledata_did(
        self, *, peopledata_did: str
    ) -> peopledataDIDResponseBody:
        """Resolve remote peopledata DID"""

        # Initialize DID peopledata
        peopledata_did = DIDpeopledata.from_did(peopledata_did)
        ...
        # Fetch connection record marked as peopledata DID registry
        (
            connection_record,
            err,
        ) = await self.fetch_peopledata_did_registry_connection_record()
        if err:
            raise ADAManagerError(
                "Failed to fetch peopledata DID registry connection record"
            )

        # Construct read-did message
        # from_did
        pairwise_local_did_record = await wallet.get_local_did(connection_record.my_did)
        from_did = DIDpeopledata.from_public_key_b58(
            pairwise_local_did_record.verkey, key_type=KeyType.ED25519
        )

        # to_did
        to_did = DIDpeopledata.from_public_key_b58(
        ...
        read_did_message = ReadDIDMessage(
            ...
            to_did=to_did.did,
            created_time=round(time.time() * 1000),
            body=ReadDIDMessageBody(did=peopledata_did.did),
        )
            ...
            async with session.post(
                ...
                if "read-did-response" in message_json["@type"]:
                    ...
                    if read_did_response_message.body.status == "revoked":
                        raise ADAManagerError(f"peopledata DID {peopledata_did.did} is revoked")
