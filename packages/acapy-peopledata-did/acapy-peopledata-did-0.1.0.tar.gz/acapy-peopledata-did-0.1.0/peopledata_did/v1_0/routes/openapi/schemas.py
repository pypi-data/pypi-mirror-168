...
from peopledata_did.v1_0.models.diddoc_model import peopledataDIDDocSchema
from peopledata_did.v1_0.models.exchange_records.data_agreement_didcomm_transaction_record import (
    DataAgreementCRUDDIDCommTransaction,
)
from peopledata_did.v1_0.models.exchange_records.data_agreement_record import (
...
class SendReadDIDMessageMatchInfoSchema(OpenAPISchema):
    """
    Send a read-did message to the peopledata DID registry service.
    """

    did = fields.Str(description="did:peopledata identifier", required=True, **peopledata_DID)


class peopledataDIDRemoteRecordsQueryStringSchema(OpenAPISchema):
    """
    Query string schema for listing peopledata DID remote records.
    """
    ...
    # DID
    did = fields.Str(**peopledata_DID, description="peopledata decentralised identifier")

    # Status
    status = fields.Str(
        description="peopledata DID remote status",
        required=False,
        validate=validate.OneOf(["active", "revoked"]),
    )


class peopledataDIDRemoteRecordResponseSchema(OpenAPISchema):
    """
    Response schema for peopledata DID remote record.
    """

    did_doc = fields.Nested(
        peopledataDIDDocSchema,
        description="peopledata DID document",
    )
