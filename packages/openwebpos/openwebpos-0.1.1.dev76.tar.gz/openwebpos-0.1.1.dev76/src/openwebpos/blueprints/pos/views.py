...
from openwebpos.blueprints.admin.models import Store, Printer
...
def kitchen_printer():
    ...
    return printer.Network(_kitchen_printer.ip)
...
def customer_receipt(order_number):
    ...
    shop_address = f'{_store.address}\n{_store.city}, {_store.state} {_store.zip}'
    shop_address = shop_address.splitlines()
    shop_phone = f'{_store.phone}'
    if shop_phone == 'None':
        shop_phone = ''
...
# @pos.get('/print/<string:printer>/<string:order_number>')
# def print_to(printer, order_number):
#     _printer = Printer.query.filter_by(name=printer).first_or_404()
