...
from openwebpos.blueprints.pos.models.order import OrderPager, OrderType, \
    TransactionType, Order, Transaction
from .models import Store, Printer
from .forms import MenuForm, MenuItemForm, PagerForm, IngredientForm, \
    PrinterForm, StoreForm
...
def add_recipe(menu_id, ingredient_id):
    ...
    _recipe.save()
    return redirect(url_for('admin.admin_menu.menu_items', menu_name=_menu.name))
...
def printers():
    """
    Renders the admin printers page.
    """
    _printers = Printer.query.all()
    form = PrinterForm()
    return render_template('admin/printers.html', title='Admin - Printers',
                           printers=_printers, form=form)
...
def add_printer():
    ...
    form = PrinterForm()
    if form.validate_on_submit():
        p = Printer(name=form.name.data, ip=form.ip.data, port=form.port.data,
...
def store():
    """
    Renders the admin store page.
    """
    _store = Store.query.first()
    form = StoreForm(obj=_store)
    return render_template('admin/store.html', title='Admin - Store',
                           form=form)
...
def update_store():
    ...
    form = StoreForm(obj=_store)
    if form.validate_on_submit():
        _store.name = form.name.data
        _store.address = form.address.data
        _store.phone = form.phone.data
        _store.save()
