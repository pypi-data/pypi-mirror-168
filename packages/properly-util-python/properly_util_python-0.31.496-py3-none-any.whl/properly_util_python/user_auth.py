...
class UserAuthHelper:
    ...
    def get_properly_id_from_user_verified_email(self, email: str):
        ...
        items = response.get("Items")
        if len(items) > 1:
            self.logger.error("DuplicateVerifiedEmailError",
                              "More than one user associated with verified email: {}".format(email))
    ...
    def get_properly_id_from_user_auth_email(self, auth_email: str):
        ...
        items = response.get("Items")
        if len(items) > 1:
            self.logger.error("DuplicateUnverifiedEmailError",
