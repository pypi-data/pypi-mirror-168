...
from spider_bot.models.base import BaseDB
from spider_bot.models.posts import Post
from spider_bot.models.posturls import PostURL
from spider_bot.models.users import User
...
class DBAPI(BaseDB):
    ...
    def get_users_todo(self, working_status: Optional[bool] = None):
        ...
        for i in users_urls:
            if i:
                logger.debug("get_users_todo user url: %s", i[0])
                yield i[0]
    ...
    def get_users_to_get_profiles(self):
        ...
        for i in users_urls:
            if i:
                logger.debug("get_users_to_get_profiles user url: %s", i[0])
                yield i[0]
    ...
    def update_user_profile(self, user_url: str, name: str, avatar: bytes):
        ...
        self.commit()
        logger.debug("update_user_profile %s", user_url)
        return True
    ...
    def update_user_working_status(self, user_url: str, working_status: Optional[bool] = None):
        ...
        self.commit()
        logger.debug("update_user_working_status %s", user_url)
        return True
    ...
    def add_post_url(self, user_url: str, post_url: str):
        ...
        logger.debug("add_post_url %s", post_url)
        return True
    ...
    def update_url_content_status(self, post_url: str, content_status: bool):
        ...
        self.commit()
        logger.debug("update_url_content_status %s", post_url)
        return True
    ...
    def get_posturls_to_getcontent(self):
        ...
        for i in urls:
            if i:
                logger.debug("get_posturls_to_getcontent %s", i[0])
                yield i[0]
    ...
    def add_post(self, post_url: str, post_time: str, text: str, screenshot: bytes):
        ...
        logger.debug("add_post %s %s", post_url, post_time)
    ...
    def get_post(self, post_url: str):
        ...
        post = self.session.query(Post).filter(Post.post_url == post_url).first()
        logger.debug("get_post %s", post_url)
        return post
