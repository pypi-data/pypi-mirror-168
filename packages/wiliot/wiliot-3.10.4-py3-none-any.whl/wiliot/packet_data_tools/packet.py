...
class Packet(object):
    ...
    def as_dict(self, sprinkler_index=None):  # None not tested
        ...
        data['gw_process'] = [self.gw_process] * dict_len  # boolean should be a list for pd.DataFrame.from_dict(data)
        data['is_valid_packet'] = [self.is_valid_packet] * dict_len  # boolean should be a list for pd.DataFrame.from_dict(data)
