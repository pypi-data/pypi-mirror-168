...
def serialize_data_from_file(values=None, logger=None, log_path=None, management_client=None, is_packet_data=False):
    ...
    if values['upload'] == 'Yes':
        if is_packet_data:
            upload_success = upload_to_cloud_api(batch_name=batch_name, tester_type='offline-test', run_data_csv_name=run_data_csv_name,
                                tags_data_csv_name=tags_data_csv_name, to_logging=True, env=values['env'], logger_=logger.name,
                                is_batch_name_inside_logs_folder=False, packets_instead_tags=is_packet_data)
        else:
            upload_success = upload_to_cloud_api(batch_name=batch_name, tester_type='offline-tester', run_data_csv_name=run_data_csv_name,
                                tags_data_csv_name=tags_data_csv_name, to_logging=True, env=values['env'], logger_=logger.name,
                                is_batch_name_inside_logs_folder=False, packets_instead_tags=is_packet_data)
