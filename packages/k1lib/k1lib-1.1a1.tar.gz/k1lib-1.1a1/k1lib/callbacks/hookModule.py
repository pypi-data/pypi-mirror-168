...
class ModuleData:
    ...
    def _plot(self, axes, field:str, rangeSlice:slice, f):
        ...
        axes[0].plot(fR.range_[::step] | f | cli.deref(), forwardData[fR.slice_][::step] | f | cli.deref(), alpha=0.5)
        axes[1].plot(bR.range_[::step] | f | cli.deref(), backwardData[bR.slice_][::step] | f | cli.deref(), alpha=0.5)
    def __repr__(self):
...
@k1lib.patch(HookModule)
@k1lib.patch(Module)
def plot(self, *fields:List[str], f=cli.iden()):
    ...
    if len(fields) == 0:
        ...
        for field in forwardData.state.keys():
            ...
            if type(fieldData) == list and k1lib.isNumeric(fieldData[0]):
                fields.append(field)
    return k1lib.viz.SliceablePlot(partial(plotF, modules, fields, f))