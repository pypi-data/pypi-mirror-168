...
class BertLayer(nn.Module): 
    ...
    def __init__(self, config, has_word_attn=False, seq_len=None): #has_word_attn参数zj
        ...
        if self.has_word_attn:
            ...
            #将词的维度转换成和Character Input隐藏层维度一样，方便Attention
            self.word_transform = nn.Linear(config.word_embed_dim, config.hidden_size) # word_embed_dim对应论文中公式1的W1的dw
            self.word_word_weight = nn.Linear(config.hidden_size, config.hidden_size) # hidden_size对应论文中公式2的W1,W2的dc
            attn_W = torch.zeros(config.hidden_size, config.hidden_size) # dc × dc
            self.attn_W = nn.Parameter(attn_W) # nn.Parameter作用是将一个不可训练的Tensor的参数转换成可训练的参数
            ...
            if seq_len:
                ...
                # 使用线性层
                # self.layout_transform = nn.Linear(seq_len, 1)
                # self.hidden_weight = nn.Linear(config.hidden_size, config.hidden_size) # hidden_size对应论文中公式2的W1,W2的dc
                
                # 使用bilstm
                # self.rnn = nn.LSTM(seq_len, 64, num_layers=1, batch_first=True, bidirectional=True) #hidden_size为64   
    ...
    def forward(
        ...
        # apply_chunking_to_forward函数节省显存，包装了一个切分小batch或者低维数操作的功能，计算完后再拼接返回
        #feed_forward_chunk函数就执行BertIntermediate，BertOutput的forward()函数
        #因此，layer_output为transformer encoder的结果
        layer_output = apply_chunking_to_forward(  #直接看feed_forward_chunk函数即可
        ...
        if self.has_word_attn:
            ...
            # transform
            # 以下对应公式2，论文中的Non-Linear Transformation模块 
            # 获取多音字对应匹配到的word_embedding
            ...
            #以下为公式3
            # attention_output = attention_output.unsqueeze(2) # [N, L, dc] -> [N, L, 1, dc]
            # 先将输出的所有隐藏层经过一个全连接层进行转换
            
            #使用一维卷积
            hidden=self.conv1(layer_output.clone()) # [N, L, dc] -> [N, 1, dc]
            hidden=self.relu(hidden)

            #print("1111111:",hidden.size())
            # 使用线性层转换维度
            # hidden = self.layout_transform(torch.transpose(layer_output, 1, 2)) # 从[N, L, dc] -> [N, dc, 1]
            # hidden = self.act(hidden) #新加
            # hidden = hidden.transpose(1, 2) # 从[N, dc, 1] -> [N, 1, dc]
            # hidden = self.hidden_weight(hidden) #新加
            #hidden = self.dropout(hidden) #加这行会降低精度
            
            #使用Bilstm
            # hidden=torch.transpose(layer_output.clone(), 1, 2) # [N, L, dc] -> [N, dc, L]
            # lstm_output,_=self.rnn(hidden) # [N, dc, L] -> [N, dc, 2*lstm_hidden]
            # batch_size = layer_output.size(0)
            # hidden = lstm_output[torch.arange(batch_size)]
            ...
            #print("00110203123:", alpha.size())
            alpha = alpha.squeeze(2)  # [N, 1, W]
            #print("033333333333:", alpha.size())
            alpha = alpha + (1 - poly_input_word_mask.float()) * (-10000.0) #用于忽略padding的影响，input_word_mask为0或1，因此正常词不会受影响，可参考博客https://blog.csdn.net/myboyliu2007/article/details/115611660
            #print("133333333333:", alpha.size())
            alpha = torch.nn.Softmax(dim=-1)(alpha)  # [N, 1, W]
            #print("233333333333:", alpha.size())
            alpha = alpha.unsqueeze(-1)  # [N, 1, W, 1]
            #print("333333333333:", alpha.size())
            # 以下为公式4
            #print("00000:", word_outputs.size()) #torch.Size([32, 1, 5, 768])
            #print("00000000:", alpha.size()) #torch.Size([32, 32, 5, 1])
            #a=word_outputs * alpha #32,32,5,768
            #print("444222222:", a.size())
            weighted_word_embedding = torch.sum(word_outputs * alpha, dim=2)  # [N, 1, W, dc] 和 [N, 1, W, 1] -> [N, 1, dc] 
            # print("222222222:", weighted_word_embedding.size())
            # 以下为公式5 
            # 混合多音字隐藏层和经过attention得到的词信息
            #print("111111111:", layer_output_poly_hidden.size()) # torch.Size([32, 1, 768])
            #print("222222222:", weighted_word_embedding.size()) # torch.Size([32, 32, 768])
            layer_output_poly_hidden = layer_output_poly_hidden + weighted_word_embedding
            #print("888888888:", layer_output_poly_hidden.size()) # torch.Size([32, 32, 768])
