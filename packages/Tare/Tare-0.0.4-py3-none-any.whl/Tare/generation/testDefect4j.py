...
def getNodeNo(root, subroot, idx):
    ...
    nextidx = idx + 1
    for x in root.child:
        t, nextidx = getNodeNo(x, subroot, nextidx)
        if t != -1:
            return t, 0
    return -1, nextidx
...
def testDefect4j(bugid, pathloc, locnum, pathcontext, model, pathVoc, savepath='./', useCuda=True):
    ...
    for i, xss in enumerate(prlist):
        for idx in ids[i]:
            ...
            timecurr = time.time()
            x = xss
            locationdir = os.path.join(pathloc, '%s/%d.txt' % (x.lower(), idx))
            ...
            dirs = os.popen('defects4j export -p dir.src.classes -w buggy%s'%idsss).readlines()[-1]
            data = []
            for j in tqdm(range(locnum)):
                if j >= len(location):
                    break
                try:
                    #if j != 2:
            ...
            solveone2(data, model, savepath, pathVoc, pathcontext, useCuda=useCuda)
            os.system('rm -rf buggy%s' % (idsss))