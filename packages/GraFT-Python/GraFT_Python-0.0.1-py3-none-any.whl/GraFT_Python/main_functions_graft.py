...
params_default = {'max_learn': 1e3,
    ...
    'normalizeSpatial' : False,                                      # default behavior - time-traces are unit norm. when true, spatial maps normalized to max one and time-traces are not normalized
     ...
     'xmin' : 151,#151
     'xmax' : 250,#350                                                            # Can sub-select a portion of the full FOV to test on a small section before running on the full dataset
     'ymin' : 151,#101
     'ymax' : 200,#300 
...
def GraFT(data, phi, kernel, params):
    ...
    phi = dictInitialize(phi, (n_times, n_cols))
...
def dictionaryRWL1SF(data, phi, corr_kern, A = [], params = {}):
    ...
    if checkEmptyList(A):
        A = np.random.rand(n_neurons, p)# np.zeros((n_neurons, p))
...
def dictInitialize(phi = [], shape_dict = [], norm_type = 'unit', to_norm = True, dist = 'uniform'):
    ...
    if len(phi) > 0:        
        ...
        #if dist == 'uniform':
        phi = createMat(shape_dict, dist)


        return dictInitialize(phi, shape_dict, norm_type, to_norm, dist)

    
def createMat(shape_dict, dist = 'uniforrm', params = {'mu':0, 'std': 1}):
    """
    Parameters
    ----------
    shape : TYPE
    ...
    if len(data) == 0 or np.sum(data**2) == 0:
        ...
        if nonneg:
            ...
            if A == [] or (A==0).all():
                ...
                if np.isnan(A).any(): raise ValueError('fgfdgdfg')
                #lb = np.zeros((p,1)), 
                #ub = np.inf*np.ones((p,1)),
