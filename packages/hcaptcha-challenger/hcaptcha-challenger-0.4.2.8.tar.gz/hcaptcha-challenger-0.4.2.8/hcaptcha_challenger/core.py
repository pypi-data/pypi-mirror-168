...
class HolyChallenger:
    ...
    def classify(self, prompt: str, images: typing.List[str]) -> typing.Optional[typing.List[bool]]:
        ...
        for img in images:
            ...
            try:
                with open(img, "rb") as file:
                    response.append(
                        model.solution(img_stream=file.read(), label=self.label_alias[self.label])
                    )
            except Exception as err:
                logger.exception(err)
                response.append(False)
