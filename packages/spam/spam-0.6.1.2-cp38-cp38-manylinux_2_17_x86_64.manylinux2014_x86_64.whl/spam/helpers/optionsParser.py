...
def pixelSearch(parser):
    ...
    if (args.SEARCH_RANGE[0] > args.SEARCH_RANGE[1]) or \
       (args.SEARCH_RANGE[2] > args.SEARCH_RANGE[3]) or \
       (args.SEARCH_RANGE[4] > args.SEARCH_RANGE[5]):
...
def gdicParser(parser):
    ...
    parser.add_argument(dest='MESH_FILE',
                        default=None,
                        type=argparse.FileType('r'),
                        help="Path to HDF5 file containing mesh information needed for the correlation (points, connectivity, labels)")
...
def mesh(parser):
    ...
    parser.add_argument('-ascii',
                        ...
                        action="store_true",
                        dest='ASCII',
                        help='Activate ascii output. Default = False')

    parser.add_argument('-tl',
                        '--tetLabel',
                        action="store_true",
                        dest='TET_LABEL',
                        help='Label an image with the tetrahedra IDs. Default = False')
    ...
    parser.add_argument('-tlim',
                        ...
                        default=None,
                        type=argparse.FileType('r'),
                        help="Path to tiff file containing the image used to get size of the tetrahedra labeled image (doesn't actually read the image).")

    parser.add_argument('-tlshape',
                        '--tetLabelImageShape',
                        nargs=3,
                        type=int,
                        default=None,
                        dest='TET_IM_SHAPE',
                        help='Size of the tetrahedra labeled image (zyx).')
    ...
    if not any([args.MESH_TYPE_CUBOID]):
        print(f'WARNING: you need to enter at least one mesh type: --createCuboid, ...')
        exit()
    ...
    # test if we want to label an image with the tet ID
    if args.TET_LABEL:
        # get image size
        if args.TET_LABEL_IM:
            # 1. from other image
            tiff = tifffile.TiffFile(args.TET_LABEL_IM.name)
            args.TET_IM_SHAPE = tiff.series[0].shape

        elif args.TET_IM_SHAPE:
            # 2. from input shape
            pass

        else:
            # 3. from mesh coordinates (on the fly)
            print("WARNING: you need to enter either a image (-tlim) or a shape (-tlshape). SPAM is not that clever... on purpose... because devs are lazy.")
            exit()
...
def meshSubdomains(parser):
    ...
    help = [
        "Create VTK outputs for each mesh.",
        "Default = False."
    ]
    parser.add_argument('-vtk',
                        action="store_true",
    ...
    help = [
        "Create MSH outputs for each mesh.",
        "Default = False."
    ]
    ...
    help = [
        "Launch gmsh graphical interface.",
        "Default = False."
    ]
