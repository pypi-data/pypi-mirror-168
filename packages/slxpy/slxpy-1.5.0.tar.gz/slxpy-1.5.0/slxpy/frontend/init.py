...
def init_interactive(workdir: Path):
    check_valid_workspace(workdir)
    model, class_name, namespace = get_info_interactive(workdir)
    workdir.mkdir(parents=True, exist_ok=True)
    model_cfg_path, env_cfg_path = init(workdir, model, class_name, namespace)
    ...
    output_text = textwrap.dedent(f"""\
        ...
        Initialize config files:
            ...
            {env_cfg_path.name:<12} - Raw/gym-like environment wrapper config.
    """).strip()
...
def init(workdir: Path, model: str, class_name: str, namespace: str):
    ...
    model_cfg_text = env.get_template(f"{C.model_config_name}.jinja").render(config=model_config)
    model_cfg_path = workdir / C.model_config_name
    model_cfg_path.write_text(model_cfg_text)

    env_config = EnvConfig.default()
    env_cfg_text = env.get_template(f"{C.env_config_name}.jinja").render(config=env_config)
