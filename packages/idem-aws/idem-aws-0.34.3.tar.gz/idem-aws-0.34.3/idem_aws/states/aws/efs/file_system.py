    ...
    **Autogenerated function**
    ...
    if current_state:
        is_throughput_mode_updated = (
            hub.tool.aws.efs.file_system_utils.is_throughput_mode_updated(
                current_state=current_state, desired_state=desired_state
            )
        )

        is_tags_updated = (current_state.get("tags")) != tags

        is_backup_policy_updated = current_state.get("backup") != desired_state.get(
            "backup"
        )
    ...
    if before:
        ...
        # Update tags
        if is_tags_updated:
            update_tags = await hub.exec.aws.efs.tag.update_tags(
                ctx,
                resource_id=resource_id,
