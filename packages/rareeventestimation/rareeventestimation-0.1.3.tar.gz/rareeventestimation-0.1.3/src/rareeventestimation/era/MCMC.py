...
from rareeventestimation.era.EMvMFNM import EMvMFNM, lognakagamipdf, logvMFpdf, logsumexp
...
class MCMC:
    ...
    def __init__(self, uk, gk_current, wnork, nchain, burn, limit_state_fun_current, sigma, xi=0, gk_fine=[], limit_state_fun_fine=id,
                 dim_last=None, mcmc_type='aCS', k_init=1):
    ...
    def mcmc_procedure_tempering_acs(self):
        ...
        self.gk_current = np.zeros([self.nsamlev+self.burn])  # delete previous samples
        self.uk = np.zeros([self.nsamlev+self.burn, self.dim])  # delete previous samples
        ...
        for k in range(self.nchain):
            ...
            for i in range(self.lenchain + self.burn):
                ...
                acceptance_rate = norm.cdf(-gcand / self.sigma) / norm.cdf(-g0 / self.sigma)
    ...
    def mcmc_procedure_tempering_vmfnm(self):
        ...
        [mu_vMFNM, kappa, m, omega, alpha_em] = EMvMFNM(self.uk.T, self.wnork, self.k_init)
        mu_vMFNM = mu_vMFNM.T

        self.gk_current = np.zeros([self.nsamlev+self.burn])  # delete previous samples
        self.uk = np.zeros([self.nsamlev+self.burn, self.dim])  # delete previous samples

        ucandidates = vMFNM_sample(mu_vMFNM, kappa, omega, m, alpha_em, self.nchain * (self.lenchain + self.burn))
        z = 0
        ...
        for k in range(self.nchain):
            ...
            for i in range(self.lenchain + self.burn):
                ...
                acceptance_rate = sp.stats.norm.cdf(-gcand / self.sigma) / sp.stats.norm.cdf(-g0 / self.sigma) *\
                                  np.exp(-likelihood_ratio_log(u0.reshape((1, self.dim)), mu_vMFNM, kappa, omega, m, alpha_em)[:] +
                                         likelihood_ratio_log(ucand.reshape((1, self.dim)), mu_vMFNM, kappa, omega, m, alpha_em)[:])
