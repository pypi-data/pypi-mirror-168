...
async def real_esrgan_get_mode(event: MessageEvent, state: T_State = State()):
...
async def real_esrgan_handle_img(event: MessageEvent, state: T_State = State()):
    # 先拿到需要转换的图
    ...
    try:
        ...
        result = await get_result(json_data)  # 然后进行超分辨率重建
        if result is None:
