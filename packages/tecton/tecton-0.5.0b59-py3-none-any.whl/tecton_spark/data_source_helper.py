...
def validate_data_source_timestamp_present_for_feature_view(data_source: BatchDataSource):
    """
    Verifies the data_source has the timestamp_column_properties field or supports_time_filtering is True if it is a Data Source Function.

    :param data_source: batch data source
    """
    if not is_data_source_function(data_source) and not data_source.HasField("timestamp_column_properties"):
        raise Exception(f"DataSource {data_source.fco_metadata.name} requires timestamp column name for FeatureView")

    if is_data_source_function(data_source) and not data_source.spark_data_source_function.supports_time_filtering:
        raise Exception(
