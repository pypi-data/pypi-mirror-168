...
def block(block_name, path=None):
  ...
  with open('flow/new_file.json', 'r') as f:
    ...
    if len(json_data['blocks']) == 0: 
      data = {'project_id': json_data['project_id'], 'block_name': block_name, 'username': os.environ['username'], 'key': os.environ['key']}
    else:
      data = {'project_id': json_data['project_id'], 'block_name': block_name, 'connect_with': json_data['blocks'][-1], 'username': os.environ['username'], 'key': os.environ['key']}
...
def node(node_name, node_description, path=None):
  ...
  with open('flow/new_file.json', 'r') as f:
    ...
    if len(json_data['nodes']) == 0: 
      data = {'node_name': node_name, 'node_description': node_description,
              'username': os.environ['username'], 'key': os.environ['key'], 'block_id': json_data['blocks'][-1]}
    else:
