  ...
  with open('flow/new_file.json', 'r') as f:
    ...
    if len(json_data['nodes']) == 0: 
      data = {'node_name': node_name, 'node_description': node_description,
              'username': os.environ['username'], 'key': os.environ['key'], 'block_id': json_data['blocks'][-1], 'filepath': sys.argv[0]}
    else:
...
def tracefunc(frame, event, arg, indent=[0]):
    ...
    if "Python" in frame.f_code.co_filename or "<" in frame.f_code.co_filename:
        pass
    else:
        if event == "call":
            ...
            node(frame.f_code.co_name, node_description="", path=None)
        elif event == "return":
