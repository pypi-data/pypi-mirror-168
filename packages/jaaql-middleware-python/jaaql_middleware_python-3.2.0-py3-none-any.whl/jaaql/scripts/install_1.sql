...
create or replace function drop_tenant_database(db_name postgres_addressable_name) returns void as
$$
DECLARE
    the_tenant text;
BEGIN
    ...
    DELETE FROM tenant_database WHERE name = db_name AND tenant = the_tenant;
    PERFORM dblink_exec('dbname=' || current_database(), 'DROP DATABASE ' || the_tenant  || '__' + db_name);
END
...
BEGIN
    ...
    if not skip_db then
        PERFORM dblink_exec('dbname=' || current_database(), 'CREATE DATABASE ' || the_tenant  || '__' || db_name);
...
BEGIN
    ...
    GRANT execute on function drop_tenant_database(postgres_addressable_name) to jaaql with grant option;
