...
class JAAQLController(BaseJAAQLController):
    ...
    def create_app(self):
        ...
        def submit(http_inputs: dict, user_id: str, tenant: str):
            return self.model.submit(http_inputs, user_id, tenant)

        @self.cors_route('/submit-file', DOCUMENTATION__submit_file)
        def submit_file(http_inputs: dict, user_id: str, tenant: str):
            return self.model.submit(http_inputs, user_id, tenant, force_transactional=True)
        ...
        def reset_password_reset(http_inputs: dict, tenant: str):
            return self.model.reset_password_perform_reset(http_inputs, tenant)

        @self.cors_route(ENDPOINT__jaaql_emails, DOCUMENTATION__email)
        def emails(http_inputs: dict, tenant: str, oauth_token: str, user_id: str, ip_address: str, is_public: bool):
            if is_public:
                raise HttpStatusException(ERR__user_public, HTTPStatus.UNAUTHORIZED)

            return self.model.send_email(http_inputs, oauth_token, user_id, tenant, ip_address)

        @self.cors_route('/documents', DOCUMENTATION__document)
        def documents(http_inputs: dict, oauth_token: str, ip_address: str, tenant: str, response: JAAQLResponse):
            if self.is_get():
                return self.model.fetch_document(http_inputs, response)
            else:
                return self.model.render_document(http_inputs, oauth_token, ip_address, tenant)
