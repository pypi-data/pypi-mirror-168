...
class JAAQLModel(BaseJAAQLModel):
    ...
    def log(self, user_id: str, occurred: datetime, duration_ms: int, exception: str, contr_input: str, ip: str, status: int,
        ...
        pass  # TODO use a log threading pool
    ...
    def drop_email_account(self):
        pass  # TODO change how we are doing email accounts, end to end signup request with data in a new database, end to end reset password
    ...
    def add_tenant(self, connection: DBInterface, name: str, tenant_admin_password: str = None, attach_tenant_admin_as: str = None):
        ...
        tenant_admin_account_id = execute_supplied_statement_singleton(connection, QUERY__create_tenant, {KEY__the_tenant: name})[RET__rows][0]
    ...
    def add_account_password(self, user_id: str, password: str):
        ...
        execute_supplied_statement(self.jaaql_lookup_connection, QUERY__add_password, {KEY__account: user_id, KEY__password_hash: password},
                                   encryption_key=self.get_db_crypt_key(), encryption_salts={KEY__password_hash: salt},
                                   encrypt_parameters=[KEY__password_hash])
    ...
    def fetch_user_from_username(self, username: str, tenant: str):
        return execute_supplied_statement_singleton(self.jaaql_lookup_connection, QUERY__fetch_user_from_username, {KEY__username: username},
                                                    ...
                                                    encryption_salts={KEY__username: self.get_repeatable_salt(tenant)}, as_objects=True)[KEY__id]
    ...
    def user_invite(self, user_id: str, configuration: str = None, email_template: str = None, template_lookup: dict = None):
    ...
    def fetch_interface_for_tenant(self, tenant: str, application: str, configuration: str, schema: str):
        ...
        return self.create_interface_for_db(ROLE__jaaql, tenant + "__" + db_name)
    ...
    def request_signup(self, inputs: dict):
        template = execute_supplied_statement_singleton(self.jaaql_lookup_connection, QUERY__fetch_email_template_with_app_url, {
            KEY__name: inputs[KEY__email_template_name],
        ...
        invite_keys = self.user_invite(user_id, inputs[KEY__configuration], inputs[KEY__email_template_name], pkey_vals)
    ...
    def signup_status(self, inputs: dict):
        ...
        existed = False
        try:
            self.fetch_user_from_username(resp[KEY__email], self.jaaql_lookup_connection)
            existed = True
    ...
    def sign_up_user_with_token(self, token: str, password: str):
        ...
        }, encryption_salts={KEY__enc_username: self.get_repeatable_salt(resp[KEY__tenant])}, decrypt_columns=[KEY__password_hash, KEY__username],
        ...
        self.add_account_password(res[KEY__user_id], password)
        ...
        if resp[KEY__email_template] and resp[ATTR__data_lookup_json]:
            template = execute_supplied_statement_singleton(self.jaaql_lookup_connection, QUERY__fetch_email_template_with_app_url,
                                                            {KEY__name: resp[KEY__email_template], KEY__tenant: tenant,
                                                             KEY__configuration: resp[KEY__configuration]}, as_objects=True)
            interface = self.fetch_interface_for_tenant(tenant, template[KEY__application], resp[KEY__schema], template[KEY__schema])
    ...
    def send_reset_password_email(self, inputs: dict):
        ...
        if user_exists:
            ...
            self.email_manager.construct_and_send_email(self.url, app_url, template, user[KEY__user_id], inputs[KEY__email], None, {},
                                                        optional_params)
        else:
            params = {
    ...
    def render_document(self, inputs: dict, oauth_token: str, ip_address: str, tenant: str):
        inputs[KEY__oauth_token] = self.refresh_auth_token(oauth_token, ip_address)
    ...
    def send_email(self, inputs: dict, oauth_token: str, user_id: str, tenant: str, ip_address: str):
        ...
        if template[KEY__data_validation_table] is not None and params is None:
            params = {}
        if template[KEY__data_validation_table] is not None:
            interface = self.fetch_interface_for_tenant(tenant, template[KEY__application], inputs[KEY__configuration],
                                                        template[KEY__schema])
        ...
        self.email_manager.construct_and_send_email(self.url, app_url, template, user_id, recipient_names, recipients, {}, params,
                                                    ...
                                                    attachment_access_token=self.refresh_auth_token(oauth_token, ip_address))

    def reset_password_perform_reset(self, inputs: dict, tenant: str):
        ...
        if resp[KEY__email_template] and resp[ATTR__data_lookup_json]:
            template = execute_supplied_statement_singleton(self.jaaql_lookup_connection, QUERY__fetch_email_template_with_app_url,
                                                            {KEY__name: resp[KEY__email_template], KEY__tenant: tenant}, as_objects=True)
            interface = self.fetch_interface_for_tenant(tenant, template[KEY__application], resp[KEY__schema], template[KEY__schema])
            res[KEY__parameters] = self.select_from_data_validation_table(interface, template[KEY__data_validation_table],
                                                                          json.loads(resp[ATTR__data_lookup_json]))
    ...
    def create_tenant_account(self, connection: DBInterface, tenant: str, inputs: dict, specifying_tenant: bool = False):
        ...
        if inputs.get(inputs[KEY__password]):
            self.add_account_password(account_id, inputs[KEY__password])
    ...
    def install(self, db_connection_string: str, superjaaql_password: str, default_tenant_password: str, install_key: str, allow_uninstall: bool):
        if self.jaaql_lookup_connection is None:
            ...
            resp = self.jaaql_lookup_connection.execute_query(conn, QUERY__setup_system)
            tenant_admin_account_id = resp[1][0][0]
            self.jaaql_lookup_connection.commit(conn)
    ...
    def get_auth_token(self, username: str, tenant: str, ip_address: str, password: str = None, response: JAAQLResponse = None):
        ...
        salt_tenant_username = self.get_repeatable_salt(tenant + hash_username)

        encrypted_ip = jaaql__encrypt(ip_address, self.get_db_crypt_key(), salt_tenant_username)
    ...
    def fetch_pivoted_application_schemas(self, tenant: str):
        data = execute_supplied_statement(self.jaaql_lookup_connection, QUERY__fetch_application_schemas, {KEY__the_tenant: tenant}, as_objects=True)
    ...
    def create_interface_for_db(self, user_id: str, database: str):
        ...
        return create_interface(self.config, address, port, database, username, password=password, role=user_id)

    def submit(self, inputs: dict, user_id: str, tenant: str, force_transactional: bool = False):
        ...
        for cur in inputs:
            if KEY__application in cur:
                ...
                if KEY__schema in cur:
                    ...
                    if len(config) == 1:
                        found_db = config[config.keys[0]][KEY__database]
                    else:
            ...
            if cur[KEY__database] not in required_dbs:
                required_dbs[cur[KEY__database]] = self.create_interface_for_db(user_id, cur[KEY__database])

        return [InterpretJAAQL(required_dbs[cur.pop(KEY__database)]).transform(inputs, force_transactional=force_transactional) for cur in inputs]
