...
class BaseJAAQLModel:
    ...
    def group(self, data: [dict], key_field_name: str):
        ...
        for itm in data:
            ...
            if len(list_itm) == 0:
                if len(val_itm) == 1:
                    outer_dict[data[key_field_name]] = itm[val_itm[0]]
                else:
                    outer_dict[data[key_field_name]] = {key: itm[key] for key in val_itm}
            else:
                outer_dict[data[key_field_name]] = self.group(list_itm[0], key_field_name)
