...
from graph_parse.enums import ComparitiveOperationEnum, TraversalEventEnum
...
class Node:
    ...
    def from_pydantic(self, pydantic_model: BaseModel) -> Node:
        return type(f"{pydantic_model.__class__.__name__}Node", (Node,), {
...
class TraversalEventPayload(BaseModel):
    event: TraversalEventEnum
    time: int 
...
def default_event_handler(payload):
    if payload.event == TraversalEventEnum.EdgeException:
...
class Graph:
    ...
    def __init__(self, edges: List[Edge] = [], outputs: List[str] = [], event_handler: Callable = default_event_handler):
        self.edges = edges
    ...
    def traverse(self, *args: Union[Node, BaseModel], outputs: List[str] = []) -> dict:
        ...
        args = [arg if isinstance(arg, Node) else Node().from_pydantic(arg.dict()) for arg in args]
                ...
                for edge in edges:
                    ...
                    if edge.function:
                        ...
                        try:
                            ...
                            output = None 
                            self.trigger_event(TraversalEventEnum.EdgeException, exception=e)
    ...
    def create_subgraph(self, outputs: List[str]) -> Graph:
        frontier = sum([self.search(sink=(output,), comparitive_operator=ComparitiveOperationEnum.Intersect) for output in outputs], [])
        ...
        while frontier:
            ...
            edges += frontier
            frontier = sum([self.search(sink=fr_edge.source, comparitive_operator=ComparitiveOperationEnum.Equal, exclude=edges_serialized) for fr_edge in frontier], [])

        return self.__class__(edges=list(edges), outputs=outputs, event_handler=self.event_handler)

    def trigger_event(self, event: TraversalEventEnum, **kwargs):
        self.event_handler(TraversalEventPayload(
            event=event,
            time=time_ns(),
            **kwargs
        ))