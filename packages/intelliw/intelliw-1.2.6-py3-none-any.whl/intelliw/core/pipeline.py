...
class Pipeline:
    ...
    def _initialize(self, alg_file, model_file=None):
        try:
            ...
            if config.is_server_mode():
                make_decorators_server(instance, self.reporter)
    ...
    def importmodel(self, path, load_model=True):
        ...
        try:
            ...
            _st, _at, _ft, _tt = self.prepare_transforms(trans_cfg)
            self.splitdata_transforms = _st
            self.alldata_transforms = _at
            self.feature_transforms = _ft
            gl.set_dict({
                'feature_process': _ft,
                'timeseries_process': _tt
            })
    ...
    def _get_train_kwargs(self, reader, column_meta):
        ...
        kwargs = {
            ...
            "column_meta": gl.pop("column_meta") if gl.get("column_meta") is not None else [i["code"] for i in column_meta],
        ...
        if gl.get("model_type") == 9:
            kwargs["timeseries_process_cfg"] = gl.pop(
                "timeseries_process", {}).get(
                    Const.pre_train)
    ...
    def train(self, datasets: DataSets):
        ...
        try:
            ...
            self.reporter.report(message.ok_train_finish)
    ...
    @linkserver()
    def _infer_process(self, data, request, func='infer', need_featrue=True):
        if need_featrue:  # 前处理
    ...
    def infer(self, data, request, func='infer', need_featrue=True):
        ...
        try:
            ...
            result = self._infer_process(data, request, func, need_featrue)
                    ...
                    for item in functions_cfg:
                        ...
                        if key == functions.Const.feature_process:
                            ...
                            if func is not None:
                                ...
                                _t = split_transforms if can_split is True else alldate_transforms
                                _t = init_dict(_t)
    ...
    def _raise_exception(self, msg: message.CommonResponse, e: Exception = None, is_farmeworl_error=True):
        raise_logger = logger if is_farmeworl_error else _get_algorithm_logger()
