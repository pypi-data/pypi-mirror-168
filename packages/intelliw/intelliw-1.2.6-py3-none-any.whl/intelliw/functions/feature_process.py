...
try:
    ...
    raise ImportError("\033[31mIf use feature process, you need: pip install scikit-learn\033[0m")
...
def feature_process(data, config: list, stage: str) -> Iterable:
    ...
    if processor_mode == "infer":
        ...
        if intelliw_core_config.SOURCE_TYPE == 4:
            return data
...
class NumericFeature(Feature):
    ...
    def do_process(self, data_frame):
        ...
        if self.mode == 'train':
            ...
            elif self.check_repair_config['miss_repair'] == '6':
                if not(self.group_cols is not None and type(self.group_cols) == list and len(self.group_cols) != 0):
                    raise FeatureProcessException(
                        f"origin_col_num:{self.index}, when miss_repair='6', group_cols can't be empty")
                groups = data_frame.groupby(
                    self.group_cols, observed=True).groups
                for group, idxs in groups.items():
                    df_tmp = feat_df.loc[idxs].copy()
                    if len(df_tmp[~df_tmp.isnull()]) == 0:
                        fill_item = feat_df.mode()[0]       # 如果类内全是null，则用全局替代
                    else:
                        fill_item = df_tmp[~df_tmp.isnull()].mode()[0]
                    feat_df.loc[idxs] = feat_df.loc[idxs].fillna(fill_item)
            elif type(self.check_repair_config['miss_repair']) == dict:
                if self.check_repair_config['miss_repair']['repair_type'] == '7':
                    if not('quantile_low' in self.check_repair_config['miss_repair'] and 'quantile_high' in self.check_repair_config['miss_repair']):
                        raise FeatureProcessException(
                            f"origin_col_num:{self.index}, when miss_repair='7', quantile_low and  quantile_high must be set")
                    low = self.check_repair_config['miss_repair']['quantile_low']
                    high = self.check_repair_config['miss_repair']['quantile_high']
                    if not(0.0 <= low <= high <= 1.0):
                        raise FeatureProcessException(
                            f"origin_col_num:{self.index}, quantile_low and quantile_high must between 0.0 and 1.0, and quantile_low must be smaller than quantile_high ")
                    val = feat_df.quantile(
                        low) + (feat_df.quantile(high)-feat_df.quantile(low)) * random.random()
                    if self.type in ['int', 'INT']:
                        val = int(val)
                    feat_df = feat_df.fillna(val)
            ...
            if self.auto_check_repair:
                ...
                if 'outlier_check' not in self.check_repair_config:
                    raise FeatureProcessException(
                        f"origin_col_num:{self.index}, outlier_check config missing!")
                if not (self.check_repair_config['outlier_check'] in ['1', '2', '3']):
                    raise FeatureProcessException(
                        f"origin_col_num:{self.index}, outlier_check must be in ['1', '2', '3']!")
                low, high = 0, 0
                # 3 西格玛
                if self.check_repair_config['outlier_check'] == '1':
                    ...
                    low = feat_df.quantile(
                        0.25) - 1.5 * (feat_df.quantile(0.75) - feat_df.quantile(0.25))
                    high = feat_df.quantile(
                        0.75) + 1.5 * (feat_df.quantile(0.75) - feat_df.quantile(0.25))
            ...
            if not('outlier_repair' in self.check_repair_config):
                raise FeatureProcessException(
                    f"origin_col_num:{self.index}, outlier_repair config missing!")
            if not (self.check_repair_config['outlier_repair'] in ['1', '2', '3']):
                raise FeatureProcessException(
                    f"origin_col_num:{self.index}, outlier_repair must in ['1', '2', '3']!")
