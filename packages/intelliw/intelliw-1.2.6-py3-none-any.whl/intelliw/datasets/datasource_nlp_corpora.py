...
LastEditTime: 2022-09-22 14:59:48
LastEditors: Hexu
Description: nlp数据集
...
class DataSourceNLPCorpora(AbstractDataSource):
    ...
    def __init__(self, source_address: str, ds_id: str, source_train_type: int, download_dir='./tmp_local_nlp_corpora_origin_data/'):
        ...
        self.reader_type = config.NLP_CORPORA_INPUT_TYPE if config.NLP_CORPORA_INPUT_TYPE else CorpusInputType.local
        self.dirpath = [config.NLP_CORPORA_TRAIN_FILEPATH,
                        config.NLP_CORPORA_VAL_FILEPATH, config.NLP_CORPORA_TEST_FILEPATH]

        # 3种模式  loacl 和 file 需要进行预处理
        # loacl  语料文件在本地 本地测试用
        # file   接口获取的为语料文件
        # row    接口获取的为某行语料
        ...
        if self.reader_type != CorpusInputType.row:
            if self.reader_type == CorpusInputType.local:
                loacl_corpus = config.NLP_CORPORA_PATH
            else:
                loacl_corpus = self.__download_file(
                    self.source_address, self.ds_id, self.source_train_type, download_dir)
            self._file_list, self._csv_label = self._file_process(loacl_corpus)
    ...
    def __download_file(self, source_address, ds_id, source_train_type, download_dir):
        ...
        response = iuap_request.post_json(
            url=source_address, data=request_data)
        response.raise_for_status()
        body = response.json
        file_links = body.get("data", [])
        if len(file_links) == 0:
            raise DatasetException(f"nlp corpora is empty, get data is {body}")
    ...
    class __OnlineReader:
        ...
        def _read_page(self, page_num, page_size):
            ...
            response = iuap_request.post_json(
                url=self.input_address, data=request_data)
            response.raise_for_status()
