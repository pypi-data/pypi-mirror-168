...
LastEditTime: 2022-07-05 11:09:05
LastEditors: Hexu
...
def try_read_file(filepath, nrows=None, skiprows=None):
    ...
    for c in coding:
        try:
            csv = pd.read_csv(filepath, nrows=nrows,
                              skiprows=skiprows, encoding=c)
            ok = True
            break
    ...
    class __DataSourceReaderLocalCsv:
            ...
            if type(gl.get("target_metadata")) is list:
                for m in gl.get("target_metadata"):
                    ...
                    if target_col > len(self.meta):
                        raise DatasetException(f"target column index:{target_col}, There are {len(self.meta)} columns in the source csv, index out of range")
                    self.meta[target_col] = {'code': m["target_name"]}
        ...
        class __Reader:
            ...
            def read(self, amount=1):
                try:
                    self.csv = try_read_file(
                        self.file_path, skiprows=self.line_offset)
                ...
                batch = self.csv.iloc[0:amount].values.tolist()
                if len(batch) < amount:
                    self.is_finish = True
