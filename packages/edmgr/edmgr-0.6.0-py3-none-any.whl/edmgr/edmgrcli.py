...
from edmgr.download import HTTPDownload, ProgressBar
...
def download_artifacts(
    ...
    for artifact in artifacts:
        ...
        download_stream: HTTPDownload = client.get_artifact_download_http(
            entitlement_id=entitlement_id, release_id=release_id, artifact_id=art_id
        )

        while download_stream.error.get("name") == "download-error-eula-required":
            echo_error(download_stream.error["description"])
            echo(
                "Please sign the EULA at the following link, then proceed or abort the download."
            )
            echo()
            echo(download_stream.error["url"])
            echo()
            confirm(
