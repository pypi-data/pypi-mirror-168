...
def clustering(data,eps, density_metric='cutoff', dist_metric='euclidean',knn_method="kd_tree", knn_num=20, leaf_size=300, connect_eps=1,fast=False):
    ...
    density = cluster.get_density(eps,density_metric,matrix_dist)
...
class BaseCluster(object):
    ...
    def _calc_repulsive_force_fast(self,data, k_num, density, leaf_size,dist_metric="euclidean",knn_method="kd_tree"):
        ...
        if(knn_method=="ball_tree"):
            tree = BallTree(data,leaf_size=leaf_size,metric=DistanceMetric.get_metric(dist_metric))
        else:
            tree = KDTree(data, leaf_size=leaf_size,metric=DistanceMetric.get_metric(dist_metric))
...
class Cluster(BaseCluster):
    ...
    def get_density(self,eps,metric,dist_mat):
        if(metric=='gauss'):
...
if __name__=='__main__': 
    ...
    filePath = r'../../../data/temp_2020_week.nc'
    dataset = xr.open_dataset(filePath)
    temp_2020 = dataset['air_temperature']
    # lon_range = temp_2020.lon[(temp_2020.lon>70)&(temp_2020.lon<140)]
    # lat_range = temp_2020.lat[(temp_2020.lat>15)&(temp_2020.lat<55)]
    # var = temp_2020.sel(lon=lon_range,lat = lat_range)
    # temp_xarray = var.coarsen(lon=5).mean().coarsen(lat=5).mean()
    temp_xarray=temp_2020.sel(time=slice(temp_2020.time[0],temp_2020.time[19]))
    knn_num_arr = [100]
    for i in knn_num_arr:
        print("-------knn_num: ",i)
        result = st_clustering(xr_data=temp_xarray,spatial_eps=4,time_eps=3,attr_eps=2.5,knn_method="kd_tree",knn_num=i,leaf_size=3000,connect_eps=0.9)
    pass