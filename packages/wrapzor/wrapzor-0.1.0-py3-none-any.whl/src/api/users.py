...
@inject_tokens()
async def create_user(data: dict, client: AsyncClient, api: Api) -> Response:
...
async def get_users(
    ...
    response = await client.get(url, params=params) if params else await client.get(url)
    return response


@inject_tokens()
async def update_user(
    data: dict, client: AsyncClient, api: Api, user_id: str | None = None
) -> Response:
    if user_id is None:
        url = f"{api.domain}/crm/{api.version}/users"
    else:
...
async def get_user_territories_validate_before_transfer(
    user_id: str, client: AsyncClient, api: Api, territory_id: str | None = None
) -> Response:
    if territory_id is None:
