...
class IndustryTracking:
    ...
    def load(self):
        ...
        self.index_con_roe = pd.read_excel('{0}index_con_roe.xlsx'.format(data_path))
        self.index_con_roe = self.index_con_roe.set_index('DateTime').unstack().reset_index()
        self.index_con_roe.columns = ['INDEX_SYMBOL', 'TRADE_DATE', 'CON_ROE_FY1']
        self.index_con_roe['TRADE_DATE'] = self.index_con_roe['TRADE_DATE'].apply(lambda x: str(x).replace('-', '')[:8])
        ...
        self.index_con_roe_yoy = pd.read_excel('{0}index_con_roe_yoy.xlsx'.format(data_path))
        self.index_con_roe_yoy = self.index_con_roe_yoy.set_index('DateTime').unstack().reset_index()
        self.index_con_roe_yoy.columns = ['INDEX_SYMBOL', 'TRADE_DATE', 'CON_ROE_YOY']
        self.index_con_roe_yoy['TRADE_DATE'] = self.index_con_roe_yoy['TRADE_DATE'].apply(lambda x: str(x).replace('-', '')[:8])
        ...
        self.index_pb_lf = pd.read_excel('{0}index_pb_lf.xlsx'.format(data_path))
        self.index_pb_lf = self.index_pb_lf.set_index('DateTime').unstack().reset_index()
        self.index_pb_lf.columns = ['INDEX_SYMBOL', 'TRADE_DATE', 'PB_LF']
        self.index_pb_lf['TRADE_DATE'] = self.index_pb_lf['TRADE_DATE'].apply(lambda x: str(x).replace('-', '')[:8])
    ...
    def get_amt_prosperity(self):
        ...
        for factor in factor_list:
            for index in self.indexs['代码'].unique().tolist():
                ...
                # ret or forward_ret
                index_finance_index['CLOSE_INDEX'] = index_finance_index['CLOSE_INDEX'].pct_change(1).shift(-1)
                # forward_excess_ret
                # index_finance_index = index_finance_index.merge(index_daily_k_bmk.rename(columns={'CLOSE_INDEX': 'CLOSE_INDEX_BMK'}), on=['TRADE_DATE'], how='left')
                # index_finance_index['CLOSE_INDEX'] = (index_finance_index['CLOSE_INDEX'].pct_change(1) - index_finance_index['CLOSE_INDEX_BMK'].pct_change(1)).shift(-1)
                ...
                # index_finance_index = index_finance_index.merge(index_daily_k_index,  on=['INDEX_SYMBOL', 'REPORT_DATE'], how='left')
                # index_finance_index['CLOSE_INDEX'] = index_finance_index['CLOSE_INDEX'].shift(-1)
                # index_finance_index = index_finance_index.dropna().sort_values('REPORT_DATE')
                if len(index_finance_index) <= 2:
                    continue
        ...
        indexs_corr.to_excel('{0}factor_corr/factor_forward_ret_corr.xlsx'.format(self.data_path))
        ...
        # indexs_corr.dropna(subset=['营业收入（元）']).to_excel('{0}factor_corr/factor_forward_excess_ret_corr.xlsx'.format(self.data_path))
        # indexs_corr.to_excel('{0}factor_corr/factor_forward_ret_1q_corr.xlsx'.format(self.data_path))
        # indexs_corr.dropna(subset=['营业收入（元）']).to_excel('{0}factor_corr/factor_forward_excess_ret_1q_corr.xlsx'.format(self.data_path))
        return
    ...
    def get_amt_con_prosperity(self):
        ...
        for factor in factor_list:
            for index in self.indexs['代码'].unique().tolist():
                ...
                # ret or forward_ret
                index_consensus_index['CLOSE_INDEX'] = index_consensus_index['CLOSE_INDEX'].pct_change(1).shift(-1)
                # forward_excess_ret
                ...
                # index_consensus_index['CLOSE_INDEX'] = (index_consensus_index['CLOSE_INDEX'].pct_change(1) - index_consensus_index['CLOSE_INDEX_BMK'].pct_change(1)).shift(-1)
        ...
        indexs_corr.to_excel('{0}factor_corr/con_factor_forward_ret_corr.xlsx'.format(self.data_path))
        ...
        # indexs_corr.dropna(subset=['一致预测营业收入（元）']).to_excel('factor_corr/{0}con_factor_forward_excess_ret_corr.xlsx'.format(self.data_path))
        # indexs_corr.to_excel('{0}factor_corr/con_factor_forward_ret_1q_corr.xlsx'.format(self.data_path))
        # indexs_corr.dropna(subset=['一致预测营业收入（元）']).to_excel('{0}factor_corr/con_factor_forward_excess_ret_1q_corr.xlsx'.format(self.data_path))
        return
    ...
    def get_amt_prosperity_factor(self):
        ...
        index_daily_k = index_daily_k.drop(['YEAR_MONTH', 'MONTH'], axis=1).set_index('TRADE_DATE').sort_index()
        ...
        index_factor = index_finance.merge(index_consensus, on=['REPORT_DATE', 'INDEX_SYMBOL'], how='left')
        index_corr = pd.read_excel('{0}corr_{1}.xlsx'.format(self.data_path, self.end_date))
        # index_corr = pd.read_excel('{0}excess_corr_{1}.xlsx'.format(self.data_path, self.end_date))
        # index_corr = pd.read_excel('{0}decay_corr_{1}.xlsx'.format(self.data_path, self.end_date))
        # index_corr = pd.read_excel('{0}decay_excess_corr_{1}.xlsx'.format(self.data_path, self.end_date))
        ...
        for idx, index in enumerate(self.indexs[self.indexs['板块'] == '先进制造']['代码'].unique().tolist()):
            ...
            if len(index_factor_list) == 0:
                continue
            index_factor_index = index_factor_index.drop('INDEX_SYMBOL', axis=1).set_index('REPORT_DATE').sort_index().dropna(subset=['MAIN_INCOME'])
            ...
            index_factor_index = pd.DataFrame(index_factor_index).sort_index().reset_index()#.iloc[-5:]
            index_factor_index.columns = ['REPORT_DATE', 'PROSPERITY']
            index_factor_index['INDEX_SYMBOL'] = index
            index_factor_index = index_factor_index.merge(index_daily_k_index, on=['INDEX_SYMBOL', 'REPORT_DATE'], how='inner')
            index_prosperity_list.append(index_factor_index)
            corr = round(np.corrcoef(index_factor_index['PROSPERITY'], index_factor_index['FORWARD_RET'])[0, 1], 2)
            ...
            ax1.set_xlabel('')
            ax1.set_ylabel('景气水平', fontsize=10)
        ...
        ax2.legend(loc=2)
        plt.tight_layout()
        plt.savefig('{0}prosperity/prosperity.png'.format(self.data_path))
        # plt.savefig('{0}prosperity/excess_prosperity.png'.format(self.data_path))
        # plt.savefig('{0}prosperity/decay_prosperity.png'.format(self.data_path))
        # plt.savefig('{0}prosperity/decay_excess_prosperity.png'.format(self.data_path))
        index_prosperity.to_excel('{0}prosperity/prosperity.xlsx'.format(self.data_path))
        # index_prosperity.to_excel('{0}prosperity/excess_prosperity.xlsx'.format(self.data_path))
        # index_prosperity.to_excel('{0}prosperity/decay_prosperity.xlsx'.format(self.data_path))
        # index_prosperity.to_excel('{0}prosperity/decay_excess_prosperity.xlsx'.format(self.data_path))
        ...
        index_prosperity_corr.to_excel('{0}prosperity/prosperity_corr.xlsx'.format(self.data_path))
        # index_prosperity_corr.to_excel('{0}prosperity/excess_prosperity_corr.xlsx'.format(self.data_path))
        # index_prosperity_corr.to_excel('{0}prosperity/decay_prosperity_corr.xlsx'.format(self.data_path))
        # index_prosperity_corr.to_excel('{0}prosperity/decay_excess_prosperity_corr.xlsx'.format(self.data_path))
        return
    ...
    def combine_factor(self):
        ...
        index_daily_k = index_daily_k.unstack().reset_index()
        index_daily_k.columns = ['INDEX_SYMBOL', 'TRADE_DATE', 'FORWARD_RET']
        index_prosperity = pd.read_excel('{0}prosperity/excess_prosperity.xlsx'.format(self.data_path), index_col=0)
        index_prosperity['REPORT_DATE'] = index_prosperity['REPORT_DATE'].astype(str)
        index_prosperity = index_prosperity.merge(self.calendar_trade_df.rename(columns={'CALENDAR_DATE': 'REPORT_DATE'}), on=['REPORT_DATE'], how='left')
        index_pe_ttm = self.index_valuation.copy(deep=True)
        ...
        for idx, index in enumerate(self.indexs[self.indexs['板块'] == '先进制造']['代码'].unique().tolist()):
            ...
            ax1.set_xlabel('')
            ax1.set_ylabel('估值景气指标', fontsize=10)
            ax1.set_ylim([-200.0, 100.0])  # [-200.0, 100.0], [-7.0, 5.0]
        ...
        sns.barplot(ax=ax2, x='REPORT_DATE', y='FORWARD_RET', data=index_prosperity, hue='板块', hue_order=index_order, palette=new_line_color_list)
        ax2.set_xlabel('')
        ax2.set_ylabel('未来一个季度超额收益', fontsize=10)  # 未来一个季度超额收益, 未来四个季度超额收益
        ax2.yaxis.set_major_formatter(FuncFormatter(to_percent))
        ax2.set_ylim([-50.0, 300.0])  # [-30.0, 120.0], [-50.0, 300.0]
        ax2.legend(loc=2)
        plt.tight_layout()
        plt.savefig('{0}factor/factor_1.png'.format(self.data_path))
        # plt.savefig('{0}factor/decay_factor_1.png'.format(self.data_path))
        index_factor.to_excel('{0}factor/factor_1.xlsx'.format(self.data_path))
        # index_factor.to_excel('{0}factor/decay_factor_1.xlsx'.format(self.data_path))
        ...
        index_factor_corr.to_excel('{0}factor/factor_corr_1.xlsx'.format(self.data_path))
        # index_factor_corr.to_excel('{0}factor/decay_factor_corr_1.xlsx'.format(self.data_path))
        return
