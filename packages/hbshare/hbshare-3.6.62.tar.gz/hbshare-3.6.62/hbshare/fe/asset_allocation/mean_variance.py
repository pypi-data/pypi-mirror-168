...
class MeanVariance:
    def __init__(self, asset_type, asset_list, method, start_date, end_date, is_reblance, reblance_type, n, frequency, compute_return_days, compute_risk_days,
                       ...
                       target_risk=0.03, target_return=0.05, risk_aversion=3.0, risk_free_rate=0.03, benchmark='000300'):
                    ...
                    for date in reblance_list:
                        ...
                        if min(ret_density) < 0.3 or min(cov_density) < 0.3:
                            ...
                            if min(ret_density) < 0.3 and min(cov_density) >= 0.3:
                                print('{0} have no enough nav data for compute return model!'.format(','.join([self.asset_list[i] for i in ret_density_ser.index.tolist()])))
                            if min(ret_density) >= 0.3 and min(cov_density) < 0.3:
                                print('{0} have no enough nav data for compute risk model!'.format(','.join([self.asset_list[i] for i in cov_density_ser.index.tolist()])))
                            if min(ret_density) < 0.3 and min(cov_density) < 0.3:
                                print('{0} have no enough nav data for compute return model and risk model!'.format(','.join([self.asset_list[i] for i in list(set(ret_density_ser.index.tolist() + cov_density_ser.index.tolist()))])))
                            interval_data = dict()
