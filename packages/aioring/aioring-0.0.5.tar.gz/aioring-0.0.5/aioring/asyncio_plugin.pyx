...
#    cdef bint closed
    ...
    def on_ring_events(self):
        cdef list cqes
        try:
            cqes = self.ioring.get_completions()
        except:
            return
...
cdef class ReaderAsyncioPlugin(IoRingAsyncioPlugin):
    ...
    def __init__(self, *args):
        super().__init__(*args)
        self.loop.add_reader(self.ioring.get_event().fd, self.on_ring_events)
...
cdef class WaitingThreadAsyncioPlugin(IoRingAsyncioPlugin):
    ...
    def thread_run(self):
        self.event = self.ioring.get_event()
