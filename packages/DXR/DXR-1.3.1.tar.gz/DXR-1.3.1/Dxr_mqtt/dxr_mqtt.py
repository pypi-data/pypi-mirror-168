...
def monky_print(*args, **kwargs):
    ...
    for arg in args:
        if isinstance(arg, Exception):
            ...
            break
    if isException:
        logger.opt(colors=True).exception(f'<yellow>{file_name}:{line}</yellow>(<blue>{thread_name}:{thread_count}</blue>) {args}')
    else:
        # 将args转换为字符串
        args = [str(arg) for arg in args]
        # 将字符串拼接起来
        args = " ".join(args)
        logger.opt(colors=True).info(f'<yellow>{file_name}:{line}</yellow>(<blue>{thread_name}:{thread_count}</blue>) {args}')
