...
def monky_print(*args, **kwargs):
    ...
    if 'exception' in kwargs:
        e = kwargs['exception']
        logger.exception(f"{file_name}:{line} thread_name:{thread_name} thread_count:{thread_count} {args} {e}")
    else:
        logger.info(f"{file_name}:{line} thread_name:{thread_name} thread_count:{thread_count} {args}")
