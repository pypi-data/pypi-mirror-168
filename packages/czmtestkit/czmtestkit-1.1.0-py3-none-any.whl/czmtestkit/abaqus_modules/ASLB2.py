...
def ASLB2(dict):
	...
	**Create and submit Asymmetric Single Leg Bending (ASLB) test with plain strain boundary conditions using Abaqus/CAE.**
    ...
    .. Note:: The function :func:`ASLB` is only different from :func:`ASLB2` in the material definition of the bulk. While :func:`ASLB` defines both top and bottom adherands or plies in :numref:`ASLBscheme2` with the same engineering constants, :func:`ASLB2` defines these regions separately.
    
    The ASLB specimen with geometry from :numref:`ASLBscheme2` is generated with unit width (:math:`B = 1`). The mixed-mode damage is modelled using the `BK criteria`_
    Additionally, along with boundary conditions from :numref:`ASLBscheme2`, the translation along `E2` of all the nodes on faces perpendicular to `E2` are fixed to replicate plain-strain boundary conditions.
	...
	:Parameters:
        ...
        **dict** (`dict`):
            ...
            :'GcNormal': Fracture toughness in opening mode :math:`G_{C_{I}}`. See :numref:`BiLinTSLscheme5`
            
            :'GcShear': Fracture toughness in shear mode :math:`G_{C_{sh}}`. See :numref:`BiLinTSLscheme5`
            
            :'gFailureNormal': Final or failure displacement gap in opening mode :math:`\Delta_{I}^f`. See :numref:`BiLinTSLscheme5`
            
            :'gFailureShear': Final or failure displacement gap in shear mode :math:`\Delta_{sh}^f`. See :numref:`BiLinTSLscheme5` 
    ...
    .. figure:: /imgs/Bilinear.png
        ...
        **Bilinear Traction Separation Law** or the linear softening law `[2]`_.
        ...
        `Interfaces tend to have different properties for opening (mode-I) and shear modes (mode-II and mode-III) in` :numref:`FractureModes5` `, resulting in different traction separation laws represented above using subscripts `I` for opening mode and `sh` for shear modes for the parameters.`
