...
def ADCB2powerLaw(dict):
    """

	**Create and submit Asymmetric Double Cantilever Beam (ADCB) test with plain strain boundary conditions using Abaqus/CAE.**
    
    .. Note:: The function :func:`ADCB2powerLaw` is only different from :func:`ADCB2` in the mixed-mode criteria of the damage law. While :func:`ADCB2powerLaw` uses the power law, :func:`ADCB2` uses the `BK criteria`_.
    
    The ADCB specimen with geometry from :numref:`ADCBscheme3` is generated with unit width (:math:`B = 1`). The mixed-mode damage is modelled using the power law
    Additionally, along with boundary conditions from :numref:`ADCBscheme3`, the translation along `E2` of all the nodes on faces perpendicular to `E2` are fixed to replicate plain-strain boundary conditions.
	...
	:Parameters:
        ...
        **dict** (`dict`):
            ...
            :'GcNormal': Fracture toughness in opening mode :math:`G_{C_{I}}`. See :numref:`BiLinTSLscheme3`
            
            :'GcShear': Fracture toughness in shear mode :math:`G_{C_{sh}}`. See :numref:`BiLinTSLscheme3` 
            
            :'gFailureNormal': Final or failure displacement gap in opening mode :math:`\Delta_{I}^f`. See :numref:`BiLinTSLscheme3`
            
            :'gFailureShear': Final or failure displacement gap in shear mode :math:`\Delta_{sh}^f`. See :numref:`BiLinTSLscheme3` 
            
            :'powerLaw': :math:`\gamma` of the power law.
    ...
    .. figure:: /imgs/Bilinear.png
        ...
        **Bilinear Traction Separation Law** or the linear softening law `[2]`_.
        ...
        `Interfaces tend to have different properties for opening (mode-I) and shear modes (mode-II and mode-III) in` :numref:`FractureModes3` `, resulting in different traction separation laws represented above using subscripts `I` for opening mode and `sh` for shear modes for the parameters.`
    ...
    .. Tip:: Analytical results for this test using  Timoshenko beam theory and Castigliano theorem as described in appendix B of the master thesis `[1]`_ can be obtained using methods of the :class:`czmtestkit.py_modules.ADCB` class. 
    ...
    **References:**
    ...
    mdb.models['Model-1'].materials['Material-2'].quadsDamageInitiation.DamageEvolution(
        mixedModeBehavior=POWER_LAW, power=powerLaw, table=((GcNormal, GcShear, GcShear), ), type=
        ENERGY)
