...
def ADCB(dict):
	...
	**Create and submit Asymmetric Double Cantilever Beam (ADCB) test with plain strain boundary conditions using Abaqus/CAE.**
    ...
    .. Note:: The function :func:`ADCB` is only different from :func:`ADCB2` in the material definition of the bulk. While :func:`ADCB` defines both top and bottom adherands or plies in :numref:`ADCBscheme` with the same engineering constants, :func:`ADCB2` defines these regions separately.
    
    The ADCB specimen with geometry from :numref:`ADCBscheme` is generated with unit width (:math:`B = 1`). The mixed-mode damage is modelled using the `BK criteria`_
    Additionally, along with boundary conditions from :numref:`ADCBscheme`, the translation along `E2` of all the nodes on faces perpendicular to `E2` are fixed to replicate plain-strain boundary conditions.
	...
	:Parameters:
        ...
        **dict** (`dict`):
            ...
            :'GcNormal': Fracture toughness in opening mode :math:`G_{C_{I}}`. See :numref:`BiLinTSLscheme`
            
            :'GcShear': Fracture toughness in shear mode :math:`G_{C_{sh}}`. See :numref:`BiLinTSLscheme`
            
            :'gFailureNormal': Final or failure displacement gap in opening mode :math:`\Delta_{I}^f`. See :numref:`BiLinTSLscheme`
            
            :'gFailureShear': Final or failure displacement gap in shear mode :math:`\Delta_{sh}^f`. See :numref:`BiLinTSLscheme` 
    ...
    .. figure:: /imgs/Bilinear.png
        ...
        **Bilinear Traction Separation Law** or the linear softening law `[2]`_.
        ...
        `Interfaces tend to have different properties for opening (mode-I) and shear modes (mode-II and mode-III) in` :numref:`FractureModes` `, resulting in different traction separation laws represented above using subscripts `I` for opening mode and `sh` for shear modes for the parameters.`
