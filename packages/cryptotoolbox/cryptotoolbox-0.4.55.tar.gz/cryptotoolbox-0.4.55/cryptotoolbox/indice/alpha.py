...
def compute_alpha_metrics(data_df = None, core_tokens=['BTC', 'ETH'], extra_tokens=['AVAX'], moderating_factor = 0.1, frequency='weekly', generate_kpis = True, plot_html=True, stop_loss_quantile = 0.75,take_profit_quantile = 0.75,write_excel_file = True, local_root_directory = None, plot_metrics = True,plot_html_save = True):
    ...
    if generate_kpis:
        ...
        freq_aggregated_df['epochs compounded return'] = np.cumprod(1+freq_aggregated_df['epoch realized return'].values )
...
def compute_alpha_monthly_metrics(data_df = None, core_tokens=['BTC', 'ETH'], extra_tokens=['AVAX'], moderating_factor = 0.1, frequency='weekly', generate_kpis = True, plot_html=True, stop_loss_quantile = 0.75,take_profit_quantile = 0.75,write_excel_file = True, local_root_directory = None, plot_metrics = True,plot_html_save = True):
    ssjs = core_tokens + extra_tokens
    universe_literal = '_'.join(ssjs)
    if plot_html:
        title = f'HedgeFarm Strategy Universe {universe_literal} moderating factor {moderating_factor}'
    ...
    if generate_kpis:
        ...
        to_store_df['weekly_epoch_number'] = to_store_df['has_just_rebalanced'].cumsum()

        def monthlize_epoch(row):
            weekly_epoch_number = row['weekly_epoch_number']
            epoch_number = weekly_epoch_number-weekly_epoch_number%4
            return epoch_number

        to_store_df['epoch_number'] = to_store_df.apply(monthlize_epoch, axis = 1)
        to_store_df['epoch_number_diff'] = to_store_df['epoch_number'].diff()
        to_store_df['has_just_rebalanced'] = to_store_df['epoch_number_diff'] >0
        ...
        if write_excel_file:
            writer = pd.ExcelWriter(local_root_directory + f'{universe_literal}_monthly_ls_epochs_details_ls.xlsx',
                                    engine='xlsxwriter')
            freq_aggregated_df.to_excel(writer, sheet_name='epoch')
            writer.save()

            writer = pd.ExcelWriter(
                local_root_directory + f'{universe_literal}_monthly_kpi_df_ls_strategy.xlsx',
                engine='xlsxwriter')
            # whole_period_kpi_df.to_excel(writer, sheet_name=frequency)
            btd_kpis = btd_kpis.to_frame()
            btd_kpis.columns = [me_strat]
