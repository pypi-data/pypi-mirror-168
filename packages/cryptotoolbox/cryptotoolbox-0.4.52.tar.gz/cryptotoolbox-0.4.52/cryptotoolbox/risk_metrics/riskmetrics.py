        ...
        if not 'drawdown' in me_strat:
            ...
            if write_file:
                writer = pd.ExcelWriter(local_root_directory + f'perf_dd_{me_strat_name}_{clean_title}.xlsx',
                                        engine='xlsxwriter')
                data.to_excel(writer, sheet_name='kpi')
                writer.save()
            ...
            def get_month_date(row):
                return datetime(year=row['year'], month=row['month'])
            ...
            epochkpis_df['epoch compounded return'] = np.cumprod(1. + epochkpis_df['epoch realized return'].values)
