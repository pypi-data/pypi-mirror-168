...
class Exposer(object):
    ...
    def start(self):
        schedule.clear()
        schedule.every(self.timer_duration).seconds.do(self.expose_data_to_influxdb())
        schedule.run_pending()
        logger.info("{} monitor job init succeed for every {} seconds!", self.app_name, self.timer_duration)
