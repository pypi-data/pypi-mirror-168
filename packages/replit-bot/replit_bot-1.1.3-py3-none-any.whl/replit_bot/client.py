...
class Client(BasicEventEmitter):
    def __init__(self, sid: str = None, username: str = None, password: str = None) -> None:
        super()
        super().__init__()
        ...
        if ("currentUser" not in data or not data["currentUser"]): raise InvalidSid("SID invalid")
    ...
    def search(self, query: str, options: Dict[str, Any] = {}):
        global post
        current = {"cache": True, "query": query, "categories": ["Tags"], "onlyCalculatedHits": False}
        ...
        for r in search["replResults"]["results"]["items"]:
            repl = Repl(self)
            repl.update(r)
            if (options.cache): self.repls.cache[repl.id] = repl
        ...
        for u in search["userResults"]["results"]["items"]:
            user = User(self)
            user.update(u)
            if (options.cache): self.users.cache[user.username] = user
            results["users"][user.username] = user
        for p in search["postResults"]["results"]["items"]:
            post = Post(self)
            post.update(u)
            if (options.cache): self.users.cache[post.id] = post
    ...
    def login(self, username: str, password: str) -> str:
        return post(self.sid, "login", {"username": username,
                                        "password": password})
...
class UserManager:
    ...
    def fetch(self, userResolvable, options: Dict[str, Any] = {}):
        ...
        if (not res[query]): return None
        user = User(self.c)
        user.update(res[query])
        if (options.cache): self.cache[user.username] = user
        return user
    ...
    def search(self, query: str, options: Dict[str, Any] = {}):
        ...
        current.update(options)
        options = JSDict(current)
        res = post(self.c.sid, "userSearch", vars = {
            "query": query,
            "limit": options.limit
        })
        ...
        for u in users:
            user = User(self.c)
            user.update(u)
            if (options.cache): self.c.users.cache[user.username] = user
...
class UserEventManager:
    ...
    def fetch(self, options: Dict[str, Any] = {}) -> Dict[str, Any]:
        ...
        current.update(options)
        options = JSDict(current)
        res = post(self.c.sid, "userEvents", vars = {
            "count": options.limit,
            "after": ""
        })
        ...
        for e in events:
            ...
            event.update(e)
            if (options.cache): self.cache[event.id] = event
            c[event.id] = event
        return c
...
class ReplManager:
        ...
        if (getattr(self, "user", False) and self.user):
            if (self.user == self.c.user):
                ...
                for path in options.paths:
                    res = post(self.c.sid, "dashboardRepls", vars = {
                        "path": path,
                        "count": options.limit
                    })
                    ...
                    for r in repls:
                        repl = Repl(self.c)
                        repl.update(r)
                        if (options.cache): self.cache[repl.id] = repl
                        c[repl.id] = repl
                return c
            else:
                ...
                options.update(kwargs)
                options = JSDict(options)
                res = post(self.c.sid, "profileRepls", vars = {
    ...
    def create(self, options: Dict[str, Any] = {}):
        ...
        cache = options["cache"]
        del options["cache"]
        res = post(self.c.sid, "createRepl", vars = {
            "input": options
        })
        r = res["createRepl"]
        if ("id" not in r or not r["id"]): return None
        repl = Repl(self.c)
        repl.update(r)
        return repl
...
class PostManager:
    ...
    def fetch(self, **kwargs: Dict[str, Any]) -> None:
        ...
        if (name.case("Client") or name.case('Bot')):
            ...
            options.update(kwargs)
            options = JSDict(options)
            res = post(self.c.sid, "post", vars = {
                "id": id
            })
            if ("post" not in res or not res["post"]): return None
            _post = Post(self.c)
            ...
            options.update(kwargs)
            options = JSDict(options)
            res = post(self.c.sid, "userPosts", vars = {
            ...
            for p in posts:
                _post = Post(self.c)
                _post.update(p)
                if (options.cache): self.cache[_post.id] = _post
                c[_post.id] = _post
            return c
    def trending(self, options: Dict[str, Any] = {}) -> Dict[str, Any]:
        if (type(self.parent).__name__ == "Client" or type(self.parent).__name__ == "Bot"): return None
        options = {"cache": True, "limit": 10, "tags": []}
        res = post(self.c.sid, "trending", vars = {
            "options": {
...
class NotificationManager:
    def __init__(self, client) -> None:
        ...
        def _(notifs) -> None:
            ...
            if (len(notifs) > 0): self.markAsRead()
        
        self.track.on("update", _)

    def fetch(self, options: Dict[str, Any] = {}) -> None:
        current = {"cache": True, "seen": False, "limit": 10}
        current.update(options)
        options = JSDict(current)
        res = post(self.c.sid, "notifications", vars = {
            "count": options.limit,
            "seen": options.seen
        })
    ...
    def fetch_all(self, options: Dict[str, Any] = {}) -> None:
        ...
        res = {
            "notifications": {
                "items": [None for i in range(options.limit)]
            }
        }
...
class MultiplayerManager:
    ...
    def invite(self, userResolvable) -> None:
        ...
        post(self.c.sid, "addMultiplayer", vars = {
    ...
    def remove(self, userResolvable) -> None:
        user = self.c.users.fetch(userResolvable)
        if (not user): return None
        post(self.c.sid, "removeMultiplayer", vars = {
            "username": user.username,
            "replId": self.repl.id,
        })
...
class FollowingManager:
    ...
    def fetch(self, options: Dict[str, Any] = {}) -> Dict[str, Any]:
        ...
        current.update(options)
        options = current
        res = post(self.c.sid, "follows", vars = {
            "username": self.user.username,
            "count": options["limit"]
        })
    ...
    def setFollowing(self, userResolvable, should_follow: bool = True, options: Dict[str, Any] = {"cache": True}) -> bool:
        user = self.c.users.fetch(userResolvable)
        if (not user): return False
        res = post(self.c.sid, "follow", vars = {
            "input": {
                "targetUserId": user.id,
                "shouldFollow": should_follow
            }
        })
        if (not res["setFollowing"]): return False
...
class FollowersManager:
    ...
    def fetch(self, options: Dict[str, Any] = {}) -> Dict[str, Any]:
        ...
        current.update(options)
        options = JSDict(current)
        res = post(self.c.sid, "followers", vars = {
            "username": self.user.username,
            "count": options.limit
        })
        ...
        if (name.case("Client") or name.case("Bot")):
            ...
            if (not options.force):
                if (id in self.cache):
                    ...
                    return _comment
            res = post(self.c.sid, "replComment", vars = {
                "id": id
            })
            c = res["replComment"]
            if ('message' in c): return None
            ...
            options.update(arguements)
            options = JSDict(options)
            res = post(self.c.sid, "replComments", vars = {
                "id": self.parent.id,
                "count": options.limit
            })
            ...
            options = {"cache": True, "limit": 10}
            options.update(arguements)
            res = post(self.c.sid, "userComments", vars = {
                "input": {
...
class User:
    ...
    def update(self, data: Dict[str, Any]):
        """update profile"""
        time_created, presence_status = (None, None)
        ...
        if (time_created is not None): self.time_created = time_created
        if (presence_status is not None):
            self.last_seen = presence_status["lastSeen"]
            self.online = presence_status["isOnline"]

    def set_follow(self, should_follow: bool = True) -> bool:
    ...
    def set_block(self, should_block: bool = True) -> bool:
        """blocks a user and return whether they are blocking you"""
        res = post(self.c.sid, "block", vars = {
            "input": {
                "targetUserId": self.id,
...
class CurrentUser(User):
    ...
    def update(self, data: Dict[str, Any] = {}):
        ...
        if (login["googleAuth"]): self.auth["google"] = login["googleAuth"]["accessToken"]
        if (login["githubAuth"]): self.auth["github"] = login["githubAuth"]["accessToken"]
        if (login["facebookAuth"]): self.auth["facebook"] = login["facebookAuth"]["accessToken"]
        if (login["clui"]): self.clui = login["clui"]
        if (login["usernameRepl"]):
            del login["usernameRepl"]["url"]
            self.usernameRepl = self.c.repls.fetch(login["usernameRepl"]["id"], **login["usernameRepl"])
        return self
    ...
    def change(self, options: Dict[str, Any] = {}):
        if ("image" not in options): options["image"] = None
        image = options["image"]
        del options["image"]
        if (image):
            i = parse(image)
            if (i):
                ...
                data = _raw_post("https://replit.com/data/images/upload", data={
                    "context": 'profile-image',
                    "image": i
                }, headers = _headers)
                if (data.status_code == 200):
                    options["profileImageId"] = data["data"]["id"]
        res = post(self.c.sid, "updateUser", vars = {
            "input": options
        })
...
class UserEvent:
    ...
    def update(self, data: Dict[str, Any]):
        ...
        for key in data:
            setattr(self, key, data[key])
        if (user_events["timeUpdated"]): self.timeUpdated = user_events["timeUpdated"]
        if (user_events["user"]): self.user = self.c.users.fetch(user_events["user"])
        if (user_events["repl"]): self.repl = self.c.repls.fetch(user_events["repl"])
        if (user_events["following"]): self.following = self.c.users.fetch(user_events["following"])
        if (user_events["comment"]): self.comment = self.c.comments.fetch(user_events["comment"])
        return self
    ...
    def update(self, data: Dict[str, Any]):
        ...
        for key in data:
            if (key in thread_message_data):
                ...
                setattr(self, key, data[key])
        if (thread_message_data["timeCreated"]): self.timeCreated = thread_message_data["timeCreated"]
        if (thread_message_data["timeUpdated"]): self.timeUpdate = thread_message_data["timeUpdated"]
        if (thread_message_data["user"]): self.c.users.fetch(thread_message_data["user"])
        return self
    ...
    def update(self, data: Dict[str, Any]):
        ...
        for key in data:
            if (key in thread_data):
                ...
                setattr(self, key, data[key])
        if (thread_data["timeCreated"]): self.timeCreated = thread_data["timeCreated"]
        if (thread_data["timeUpdated"]): self.timeUpdated = thread_data["timeUpdated"]
...
class Repl:
    def __init__(self, client) -> None:
        self.c = client
        self.threads = []
        self.multiplayer = MultiplayerManager(self.c, self)
    ...
    def update(self, data: Dict[str, Any] = {}):
        ...
        for key in data:
            if (key in repl_data):
                ...
                setattr(self, key, data[key])
        if (repl_data["layoutState"]): self._layoutState = repl_data["layoutState"]
        if (repl_data["owner"]):
            u = User(self.c)
            ...
            if (repl_data["database"]):
                self.db = db
                self.db.db_url = repl_data["database"]["jwt"]
        if (repl_data["timeCreated"]): self.timeCreated = repl_data["timeCreated"]
        if (repl_data["timeUpdated"]): self.timeUpdated = repl_data["timeUpdated"]
        if (repl_data["currentUserPermissions"]): self.currentUserPermissions = repl_data["currentUserPermissions"]
    ...
    def fetchThreads(self, options: Dict[str, Any] = {"cache": True}) -> Dict[str, Any]:
        options = JSDict(options)
        res = post(self.c.sid, "replThreads", vars = {
            "id": self.id
        })
        ...
        for t in threads:
            ...
            thread.update(t)
            if (options.cache): self.threads[thread.id] = thread
            c[thread.id] = thread
        return c
    ...
    def fork(self, options: Dict[str, Any] = {}):
        current = {
            ...
            "language": (self.templateInfo.label if getattr(self.templateInfo, "label", False) else self.templateInfo["label"]),
        ...
        r = res["createRepl"]
        if (not r["id"]): return None
        repl = Repl(self.c)
        repl.update()
        if (cache): self.c.repls.cache[repl.id] = repl
        return repl

    def delete(self) -> None:
        post(self.c.sid, "deleteRepl", vars = {
            "id": self.id
        })

    def change(self, options: Dict[str, Any] = {}):
        options.update({"id": self.id})
        res = post(self.c.sid, "updateRepl", vars = {
            "input": options
        })
    ...
    def comment(self, body: str, options: Dict[str, Any] = {"cache": True}):
        options = JSDict(options)
        res = post(self.c.sid, "sendReplComment", vars = {
            "input": {
                "body": body,
                "replId": self.id
            }
        ...
        if (options.cache): self.c.comments.cache[comment.id] = comment
        return comment
...
class Post:
    ...
    def update(self, data: Dict[str, Any]):
        ...
        for key in data:
            setattr(self, key, data[key])
        if (posts["user"]): self.user = self.c.users.fetch(posts['user'])
        if (posts["repl"]): self.repl = self.c.repls.fetch(posts["repl"])
        if (posts["replComment"]):
            comment = Comment(self.c, self.repl)
            comment.update(posts["replComment"])
            self.comment = comment
            self.comment.post = self
        if (posts["timeCreated"]): self.timeCreated = posts["timeCreated"]
        if (posts["timeUpdated"]): self.timeUpdated = posts["timeUpdated"]
        return self
    ...
    def update(self, data: Dict[str, Any]):
        ...
        for key in data:
            if (key in notif_data):
                ...
                setattr(self, key, data[key])
        if (notif_data["timeCreated"]): self.timeCreated = notif_data["timeCreated"]
        if (notif_data["creator"]): self.creator = self.c.users.fetch(notif_data["creator"])
        if (notif_data["replComment"]):
            self.comment = self.c.comments.fetch(int(notif_data["replComment"]["id"]), **notif_data["replComment"])
        return self
...
class Comment:
    ...
    def update(self, data: Dict[str, Any]):
        ...
        if (comment_data["timeCreated"]): self.timeCreated = comment_data["timeCreated"]
        if (comment_data["timeUpdated"]): self.timeUpdated = comment_data["timeUpdated"]
        if (comment_data["user"]): self.user = self.c.users.fetch(comment_data["user"]["username"])
        if (comment_data["repl"]): self.repl = self.c.repls.fetch(comment_data["repl"])
        if (comment_data["post"]): self.post = self.c.posts.fetch(**comment_data["post"])
    ...
    def delete(self) -> None:
        """delete comment"""
        if ("delete" not in self.currentUserPermission): self.currentUserPermission["delete"] = None
        if (self.currentUserPermission["delete"] or self.isAuthor or self.repl.isOwner):
            post(self.c.sid, "deleteComment", vars = {
                "id": self.id
            })

    def reply(self, body: str, **options: Dict[str, Any]):
        """reply comment"""
        self.reply._has_called = True
        self.reply._used_API = getattr(self.reply, "_use_API", False)
        current = {"cache": True, "API": self.reply._use_API}
        current.update(options)
        options = JSDict(current)
        send_text = (f"@{self.user.username} {body}"
                     if (getattr(options, "mention", False) and
                         options.mention)
                     else body)
        if (options.API):
            setattr(self.reply, f"{self.id}_send_text", send_text)
            return send_text
        else:
            res = post(self.c.sid, "sendReplCommentReply", vars = {
                "input": {
                    "body": send_text,
                    "replCommentId": self.id
                }
            })
            if (not res): return None
            comment = Comment(self.c)
