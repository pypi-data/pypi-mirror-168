...
def calculate_coefficients(logits, fourier_basis, key_freqs, p):
    ...
    coses = t.cos(w*t.pi*2/p * (x + y - z)).cuda()
        ...
        with t.inference_mode():
            ...
            metrics = {
                ...
                'excluded_loss': calculate_excluded_loss(
                    ...
                    is_test = is_test,
                    labels=labels),
                'coefficients': calculate_coefficients(p = self.config.p, logits = logits, fourier_basis = fourier_basis, key_freqs = key_freqs),
            }
            wandb.log(metrics)
