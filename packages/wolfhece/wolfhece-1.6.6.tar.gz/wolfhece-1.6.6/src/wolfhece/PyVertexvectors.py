...
class zone:
    ...
    def create_polygon_from_parallel(self,ds):
        ...
        sc = [lsc.interpolate(curs,True) for curs in sloc]

        sl = [lsl.project(curs) for curs in sc]
        sr = [lsr.project(curs) for curs in sc]
        sc = [lsc.project(curs) for curs in sc]
        ...
        zonepoly.find_minmax()
        
        self.parent.fill_structure()

    def plot_linked_poly(self,fig,ax,linked_arrays,linked_labels):
...
class Zones(wx.Frame):
    ...
    def Oncreatepolygons(self,event):
        ...
        self.active_zone.create_polygon_from_parallel(ds)
