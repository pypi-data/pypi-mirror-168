...
class header_wolf():
    ...
    def get_bounds_ij(self,abs=True):
        ...
        return ([self.get_ij_from_xy(mybounds[0][0],mybounds[1][0]),self.get_ij_from_xy(mybounds[0][1],mybounds[0][0])],
...
class CropDialog(wx.Dialog):
    def __init__( self, parent):
        super(CropDialog, self).__init__(parent,title=_('Cropping array'), size=(300, 300),style = wx.DEFAULT_DIALOG_STYLE|wx.TAB_TRAVERSAL|wx.OK)
        ...
        self.m_staticText11 = wx.StaticText( self, wx.ID_ANY, u"OrigX - lower left corner", wx.DefaultPosition, wx.DefaultSize, 0 )
        self.m_staticText11.Wrap( -1 )
        ...
        self.m_staticText12 = wx.StaticText( self, wx.ID_ANY, u"OrigY - lower left corner", wx.DefaultPosition, wx.DefaultSize, 0 )
        self.m_staticText12.Wrap( -1 )
        ...
        self.m_staticText13 = wx.StaticText( self, wx.ID_ANY, u"EndX - upper right corner", wx.DefaultPosition, wx.DefaultSize, 0 )
        self.m_staticText13.Wrap( -1 )
        ...
        self.m_staticText14 = wx.StaticText( self, wx.ID_ANY, u"EndY - upper right corner", wx.DefaultPosition, wx.DefaultSize, 0 )
        self.m_staticText14.Wrap( -1 )
...
class Ops_Array(wx.Frame):
    ...
    def __init__( self, wxparent, parentarray, parentGUI ):
        ...
        selectmethodChoices = [ _("by clicks"), _("inside active vector"), _("inside active zone"), _("inside temporary vector"), _("under active vector"), _("under active zone"), _("under temporary vector") ]
        ...
        maskdata = wx.Button( self.operation, wx.ID_ANY, _("Apply on mask"), wx.DefaultPosition, wx.DefaultSize, 0 )
    ...
    def OnLoadvec(self,event):
        dlg = wx.FileDialog(None,'Select file',wildcard='Vec file (*.vec)|*.vec|Vecz file (*.vecz)|*.vecz|All (*.*)|*.*',style=wx.FD_OPEN)
...
class SelectionData():
    ...
    def add_nodes_to_selectionij(self,ij,verif=True):
        ...
        if nbini!=0:            
            if verif:
                ...
                self.myselection,counts = np.unique(self.myselection,return_counts=True,axis=0)
                
                #les éléments énumérés plus d'une fois doivent être enlevés
                #  on trie par ordre décroissant
                locsort = sorted(zip(counts.tolist(),self.myselection.tolist()),reverse=True)
                counts = [x[0] for x in locsort]
                sel = [x[1] for x in locsort]
    ...
    def select_underpoly(self,myvect:vector):
        ...
        # for i,j in mypoints:
        #     x,y = self.parent.get_xy_from_ij(i,j)
        #     self.add_node_to_selection(x,y,verif=nbini!=0)
    ...
    def update_nb_nodes_sections(self):
        ...
        if nb>10000:
            if not self.hideselection:
                dlg=wx.MessageDialog(None,'Large selection !!' + str(nb) +'\n Would you like to plot the selected cells?',style=wx.YES_NO)
                ret=dlg.ShowModal()
                if ret==wx.ID_YES:
                    self.update_plot_selection=True
                else:
...
class WolfArray(header_wolf):
    ...
    def import_from_gltf(self,fn='',fnpos=''):
        ...
        locmask=np.logical_and(np.logical_not(self.array.mask[i1:i2,j1:j2]),np.logical_not(np.isnan(newvalues.transpose())))
    ...
    def export_to_gltf(self,bounds=None,fn=''):
        ...
        points = np.asarray([[xx,self.get_value(xx+ox-refx,yy+oy-refy,nullvalue=zmin),-yy]  for xx in x for yy in y],dtype=np.float32)
        ...
        if fn=='':
            fn=self.filename.rpartition('.')[0]+'glb'
    ...
    def paste_all(self,fromarray):
        ...
        i2,j2 = self.get_ij_from_xy(fromarray.origx+fromarray.nbx*fromarray.dx,fromarray.origy+fromarray.nby*fromarray.dy)
    ...
    def get_xy_inside_polygon(self,myvect:vector,usemask=True):
        ...
        if usemask:    
            myij=np.asarray([self.get_ij_from_xy(x,y) for x,y in mypoints])
            mymask = np.logical_not(self.array.mask[myij[:,0],myij[:,1]])
            mypoints=mypoints[np.where(mymask)]
        
        return mypoints

    def get_values_insidepoly(self,myvect:vector,usemask=True):
            ...
            if type(self.cropini) is np.ndarray:
                ...
                while badvalues:
                    ...
                    if ret == wx.ID_CANCEL:
                        ...
                        self.cropini = [[float(newcrop.ox.Value),float(newcrop.ex.Value)],[float(newcrop.oy.Value),float(newcrop.ey.Value)]]  
    ...
    def fillonecellgrid(self,curscale,loci,locj,force=False):
        ...
        if exists==0 or force:
            ...
            try:
                ...
                raise NameError('Opengl in WolfArray_fillonecellgrid -- maybe a conflict with an existing opengl32.dll file - please rename the opengl32.dll in the libs directory and retry')
