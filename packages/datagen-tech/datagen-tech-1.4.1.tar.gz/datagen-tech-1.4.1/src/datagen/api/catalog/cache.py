...
class AssetCache(abc.ABC, Generic[Asset]):
    ...
    def _are_matching(self, asset_attributes: AssetAttributes, query_attributes: dict) -> bool:
        matching = True
        for query_tag_name, query_tag_value in query_attributes.items():
            asset_tag_value, query_tag_value = self._get_attributes_values(
                asset_attributes, query_tag_name, query_tag_value
            )
            if isinstance(query_tag_value, AllOf):
                matching &= all(v in asset_tag_value for v in query_tag_value)
            elif isinstance(query_tag_value, AnyOf):
                matching &= any(v in asset_tag_value for v in query_tag_value)
            elif isinstance(query_tag_value, Exactly):
                matching &= set(asset_tag_value) == set(query_tag_value)
        return matching
