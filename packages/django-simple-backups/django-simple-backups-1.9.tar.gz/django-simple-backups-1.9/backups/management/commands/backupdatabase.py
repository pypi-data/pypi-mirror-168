...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.DATABASES['default']['ENGINE'] == 'django.db.backends.sqlite3':
            ...
            co = f"mysqldump {settings.DATABASES['default']['NAME']} Sun Sep 18 19:51:53 2022|backup.sql -u {settings.DATABASES['default']['USER']} -p'{settings.DATABASES['default']['PASSWORD']}' --no-tablespaces --single-transaction --result-file {backup_file_name}"
            ...
            # upload to dropbox
            if using_gzip:
                client.files_upload(open(f"{backup_file_path}.gz", "rb").read(), database_upload_path)
