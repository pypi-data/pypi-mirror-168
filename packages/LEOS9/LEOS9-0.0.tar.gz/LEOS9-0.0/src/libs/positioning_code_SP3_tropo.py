...
def positioning_code_SP3_tropo_func(gpstime,pIF,xs,ys,zs,clks,tropo_dry,mfunc_tropo,el,elmask):

    idxobs=(~np.isnan(pIF)) & (~np.isnan(xs)) & (~np.isnan(clks)) & (el>=elmask)
    ...
    return xr, yr, zr, clkr, tropo, residual
