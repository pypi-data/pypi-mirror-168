...
def GetEffectiveNoFromAlignment(TargetHits, PrefixFileName, FilterField = -1):
    ...
    if ContentExists:
        ...
        subprocess.call(
            "bash "+BASE_DIR+"/RunClust.sh " + FAFileName + " " + str(StrictThreshold) + " " + ClustFileName, shell=True, stdout=devnull, stderr=devnull)
...
def FilterBlastHits(ClusterId, ClusterHits, Database, ClustersFileName):
    ...
    devnull = open(os.devnull, 'wb')
    subprocess.call(
        "bash "+BASE_DIR+"/RunClust.sh " + ClusterFASTA + " 0.3 " + ClusterSubclustersFileName, shell = True, stdout = devnull, stderr = devnull)
