...
def parameter_completion(params, fnames):
    ...
    for ix,fname in enumerate(fnames):
        ...
        if fname in {"linear:1", "out-lr:1"}:
            if "w" not in p: p["w"] = np.random.rand() * 4 - 2
            if "bias" not in p: p["bias"] = np.random.rand() * 4 - 2
        elif fname == "in-linear:0":
            if "scale" not in p: p["scale"] = 1.0
            if "scale_offset" not in p: p["scale_offset"] = 0.0
            if "w" not in p: p["w"] = np.random.rand() * 4 - 2
            if "bias" not in p: p["bias"] = np.random.rand() * 4 - 2            
            ...
            if "scale_offset" not in p: p["scale_offset"] = 0.0
            if "w" not in p: p["w"] = np.random.rand() * 4 - 2
            if "bias" not in p: p["bias"] = 0            
        elif fname == "in-cat:0":
            if "categories" not in p: p["categories"] = []
            if "bias" not in p: p["bias"] = 0
...
class Model(
    ...
    def copy(self) -> "Model":
        ...
        return Model(
            self._program.copy(), list(self.fnames), params=list(self.params)
        )
    ...
    def _create_sub_model_queries(self):
        def get_expr(elem):
            if elem.arity == 0:
                ...
                return f"{elem.fname}(__x0__)".replace(':1', '')
            return f"{elem.fname}(__x0__, __x1__)".replace(':2', '')
