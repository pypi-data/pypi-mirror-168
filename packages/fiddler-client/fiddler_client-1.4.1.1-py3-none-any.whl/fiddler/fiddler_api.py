...
class FiddlerApi:
    ...
    def publish_event(
        ...
        # To handle the pandas datatype present in the dictionary.
        # Ref: https://fiddlerlabs.atlassian.net/browse/FDL-6478
        # It first converts the dictionary to a dataframe and then convert it back to the dictionary
        # to remove all the pandas datatypes from dictionary
        df = pd.DataFrame.from_records([event])
        if df.shape[0] < 1:
            raise (
                    'Event is empty. Please send a valid event.'
                )
