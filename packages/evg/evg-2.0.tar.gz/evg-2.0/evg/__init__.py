...
if not _Helper.check_directory(path=local_path):
    raise "Необнаружено языковой папки!"
else:
    _directories = _Helper.directories(local_path)
    _languages = []
    if len(_directories) == 0:
    ...
    elif default_language in _directories and _Helper.count_files(f"{local_path}/{default_language}", type=".json") == 0:
        raise "Необнаружено компонентов базового языкового пакета!"
    else:
        for language in _directories:
            if _Helper.count_files(f"{local_path}/{language}", type=".json") == 0:
                continue
        ...
        for language in _languages:
            if language not in languages:
                languages[language] = {}
            categories = _Helper.files(f"{local_path}/{language}", type=".json")
