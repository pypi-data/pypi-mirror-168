...
def _recursive_render(t_base, o_base, name, context, env):
    ...
    if match_list:
        ...
        if key == 'blueprints':
            ...
            out_key = '__model'
            out_values = list(context['models'].values())  # models can be accessed at context level, NOTE: models is dict, {name: {names, schema}}}, so we use values() here
    ...
    #
        ...
        with work_in(o_base):
            # Set jinja2's path
            env.loader = FileSystemLoader(['.', os.path.join(t_base, 'includes')])  # search in current folder or template/includes folder
