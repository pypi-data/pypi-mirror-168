...
class ServiceCommands(Enum):
    ...
    def get(command_name: str):
        return ServiceCommands._member_map_[command_name]


class ServiceRoles(Enum):

    LOG: ServiceRoleValue = ServiceRoleValue("Log api service", CONST.HOST.LOG.NAME, CONST.RPC.PORT(1),
                                             [ServiceCommands.log,
                                                 ServiceCommands.command],
                                             ["telegram-send"])

    DOCS: ServiceRoleValue = ServiceRoleValue("Documents api service", CONST.HOST.DOCS.NAME, CONST.RPC.PORT(1),
                                              ...
                                              ["xlsxwriter", "xlrd", "xlutils", "openpyxl", "python-barcode", "Pillow"])
...
class LogCommands(Enum):
    ...
    SERVICE_STARTED: LogCommand = LogCommand(
        "Сервис {service_name} ({service_description}) запущен!\nИмя хоста: {host_name}\nПорт: {port}\n", LogChannels.SYSTEM, LogLevels.NORMAL.value, (ParamItem("service_name", "Name of service"), ParamItem("service_description", "Description of service"), ParamItem("host_name", "Name of host"), ParamItem("port", "Port"))),
    #
