...
class PIH:
    ...
    def rpc_error_handler(details: str, code: Tuple, command: ServiceCommands) -> None:
        #add to new thread
        if not isinstance(command, ServiceCommands) or (isinstance(command, ServiceCommands) and PIH.SERVICE.get_role_by_command(command) != ServiceRoles.LOG):
    ...
    def log(message: str, channel: LogChannels = LogChannels.DEFAULT, level: Any = LogLevels.DEFAULT) -> Any:
        try:
            ...
            return RPC.call_by_role(ServiceCommands.log, (message, channel.name, level_value))
        except Error as error:
            print("Log send error")
        
    @staticmethod
    def log_command(command: LogCommands, parameters:Tuple = None) -> bool:
        try:
            ...
            return RPC.call_by_role(ServiceCommands.command, (command.name, parameter_dict))
        except Error as error:
            print("Log send error")
