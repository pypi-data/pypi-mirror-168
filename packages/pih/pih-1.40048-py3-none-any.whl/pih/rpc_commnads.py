...
from pih.const import CONST, LogChannels, LogCommands, ServiceCommandNames
...
class RPC_COMMANDS:
    ...
    class DOCS:
        ...
        @staticmethod
        def is_inventory_report(report_file_path: str) -> bool:
            return RPC.call(RPC_COMMANDS.DOCS.create_rpc_command("is_inventory_report"), (report_file_path))

        @staticmethod
        def get_inventory_report(report_file_path: str, open_for_edit: bool = False) -> str:
            return RPC.call(RPC_COMMANDS.DOCS.create_rpc_command("get_inventory_report"), (report_file_path, open_for_edit))

        @staticmethod
        def save_report_item(report_file_path: str, item: InventoryReportItem) -> str:
            return RPC.call(RPC_COMMANDS.DOCS.create_rpc_command("save_inventory_report_item"), (report_file_path, item))

        @staticmethod
        def close_inventory_report(report_file_path: str) -> str:
            return RPC.call(RPC_COMMANDS.DOCS.create_rpc_command("close_inventory_report"), (report_file_path))
    ...
    class LOG:
        ...
        @staticmethod
        def log(message: str, channel: LogChannels, level: int) -> str:
            return RPC.call_by_role(ServiceCommandNames.log, (message, channel.name, level))

        @staticmethod
        def command(command: LogCommands, parameters: dict = None) -> bool:
            return RPC.call_by_role(ServiceCommandNames.command,  (command, parameters))
