...
```py
from dnoticias_tables.datatable import Datatable, Row

class MyCustomDatatable(Datatable):
    id = Row(name="ID", orderable=True, searchable=True)
    email = Row(name="Email", searchable=True, width=150)
...
Used to define a row in the datatable. The variable where the Row class is allocated need to have the same name as the object attribute you want to retrieve. i.e:
...
#### RowMethod
Almost the same as **Row** but this uses a custom method instead of an object attribute.
...
#### RowChoice
And again, almost the same as **Row** but this is used to display a choice field.
The only difference between this class and Row is that this class will receive a choices parameter and will return the choice label
    ...
    "data": [
        {
            ...
            "email_name": "Título",  // Row name defined on view
            "email_visible": true,  // Row visible defined on view
            "email_auto_hide": false,  // Autohide defined on view
            "email_width": 200,  // Row width defined on view
            "email": "example@mail.com",  // Row value for 'email'
            "status_choice_label": "Confirmado",  // Choice label (returned only on RowChoice!)
            "status_name": "Estado",
            "status_visible": true,
