...
class FormativeCheck(CheckData):
    ...
    def regular(self, rule, **kwargs):
        ...
        # 枚举
            ...
            return is_true
        # 整形(浮点)列表，整形列表，URL列表
        elif rule in ['is_int_float_list', "is_int_list", "is_url_list", "is_dict_list"]:
            # noinspection PyBroadException
            try:
                resource = json.loads(self.parameter)
                ...
                if isinstance(resource, list):
                    ...
                    for key in resource:
                        super().__init__(key)
                        if rule == "is_int_float_list" and not self.is_int_or_float:
                            is_true = False
                        if rule == "is_int_list" and not self.is_int:
                            is_true = False
                        if rule == "is_url_list" and not self.is_url:
                            is_true = False
                        if rule == "is_dict_list" and not isinstance(key, dict):
                            is_true = False
    ...
    def regular_logic(self, rule, choices):
        if self.regular(rule, choices=choices) is not False:
            if rule in ["is_int_float_list", "is_int_list", "is_url_list", "is_json", "is_dict_list"]:
