...
# Ustar filtering method: Papale et al. (Biogeosciences, 2006).

# index_col = 0: assuming the 0 column contains date time and use it as the index.
df = pd.read_csv(YOUR_PATH, index_col = 0)
# Assuming the date time format is yyyy-mm-dd HH:MM:SS
df.index = pd.to_datetime(df.index, format = '%Y-%m-%d %H:%M:%S') 
# Air temperature, solar radiation, and ustar must start with TA, SW, and USTAR, respective.
# Otherwise change them using the line below:
df = df.rename(columns = {'Tair': 'TA', 'Rg': 'SW_IN', 'Ustar': 'USTAR'})
# Do not contain any NaNs, drop them:
df = df.dropna()
# RETURN
# oustars: numpy array with 5, 50 and 95 percentile of u* thresholds,
# off: flags, 0 everywhere except set to 2 where u* < u*-threshold.
