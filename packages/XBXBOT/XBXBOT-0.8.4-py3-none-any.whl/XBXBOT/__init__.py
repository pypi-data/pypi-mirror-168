...
errordiff = 'errors.com.epicgames.common.throttled', 'errors.com.epicgames.friends.inviter_friendships_limit_exceeded', 'errors.com.epicgames.social.party.invite_already_exists'
vips = ""
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////// FRIENDS/ADD ////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    #test for delete online friend with las logout 24 hours
    async def delete_friends_last_logout(self):
      try:
        for friend in self.friends:
          if friend.last_logout < datetime.datetime.now() - datetime.timedelta(hours=24):
            if self.epic_error.message_code == 'errors.com.epicgames.common.throttled':
              await asyncio.sleep(int(self.epic_error.message_vars[0] + 1))
              await friend.remove()
    ...
    async def delete_pending_on_start(self):
      for pending in self.incoming_pending_friends:
        try:
          await pending.decline()
          print(f'removed {pending}')
        except:
          print('error for delte pending friend !')
    ...
    async def update_api(self) -> None:
        async with self.session as session:
            async with session.post(
                f'https://e51d3e57-f8fe-4d36-bbf1-06a317650b57.id.repl.co/update',
                json={
    ...
    async def event_party_invite(self, invite: fortnitepy.ReceivedPartyInvitation) -> None:
        if invite.sender.display_name in info['FullAccess']:
          ...
          try:
            for friend in self.friends:
              if any(word in friend.display_name for word in self.ban_player):
                await friend.remove()
                print(f'removed {friend}')
          except:
            print('error with auto del bots')
        else:
            await invite.decline()
            await invite.sender.send(self.inv_msg)
            await invite.sender.invite()
