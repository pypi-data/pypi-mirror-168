...
def compliance_summary(prismasession: PrismaSession, standards: list[str]) -> pd.DataFrame:
    """
...
def compliance_requirements_summary(prismasession: PrismaSession, standard:str) -> pd.DataFrame:
    """
    ...
    for r in requirements:
        reqcomp = prisma.get_standard_requirement_compliance_statistics(prismasession, standard_id, r['id'])
        comp[r['requirementId']] = reqcomp

    return pd.json_normalize(comp)
...
def historic(prismasession: PrismaSession, complianceStandard: str, cloudAccounts: list[str] = None, accountGroups: list[str] = None) -> pd.DataFrame:
    """
