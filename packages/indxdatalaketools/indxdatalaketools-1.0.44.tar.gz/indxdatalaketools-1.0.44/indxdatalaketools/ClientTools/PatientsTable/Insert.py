...
class Client:
    ...
    def __init__(self, google_clients, client_uuid):
        ...
        logging_client                          = google_clients.cloud_logging_client
        self.__patient_insert_logger            = logging_client.logger(client_uuid + '-patient-logger')
    ...
    def __insert_multiple_patients_to_table(self, json_structure, overwrite=False):
        ...
        if status == False:
            self.__patient_insert_logger.log_text(
                'One or more patients have not been uploaded Successfully', severity='ERROR')
    ...
    def __add_data_to_patient_table(self, patient_data):
        ...
        if response_future_1.done():
            self.__patient_insert_logger.log_text('successfully inserted ' + 
                str(patient_data) + ' to PATIENTS table', severity='INFO')
            print('successfully inserted Patient ' + str(patient_data['MRN']) + ' to PATIENTS table')
            return True

        Helpers.print_error('Failed to insert Patient ' + str(patient_data['MRN']) + ' to PATIENTS table')
        self.__patient_insert_logger.log_text('Failed to insert ' + 
                str(patient_data) + ' to PATIENTS table', severity='ERROR')
        return False
    ...
    def __create_proto_buf_row(self, patient_data):
        ...
        try:
            ...
            self.__patient_insert_logger.log_text("error creating protocol buffer " + str(e),
            severity='ERROR')
            return None
