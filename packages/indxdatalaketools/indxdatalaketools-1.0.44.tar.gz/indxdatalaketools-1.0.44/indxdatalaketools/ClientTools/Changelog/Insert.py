...
class Client:
    ...
    def __init__(self, google_clients, client_uuid):
        ...
        self.__changelog_insert_logger          = logging_client.logger(client_uuid + '-changelog-logger')
    ...
    def __insert_data_to_changelog(self, dataset_id, table_name, changelog_data):
        ...
        if response_future_1.done():
            self.__changelog_insert_logger.log_text('Inserted ' + str(changelog_data) + ' to changelog', severity='INFO')
            return True

        self.__changelog_insert_logger.log_text('Failed to insert ' 
            + str(changelog_data) + ' to CHANGELOG table', severity='ERROR')
        return False
