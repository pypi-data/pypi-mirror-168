...
class Client:
    ...
    def __init__(self, google_clients, client_uuid):
        ...
        self.__file_upload_logger       = logging_client.logger(client_uuid + '-file-logger')
    ...
    def upload_file_to_datalake(self, modality, mrn, metadata, file_path):
        ...
        if Helpers.try_blob_exists(blob):
            ...
            self.__file_upload_logger.log_text("File" + blob.name + " already exists", severity='INFO')
            return True
    ...
    def __create_blob_with_metadata(self, blob, file_contents, metadata):
        ...
        if Helpers.try_blob_exists(blob):
            ...
            self.__file_upload_logger.log_text('successfully uploaded file ' + blob.name + ' to datalake', severity='INFO')
            if metadata['MODALITY'] != metadata['CLASSIFIED_MODALITY']:
                self.__file_upload_logger.log_text('Classified Modality for ' 
                    + blob.name + ' (' + metadata['CLASSIFIED_MODALITY'] + 
                    ') does not matched metadata supplied (' + metadata['MODALITY'] + ')', severity='WARNING')
    ...
    def __try_upload_blob(self, blob, contents):
        ...
        try:
            ...
            self.__file_upload_logger.log_text('Failed To Upload Blob ' 
                + blob.name + ' using GCS client: ' + str(e), severity='ERROR')
            return False