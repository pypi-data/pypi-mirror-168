...
class DataStagCommandHandler(DataStagConnection):
    ...
    def handle_command_data(self, command_data: Union[dict, list]) -> Union[Optional[dict], List[Optional[dict]]]:
        """
    ...
    def _handle_delete_multiple(self, command: dict) -> dict:
        """
        Executes a delete multiple command which deletes a list of values matching a given mask
        :param command: The command's parameters as dictionary
        :return: The response as dictionary
        """
        search_masks = command.get(self._SEARCH_MASKS)
        recursive = command.get(self._RECURSIVE, False)
        return self.bundle_return(self.delete_multiple(search_masks, recursive=recursive))
    ...
    def _handle_get_values_by_name(self, command: dict) -> dict:
        ...
        if flat:
            ...
            return {self._DATA: [{"name": element["name"],
                                  "value": self.json_to_data(element["value"])} for element in results]}
    ...
    def _handle_find(self, command: dict) -> dict:
        ...
        recursive = command.get(self._RECURSIVE, False)
        results = self.find(mask=mask, limit=limit, relative_names=relative_names, recursive=recursive)
        return self.bundle_return(results)
