...
class DataStagVault:
    ...
    FOLDER_SEPARATOR = "."  # The sign which defines in which folder a key is located
    ...
    def push(self, name: str, data: List[StagDataTypes], timeout_s: Optional[float] = None, index=-1) -> int:
        """
        ...
        with self.lock:
            list_handle: Optional[DataStagList] = self._get_list_instance(name)
            ...
            return list_handle.add_elements(element_list, index=index, deprecation_time=dep_time)

    def pop(self, name: str, default: StagDataReturnTypes = None, index: int = 0) -> StagDataReturnTypes:
        """
    ...
    def set(self, name: str, data: StagDataTypes, timeout_s: Optional[float] = None) -> bool:
        """
    ...
    def add(self, name: str, value: Union[float, int] = 1, timeout_s: Optional[float] = None, default=0) -> \
            Union[float, int]:
        """
    ...
    def get(self, name: str, default: StagDataReturnTypes = None) -> StagDataReturnTypes:
        """
    ...
    def get_ex(self, name: str, default: StagDataReturnTypes = None, version_counter=-1) -> (int, StagDataReturnTypes):
        """
    ...
    def find(self, mask: str, limit: int = 100, relative_names: bool = False, recursive: bool = False):
        """
        ...
        with self.lock:
            ...
            global_key_list = [self._get_global_name(folder_name, key) for key in key_list]
            if not relative_names:
                key_list = global_key_list
                key_list = [key for key in key_list if self.global_dictionary[key].deprecation_time is None or
                            self.global_dictionary[key].deprecation_time > uptime]
            else:
                key_list = [key for key, g_key in zip(key_list, global_key_list) if
                            self.global_dictionary[g_key].deprecation_time is None or
                            self.global_dictionary[g_key].deprecation_time > uptime]
    ...
    def get_values_by_name(self, mask: str, limit: int = 100, flat: bool = True):
        ...
        with self.lock:
            ...
            if flat:
                results = [self.global_dictionary[name].get_value() for name in names if
                           self.global_dictionary[name].simple]
            else:
                results = [{"name": name, "value": self.global_dictionary[name].get_value()} for name in names if
                           self.global_dictionary[name].simple]
            return results
    ...
    def lelements(self, name: str, start: int, end: Optional[int]) -> List[StagDataTypes]:
        """
    ...
    def _get_element_by_name(self, name: str, deprecation_time: Optional[float] = None) -> Optional[DataStagElement]:
        """
        Tries to retrieve a database element

        :param name: The element's name
        :param deprecation_time: The server uptime in seconds. If provided the element will be deleted if it timed out.
        """
    ...
    def _get_list_instance(self, name: str) -> Optional[DataStagList]:
        """
    ...
    def _remove_from_deprecation_list(self, element: DataStagElement):
        ...
        """
        if element.deprecation_time is not None:
            prev_dep_time_round = self._get_round_deprecation_time(element.deprecation_time)
    ...
    def _update_deprecating_element(self, element: DataStagElement, dep_time: float = None):
        """
        ...
        with self.lock:
            ...
            while self.last_garbage_collection_time < collection_time:
                ...
                while len(elements):
                    ...
                    elements = self.deprecating_elements[cur_time_offset]
                if cur_time_offset in self.deprecating_elements:  # in case the list did not empty itself yet
                    del self.deprecating_elements[cur_time_offset]  # delete key
        return True
