...
def test_shared_archive():
    ...
    # verify file
    assert SharedArchive.verify_file(svg_files[0])
    assert not SharedArchive.verify_file(svg_files[0] + "/someNonsense")
    fake_name = ZIP_SOURCE_PROTOCOL + "@doesNotExist/someFile.txt"
    assert not SharedArchive.verify_file(fake_name)
