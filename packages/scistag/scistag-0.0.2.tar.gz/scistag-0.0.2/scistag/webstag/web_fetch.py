"""
The web_fetch module grants easy access to data in the web via it's web_fetch function, a one-liner to receive a
file via http within a given timeout.
"""
...
class WebCache:
    """
    The WebCache class allows the temporary storage of downloaded files in the temp directory. How long the file
    is rated as "valid" can be passed via (for example) the wbe_fetch function's cache duration parameter.
    """
...
def web_fetch(url: str, timeout_s: float = 10.0, max_cache_age=0.0,
              filename: Optional[str] = None, out_response_details: Optional[Dict] = None,
              all_codes=False) -> Optional[bytes]:
    """
    ...
    :param out_response_details: Dictionary target to retrieve response details such as
    ...
    :param all_codes: Defines if all http return codes shall be accepted. Pass a dictionary to response_details
    for the details.
...
__all__ = ["web_fetch", "WebCache", "FROM_CACHE", "STATUS_CODE", "HEADERS", "STORED_IN_CACHE"]
"Exported symbols"
