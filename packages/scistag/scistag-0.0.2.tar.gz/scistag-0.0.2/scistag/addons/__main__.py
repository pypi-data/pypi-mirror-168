...
class AddonCli:
    ...
    To install or remove an optional SciStag addon please use either the install or remove command and pass the
    addons you want to install or remove. Multiple addons can be comma separated.
    ...
    * python3 -m scistag.addons remove [ADDON_NAME,ADDON_NAME,...]
    ...
    def get_all_addons() -> str:
        ...
        for key, addon in all_addons.items():
            result.append(f"* {key} - {addon[FEATURE_INFO]} ({addon[FEATURE_SIZE] / 2 ** 20:0.1f} MB)")
        return "\n".join(result)
    ...
    def validate_addons(cls, addons: str):
        ...
        :param addons: The addon list as comma separated string as passed by the user
        :return: The list of all valid addons. An empty list if the validation failed.
        """
        addons = addons.split(",")
...
{cls.get_installed_addons()}
        """)
        parser.add_argument("addons", choices=AddonManager.get_installed_addons().keys())
        params = parser.parse_args(arguments)
        addons = cls.validate_addons(params.addons)
        if len(addons) == 0:
            sys.exit(1)
