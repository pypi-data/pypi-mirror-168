...
ADDON_DATA_FILENAME = os.path.normpath(os.path.dirname(__file__) + "/../data/addon_packages.json")
...
"The shared identifier heading with which all shared zip archives in FileStag start with"
ADDON_FS_BASE_PATH = scistag.filestag.shared_archive.ZIP_SOURCE_PROTOCOL + f"@{FS_IDENTIFIER_HEADING}"
"The path under which the addon's data can be accessed using FileStag.load_file and FileStag.exists"
...
class AddonManager:
    ...
    def get_all_addons(cls):
        """
        Returns a list of all addon packages and their respective unique identifier. This identifier can then be
        used by install_addon and remove_addon to install or remove a feature.
    ...
    def get_installed_addons(cls):
        """
        Returns a list of all installed addon packages and their respective unique identifier. This identifier can then
        be used by install_addon and remove_addon to install or remove a feature.
    ...
    def get_addon_healthy(cls, feature_name) -> (bool, str):
        ...
        with cls.access_lock:
            ...
            if filesize != feature[FEATURE_SIZE]:
                return False, f'File size mismatch/ "{local_path}" should have the size {feature[FEATURE_SIZE]}"' \
                              f' but has the size {filesize}'
            with open(local_path, 'rb') as file_handle:
                md5_hash = hashlib.md5(file_handle.read()).hexdigest()
            if md5_hash != feature[FEATURE_MD5]:
                return False, f'MD% checksum mismatch/ "{local_path}" should have the MD5 checksum {feature[FEATURE_MD5]}"' \
    ...
    def install_addon(cls, feature_name, verbose_if_installed=True) -> bool:
        ...
        :param verbose_if_installed: Defines if no message shall be printed if the addon is already installed
        :return:
        """
        with cls.access_lock:
            ...
            print(f"\nInstalling addon {feature_name} from {main_url} ({mb_size:0.1f} MB)...", flush=True)
            from scistag.webstag import web_fetch
            web_fetch(url=main_url, filename=local_path, timeout_s=int(2 + mb_size * MAX_S_PER_MB))
    ...
    def remove_addon(cls, feature_name) -> bool:
        ...
        with cls.access_lock:
            ...
            if FEATURE_FS_PATH in cls.all_addons[feature_name]:  # if the addon was loaded, unload it
            ...
            if os.path.exists(local_path):
                print(f"Error uninstalling {feature_name}. Data could not be removed.", flush=True)
                return False
            else:
    ...
    def get_addons_paths(cls, filter_mask: str) -> Dict:
        ...
        :param filter_mask: The addons to search for. Can contains * and ? placeholders.
        :return: A dictionary of the format str: str for every matched addon and it's corresponding FileStag path,
        e.g. `{"emojis.512": "localzip://@addons.emojis.512/"}`
        """
        with cls.access_lock:
            addons = cls.get_all_addons()
            valid_addons = set([key for key in addons.keys() if fnmatch(key, filter_mask)])
            result = {}
            for addon_name in valid_addons:
                ...
                if len(addon.get(FEATURE_FS_PATH, "")) > 0:
                    continue
                scistag.filestag.shared_archive.SharedArchive.register(source=cls.get_local_path(addon_name),
