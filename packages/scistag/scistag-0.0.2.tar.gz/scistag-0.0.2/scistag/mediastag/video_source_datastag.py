...
class VideoSourceDataStag(VideoSource):
    ...
    def __init__(self, connection: Optional["DataStagConnection"], data_path: str):
        """
        Initializer
        :param connection: The connection from which the image is received. The local connection by default
        :param data_path: The data path within the connection
        """
        super().__init__()
        from scistag.datastag.data_stag_connection import DataStagConnection
        self.connection: DataStagConnection = connection if connection else DataStagConnection(local=True)
    ...
    def _get_image_int(self, timestamp: Optional[float] = None) -> Tuple[float, Optional[Image]]:
        ...
        new_timestamp, new_data = self.connection.get_ts_modified(self.data_path, timestamp=timestamp)
        if new_data is None:
