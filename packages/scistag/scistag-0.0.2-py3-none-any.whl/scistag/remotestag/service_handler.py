...
class RemoteServiceHandler:
    ...
    def register_service(self, service: RemoteService) -> bool:
        ...
        with self._lock:
            if self._started:
                raise Exception("Can not register new services once the service handler was started.")
    ...
    def execute_async(self, identifier: str, parameters: RemoteParameterTypes, timeout_s: float = -1.0) -> RemoteTask:
        """
        ...
        with self._lock:
            ...
            task = RemoteTask(new_id, service=service_found, target_function=identifier, parameters=parameters,
                              timeout_s=timeout_s)
    ...
    def get_task(self, identifier_set) -> Optional[RemoteTask]:
        """
