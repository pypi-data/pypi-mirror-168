...
class RemoteTask:
    ...
    def __init__(self, task_id: RemoteTaskId, service: RemoteService, target_function: str, parameters: dict,
                 timeout_s: float = -1.0):
        """
        Initializer
        ...
        :param timeout_s: The timeout in seconds after which this task automatically gets cancelled.
        """
    ...
    def unwrap(self) -> Optional[RemoteReturnTypes]:
        """
        Unwraps the result dictionary to a single value if it just contains a single value
        :return: A dictionary in case of multiple return values otherwise the single value
        """
        with self._access_lock:
            if self._result is None:
                return None
