...
class Session:
    ...
    def __init__(self, config: dict) -> None:
        ...
        """
        self.lock = RLock()  # Access lock
        self.unloaded = False  # Set to true when the app is going to be destroyed and should not be used anymore
        self.app: Optional["scistag.slidestag.slide_application.SlideApp"] = config.get("app", None)
        if "app" in config:
            del config["app"]
        assert Session.SESSION_ID in config
        self.session_id = config['sessionId']  # The unique id
        self.guest_id = None  # Id for guest access
        self.guest_data: Optional[bytes] = None
        "Contains the previous image cached for a guest viewer"
        self._config = config  # the configuration
        self.last_interaction = 0  # Last interaction to manage garbage collection
        self.session_timeout = 0.0  # No timeout
        self.handle_used()
        self.user_data_config = {}  # registered user data elements
        ...
        self.data_connection = DataStagConnection()  # local connection
    ...
    def handle_user_data(self, user_data_name: str, data: bytes):
        """
        Requests if the user data shall be handled manually. Otherwise it's stored in
        {rootPath}.userData
    ...
    def set_user_data(self, user_data_name: str, data: bytes) -> None:
        ...
        if self.handle_user_data(user_data_name, data):
            return
        self.data_connection.set_ts(f"{self.user_data_root_path}{user_data_name}", data=data,
    ...
    def handle_unload(self):
        ...
        self.data_connection.delete_multiple(
            [self.session_data_root_path + "*"])  # delete all old data related to our session
    ...
    def get_guest_data(self) -> Optional[bytes]:
        """
