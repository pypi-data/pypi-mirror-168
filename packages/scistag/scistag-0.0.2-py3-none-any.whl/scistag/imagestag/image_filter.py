...
from scistag.imagestag.image import Image, IMAGE_SOURCE_TYPES, PixelFormat
...
class ImageFilter:
    ...
    def __init__(self):
        ...
        """
        self.required_format: Optional[str] = None
    ...
    def filter(self, input_image: Union[IMAGE_SOURCE_TYPES, dict]) -> Union[IMAGE_SOURCE_TYPES, dict]:
        """
        ...
        if self.required_format is not None:
            ...
            if self.required_format == PixelFormat.GRAY and not is_gray:
                input_image[IMAGE_FILTER_IMAGE] = Image(input_image[IMAGE_FILTER_IMAGE].get_pixels_gray(),
                                                        framework=ImsFramework.RAW)
            elif (self.required_format == PixelFormat.RGB or self.required_format == PixelFormat.RGBA) and \
                    input_image[IMAGE_FILTER_IMAGE].pixel_format == PixelFormat.GRAY:
                input_image[IMAGE_FILTER_IMAGE] = Image(input_image[IMAGE_FILTER_IMAGE].get_pixels_rgb(),
                                                        framework=ImsFramework.RAW)
        # execute filter
        result = self._apply_filter(input_image)
        result[IMAGE_FILTER_NAME] = self.name
        # convert to original input format
        if isinstance(original_input, dict):  # if a dictionary was passed in return all details
            return result
        else:
...
__all__ = ["ImageFilter", "IMAGE_FILTER_IMAGE", "IMAGE_FILTER_ORIGINAL_IMAGE", "IMAGE_FILTER_DATA",
