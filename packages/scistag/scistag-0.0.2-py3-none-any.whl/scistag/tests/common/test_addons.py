"""
Tests the addon installation and removal and the integrity of the download URLs and the archive's content
"""
import pytest
import os
from scistag.filestag import FileStag
from scistag.addons import AddonManager
from scistag.addons.addon_manager import FEATURE_TEST, FEATURE_SIZE, FEATURE_INFO, FEATURE_MD5, GROUP_INFO
from scistag.addons.addon_manager import FEATURE_LOCAL_FILENAME, FEATURE_REMOTE_FILENAMES, GROUP_IGNORE_SET
from scistag.common.logger import log_info
...
def test_install_and_removal():
    ...
    for key, feature in AddonManager.get_all_addons().items():
        ...
        if max_size != 0 and feature[FEATURE_SIZE] > max_size:
            continue
        if AddonManager.get_addon_installed(key):  # remove addons we want to install ourselves
    ...
    for key, feature in AddonManager.get_all_addons().items():
        if not feature.get(FEATURE_TEST, False) and not intense_test:
            continue
        if max_size != 0 and feature[FEATURE_SIZE] > max_size:
            log_info(f"Feature {key} exceeds maximum test size of {max_size} bytes, skipping download test.")
            continue
...
def test_addon_access():
    ...
    test_file = FileStag.load_file(svg_path + "images/noto/emojis/svg/emoji_u00a9.svg")
