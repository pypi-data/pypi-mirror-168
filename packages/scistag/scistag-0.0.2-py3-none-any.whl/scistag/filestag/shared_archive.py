...
class SharedArchive:
    ...
    def verify_file(cls, identifier: str, filename: Optional[str] = None) -> bool:
        ...
        """
        archive: Optional[SharedArchive] = None
    ...
    def load_file(cls, identifier: str, filename: Optional[str] = None) -> Optional[bytes]:
        ...
        :param identifier: The archive identifier. Alternate: a full identifier in the form
        localzip://@identifier/filename or a path to a zipfile, e.g. localzip://filename.zip/filename
        :param filename: The name of the file to load.
    ...
    @classmethod
    def unload(cls, filename: Optional[str] = None, identifier: Optional[str] = None) -> bool:
        """
        Unloads a zip file, e.g. if it's uninstalled

        :param filename: The zip file to be removed
        :param identifier: The identifier of the archive to unload
        :return: True if an archive with given filename could be found and removed
        """
        with cls.access_lock:
    ...
    @staticmethod
    def load_file_from_zip_direct(zip_filename, filename) -> Optional[bytes]:
        """
    ...
    def _split_identifier_and_filename(cls, identifier) -> (str, str):
        ...
        if not identifier.startswith("@"):
            ...
            if len(identifier_elements) < 2:
                raise ValueError("You need to pass the zip's filename followed by a slash and the name of the"
