class EasyPackModule:
    ...
    def get_module_info_file(self):
        ...
        module_info_script += "\treturn '" + self.description + "' \n\n\n"
        module_info_script += "def __package_data__():\n"
        module_info_script += "\treturn " + str(self.package_data) + " \n\n\n"
    ...
    @staticmethod
    def scaffold(folder='.', binaries_only=False):
        import os
    ...
    def read(folder):
        ...
        if os.path.exists(folder + "/__info__.py"):
            ...
            if "__package_data__" in content:
                module_info.package_data = info.__package_data__()
            if must_remove:
    ...
    def get_setup(self):
        ...
        if self.package_data:
            setup_py += ",package_data=" + str(self.package_data) + ""
    ...
    def build_module(self, dst, options = "sdist"):
        import os
        ...
        import subprocess
        p = subprocess.Popen(["python3", "setup.py", options], cwd=destination)
        p.wait()
        self.build_number += 1
        return destination
