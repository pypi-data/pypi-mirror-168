...
class SQLAlCheAccessor:
    ...
    def generate_sql_format(self, table_name: str, action: str = 'replace', colums: Optional[list] = None, filter: Optional[list] = None, add_syntax: bool = False):
        ...
        if action in ['replace', 'append']:
            ...
            return f"update {table_name} set {', '.join(filed_format1.format(k, k) for k in colums)} where {', '.join(filed_format1.format(k, k) for k in filter)}"
        elif action == 'delete':
            return f"delete from {table_name} where {', '.join(filed_format1.format(k, k) for k in filter)}"
