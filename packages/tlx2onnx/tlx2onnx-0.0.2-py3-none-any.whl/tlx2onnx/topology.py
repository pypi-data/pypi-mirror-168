...
def memory_node_info(node):
    ...
    if node.layer.__class__.__name__ in tlx.nn.inputs.__all__:
        ...
        node_info['in_dtype'] = tlx.convert_to_numpy(node.out_tensors[0]).dtype
        node_info['out_dtype'] = tlx.convert_to_numpy(node.out_tensors[0]).dtype
        ...
        node_info['in_dtype'] = tlx.convert_to_numpy(node.in_tensors[0]).dtype
        node_info['out_dtype'] = tlx.convert_to_numpy(node.out_tensors[0]).dtype
        node_info['attr'] = node.attr
    return node_info
