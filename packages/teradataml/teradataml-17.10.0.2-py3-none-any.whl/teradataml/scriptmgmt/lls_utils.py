...
def list_base_envs():
    ...
    """
    try:
        response = requests.get(_get_ues_url("base_environments"), headers=_get_auth_token())
...
def list_user_envs(env_name=None, **kwargs):
    ...
    try:
        response = requests.get(_get_ues_url(), headers=_get_auth_token())
...
def create_env(env_name, base_env, desc=None):
    ...
    try:
        ...
        response = requests.post(_get_ues_url(), headers=_get_auth_token(), json=data)
...
def remove_env(env_name):
    ...
    try:
        response = requests.delete(_get_ues_url(env_name=env_name), headers=_get_auth_token())
