#!/usr/bin/env python
# coding: utf-8
# Objective : to help calculating the incentive given for each goodaid drug_id, this script helps assign incentive to
# the drug_id's for all days within the start date and end date.
import os
import sys
import argparse
...
parser.add_argument('-et', '--email_to', default="sanjay.bohra@zeno.health", type=str, required=False)
...
logger.info("Base ratecard read with length {}".format(len(data)))
logger.info("Unique drug-id count {}".format(data['drug-id'].nunique()))

# Explode
# This step splits every drug-id, incentive for each day between incentive start date and incentive end date
...
logger.info("Exploded ratecard read with length {}".format(len(data_export)))
logger.info("Unique drug-id count {}".format(data_export['drug-id'].nunique()))
