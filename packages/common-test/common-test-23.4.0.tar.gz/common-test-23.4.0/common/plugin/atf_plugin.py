        ...
        if get_system_key(Constant.RUN_TYPE) is not None:
            ...
            if _total > 0 and _passed > 0:
                ...
                if get_system_key(Constant.ISSUE_KEY) is not None and get_system_key(Constant.ISSUE_KEY).strip() !='':
                    ...
                    if get_system_key('TestPlanId') is not None:
                        logger.info("开始同步Jira测试用例")
                        temp =FilePlugin.load_json(f'{report_html_path}/data/suites.json')
                        xpath = "$..children[?(@.status)].[name,status]"
                        testcase = jsonpath(temp, xpath)
                        caseName = testcase[::2]
                        caseStatus = testcase[1::2]
                        cycleCases = JiraPlatForm.getReclyTestCase(get_system_key('TestPlanId'), get_system_key("CycleName"))
                        logger.info("测试计划："+get_system_key('TestPlanId'))
                        ...
                        for i in range(0, len(caseName)):
                            JiraPlatForm.updatecaseByrunId(cycleCases,testcase[i], caseStatus[i])
...
if __name__ == '__main__':
    ...
    DataBus.set_key('orderInfoCode',"3333")
    print(DataBus.get_data('{"orderInfoCode": ${orderInfoCode}, "signImage": "388334"}'))
