...
async def make_call(dm_cls: type, context: Context, message: Message, topic: str):
    with statefun_context_var(context), statefun_message_var(message):
        ...
        if sub_cls_name := function_call.get('cls_name', None):
            sub_cls = ModelRegistry.get_cls_from_name(sub_cls_name)
            if issubclass(sub_cls, dm_cls):
                dm_cls = sub_cls
            else:
                raise TypeError
...
class StatefunProxy:
    ...
    def _get_invocation_value(self, f_name, _kwargs):
        ...
        res = dict(
            ...
            cls_name=self.wrapped.__name__
        )
        return res
