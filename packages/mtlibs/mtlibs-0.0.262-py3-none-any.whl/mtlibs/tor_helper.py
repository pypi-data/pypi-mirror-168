...
class TorProc():
    ...
    def start(self,
        ...
        torcc_content = []
        torcc_content.append(f"SocksPort {socks_port}")
        torcc_content.append("DataDirectory \"{}\"".format(self.tmpdirname))
        torcc_content.append("ControlPort {}".format(controll_port))
        torcc_content.append("CookieAuthentication 1")
        torcc_content.append("VirtualAddrNetworkIPv4 10.192.0.0/10")
        torcc_content.append("AutomapHostsOnResolve 1")
        torcc_content.append('TransPort 0.0.0.0:9040')
        torcc_content.append('DNSPort 0.0.0.0:5353')
        if EXTERNAL_PROXY_HOST and EXTERNAL_PROXY_PORT:
            torcc_content.append('Socks5Proxy {EXTERNAL_PROXY_HOST}:{EXTERNAL_PROXY_PORT}')
        # tor 使用外部代理
        ...
        if self.hiddenservice_ports:
            ...
            torcc_content.append("HiddenServiceDir \"{}\"".format(self.hidden_dir))

            for port in self.hiddenservice_ports:
                torcc_content.append("HiddenServicePort \"{}\"".format(port))
        ...
        torcc_content.append(
            "HashedControlPassword 16:E600ADC1B52C80BB6022A0E999A7734571A451EB6AE50FED489B72E3DF"
        )
        
        torcc_path = "/tmp/torcc_tor_helper"
