...
def start_clash():
    """启动clash"""
    # config_file = os.path.join(os.getcwd(), "data/clash/config-tproxy.yml")
    
    logger.info(f"当前脚本路径:{__file__}")
    
    package_dir = str(Path(__file__).parent)
    clash_config_path = str(Path(__file__).parent.parent.joinpath("data/clash/config-tproxy.yml"))
    logger.info(f"当前脚本文件夹路径路径:{package_dir}")
