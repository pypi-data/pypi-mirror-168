...
class TorProc():
    ...
    def start(self,
        ...
        logger.info(f"更改目录owner {self.tmpdirname}, {toruser.pw_uid}, {toruser.pw_gid}")
        os.chown(self.tmpdirname, toruser.pw_uid, toruser.pw_gid)

        torcc_lines = []
        torcc_lines.append(f"SocksPort {socks_port}")
        torcc_lines.append("DataDirectory \"{}\"".format(self.tmpdirname))
        torcc_lines.append("ControlPort {}".format(controll_port))
        ...
        if EXTERNAL_PROXY_HOST and EXTERNAL_PROXY_PORT:
            torcc_lines.append('Socks5Proxy {EXTERNAL_PROXY_HOST}:{EXTERNAL_PROXY_PORT}')
        # tor 使用外部代理
