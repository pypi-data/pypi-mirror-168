...
class InstallSuite(WritableBaseModel):
    ...
    def get_missing_products(self, ):
        ...
        # TODO hardware atarget check?
        if not self.ocr.java_lib and self.secrets.OCR_LICENSE:
            missing.append(Software.spark_ocr)
        if not self.hc.java_lib and self.secrets.HC_LICENSE:
            missing.append(Software.spark_hc)
        if not self.nlp.java_lib:
