...
def start(
        ...
        # But then we have hard dependency on jsl_home directory, force user to have one? --> Nah?
        # -- JSL-Auth Flows --
        # Browser Auth
    ...
    if suite.secrets:
        if suite.secrets.HC_LICENSE and not exclude_healthcare and Software.spark_hc.check_installed(None) \
        ...
        if suite.secrets.OCR_LICENSE and not exclude_ocr and Software.spark_ocr.check_installed(None) \
    ...
    if suite.secrets and suite.secrets.HC_LICENSE and exist_in_jvm(
    ...
    if suite.secrets and suite.secrets.OCR_SECRET and exist_in_jvm(
    ...
    if not already_launched:
        print(
            f'👌 Launched {Fore.LIGHTGREEN_EX + hardware_target.value}-Optimized JVM{Fore.RESET} with SparkSession with Jars for: {", ".join(launched_products)}')
...
def exist_in_jvm(java_class):
    ...
    #     print("FUUUUCK BROOOO JARS ARE MISISNGUUU")

    if UserHelpAutoCompletion.KEY in dir(java_obj):
        return False
        # bck = '\n'
        # print(f"🚨 It looks like the Java Class {java_class} is currenlty not present int the JVM!"
        #       f"Currently loaded Jars are: \n",
