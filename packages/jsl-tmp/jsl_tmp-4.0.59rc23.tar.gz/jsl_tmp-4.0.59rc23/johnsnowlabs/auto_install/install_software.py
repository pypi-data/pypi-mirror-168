    ...
    if len(install_results) > 0:
        ...
        for installed_software, result in install_results.items():
            if installed_software.check_installed(python_exec_path=python_exec_path, download_folder=py_dir):
                print(f'{installed_software.logo} {installed_software.name}=={installed_software.get_installed_version()} installed! ✅ {installed_software.slogan} ')
            else:
                print(f'{installed_software.logo} {installed_software.name} not installed! ❌')

        # TODO colorama?
        print(
            f'🔁{bcolors.FAIL} If you are on Google Colab, please restart your Notebook for changes to take effect {bcolors.ENDC}🔁')
    else:
        print(f'👌 Everything is already installed, no changes made')


def finalize_zip_folder(offline_zip_dir, java_dir, license_dir):
    copy_jars_from_jsl_home_to_offline_zip_dir(java_dir)
    copy_licenses_from_jsl_home_to_zip_dir(license_dir)
