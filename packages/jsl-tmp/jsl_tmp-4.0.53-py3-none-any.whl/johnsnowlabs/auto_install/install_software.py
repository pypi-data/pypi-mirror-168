...
def check_and_install_dependencies(product: AbstractSoftwareProduct,
    ...
    while hard_nodes or licensed_nodes_left_to_install() or optional_nodes_left_to_install():
        ...
        v, v_type = (hard_nodes.pop(), 'H') if hard_nodes \
            else (licensed_nodes.pop(), 'L') if licensed_nodes_left_to_install() \
            else (optional_nodes.pop(), 'O') if optional_nodes_left_to_install() \
            else 0
        ...
        if not v.pypi_name:
            ...
            continue
        if v_type == 'L' and not install_licensed:
            continue
        if v_type == 'O' and not install_optional:
            continue
        if v.check_installed(python_exec_path):
        ...
        install_results[v] = \
            v.install(secrets, py_path=python_exec_path) if v.licensed else v.install(py_path=python_exec_path)
    ...
    if not get_pip_lib_version('spark-nlp').equals(LatestCompatibleProductVersion.spark_nlp.value):
        ...
        install_results[Software.spark_nlp] = \
            Software.spark_nlp.install(re_install=True, version=LatestCompatibleProductVersion.spark_nlp.value)
    if not get_pip_lib_version('pyspark').equals(LatestCompatibleProductVersion.pyspark.value):
        # Re-install NLP incase some other library up/downgraded it while we installed it
        install_results[Software.spark_nlp] = \
            Software.pyspark.install(re_install=True, version=LatestCompatibleProductVersion.pyspark.value)
    ...
    if len(install_results) > 0:
        ...
        for installed_software, result in install_results.items():
            if installed_software.check_installed(python_exec_path):
