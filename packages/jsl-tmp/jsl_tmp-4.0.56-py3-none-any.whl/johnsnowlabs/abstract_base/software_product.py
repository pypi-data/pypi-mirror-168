...
class AbstractSoftwareProduct(ABC):
    ...
    @classmethod
    def check_installed(cls, python_exec_path: str = sys.executable, download_folder: str = None) -> bool:
    ...
    def install(cls,
        ...
        if cls.pypi_name:
            if cls.is_py4j:
                ...
                if cls.name == ProductName.hc.value and suite.hc:
                ...
                elif cls.name == ProductName.ocr.value and suite.ocr:
                    return install_standard_pypi_lib(f'{settings.py_dir}/{suite.ocr.py_lib.file_name}',
                                                     ...
                                                     download_folder=download_folder,
                                                     include_dependencies=include_dependencies, )
                elif cls.name == ProductName.nlp.value and suite.nlp:
                    return install_standard_pypi_lib(f'{settings.py_dir}/{suite.nlp.py_lib.file_name}',
                                                     cls.py_module_name,
                                                     python_path=py_path, upgrade=upgrade, re_install=re_install,
                                                     version=version, download_folder=download_folder,
                                                     include_dependencies=include_dependencies, )
