...
class InequalityMixer(function_params.FunctionParams):
    ...
    def _create_io_names(self) -> None:
        ...
        for i in range(self.data_reg_input.size):
            self._input_names += [f"{DATA_REG_INPUT_NAME}_{i}"]

        for i in range(self.data_reg_input.size):
            self._output_names += [f"{DATA_REG_OUTPUT_NAME}_{i}"]

        if isinstance(self.bound_reg_input, RegisterUserInput):
            for i in range(self.bound_reg_input.size):
                self._input_names += [f"{BOUND_REG_INPUT_NAME}_{i}"]

            for i in range(self.bound_reg_input.size):
                self._output_names += [f"{BOUND_REG_OUTPUT_NAME}_{i}"]
