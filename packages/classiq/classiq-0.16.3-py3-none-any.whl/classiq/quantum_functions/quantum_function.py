    ...
    def add_implementation(
        ...
        if self._function_data is None:
            self._function_data = FunctionData(
                ...
                interface=FunctionInterfaceData.from_registers_dict(
                    regs_dict=registers
                ),
            )
        else:
            self._function_data.interface.validate_equal_interfaces(
                FunctionInterfaceData.from_registers_dict(regs_dict=registers)
            )
            ...
            implementation.validate_ranges_of_all_registers(
                self._function_data.interface
            )
            self._function_data.implementations = new_implementations
