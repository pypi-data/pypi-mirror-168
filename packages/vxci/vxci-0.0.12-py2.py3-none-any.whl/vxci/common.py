...
from vxci.get_addons import get_module_list, get_modules2test
...
def test_module(config):
    ...
    if config.get('language', False):
        extra += ' --load-language={lang}'.format(lang=config.get('language'))
    tags = ",/".join(modules2test)
    extra_tags = config.get('odoo_test_tags')
    if extra_tags:
        tags += ',%s' % extra_tags
    extra += ' --test-tags=/{tags}'.format(tags=tags)
    _logger.debug("test tags: %s", tags)
    test_command = (
...
def resume_log(log_lines, allow_deprecated=False):
    ...
    def warnings(line):
        ...
        return criteria.match(line) \
            ...
            and (' py.warnings: ' in line and '/extra_addons/' in line)  # silent py.warnings raised from odoo core
