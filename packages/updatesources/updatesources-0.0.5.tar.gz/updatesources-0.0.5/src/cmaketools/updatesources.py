...
def _interpret_input_file(file_contents: List[str], search_root):
    ...
    for line_string in file_contents:
        ...
        if line.is_project_header:
            ...
            if path.is_file():
                file_path = str(path)
                relative_path = PathResolver.make_relative(file_path, search_root)
                ...
                relative_paths = [PathResolver.make_relative(file_path, search_root) for file_path in file_list]
