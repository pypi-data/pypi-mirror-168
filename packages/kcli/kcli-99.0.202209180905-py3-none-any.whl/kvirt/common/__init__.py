  ...
  lock_passwd: false\n""".format(vmuser=vmuser)
    ...
    if os.path.exists(os.path.expanduser('~/.kcli/cloudinit.yml')):
        with open(os.path.expanduser('~/.kcli/cloudinit.yml')) as f:
            ...
            userdata = yaml.dump(oridata, default_flow_style=False, indent=2)
...
def find_ignition_files(role, cluster):
    ...
    if container_mode():
        oldclusterpath = f"/workdir/clusters/{cluster}/{role}.ign"
        rolepath = f"/workdir/{cluster}/{role}.ign"
    else:
        oldclusterpath = f"clusters/{cluster}/{role}.ign"
        rolepath = f"{cluster}/{role}.ign"
...
def kube_create_app(config, appdir, overrides={}, outputdir=None):
    appdata = {'cluster': 'testk', 'domain': 'karmalabs.local', 'masters': 1, 'workers': 0}
