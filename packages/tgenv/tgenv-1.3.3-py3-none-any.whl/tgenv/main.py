...
@click.pass_context
@click.option("-c", "--count", type=click.INT, default=10, help=config["TEXT"]["REMOTE_VERSION_COUNT"])
    ...
    if re.match(r"v+[0-9]+\.[0-9]+\.+[0-9]*", version):
        if not has_version(version_file_path, version):
            try:
                ...
                get_release_asset(config["DEFAULT"]["TG_REPOSITORY"], version, versions_path, GITHUB_TOKEN)
...
def use(ctx, version: str):
    ...
    if re.match(r"v+[0-9]+\.[0-9]+\.+[0-9]*", version):
        if has_version(version_file_path, version):
            ...
            logger.info("No local version %s present", version)
    else:
        logger.error("Not a valid version")
