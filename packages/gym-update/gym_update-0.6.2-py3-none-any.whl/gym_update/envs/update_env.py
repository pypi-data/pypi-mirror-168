...
#take an action with the environment
  def step(self, action):
    ...
    #pat_e0= np.hstack([np.ones((self.size, 1)), np.random.uniform(-4, 4, size=(self.size,2))]) #shape (size, 3), (1, Xs, Xa)
...
#reset state and horizon    
  def reset(self, seed = None, return_info: bool = False, options: Optional[dict] = None,):  
    ...
    self.patients = truncnorm.rvs(a=0, b= 100,size=(self.size,2)) #shape (size, 2), 1st columns is Xs, second is Xa                        
