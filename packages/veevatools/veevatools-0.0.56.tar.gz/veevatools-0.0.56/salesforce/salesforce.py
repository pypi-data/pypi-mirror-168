...
#                     return pd.DataFrame(columns=results)
    ...
    ## TODO: This function can be made much more efficient by doing all of the object, permission, and profile metadata retrievals in one go
    async def field_permission_user_bulk_check(self, objects: list, permissions: list,  users= [], ):
        data = {}
        task_dict = {}
        for object in objects:
            for permission in permissions:
                task_dict[f"{object};;{permission}"] = asyncio.create_task(self.field_permission_user_check(object, permission, users))
        
        gathered_tasks = await asyncio.gather(*task_dict.values())
        
        task_results_dict = dict(zip(task_dict.keys(), gathered_tasks))
        
        
        result = []
        
        for key, value in task_results_dict.items():
            object, permission = key.split(';;')
            value['Object API Name'] = object
            value['Permission'] = permission
            result.append(value)

        result_pd = pd.concat(result, ignore_index=True)
        result_pd.columns = ['UserName','Profile','Field API Name','Permission Set','Object API Name','Permission']
        return result_pd
    ...
    async def permissionable_fields_bulk_check(self, object_list):
        ...
        for object in object_list:
            permissionable_fields[object] = task_result_dict[object]['fields'].T[task_result_dict[object]['fields'].T['permissionable'] == True].index.tolist()
    ...
    async def field_permission_user_check(self, object, permission, users=[]):
        ...
        if (len(data) > 0):
                ...
                profile_metadata_pd = profile_metadata_pd[profile_metadata_pd['Object API Name'] == object].copy()
                profile_metadata_pd.drop(columns=['field', 'Object API Name'], inplace=True)
                ...
                permission_set_violations = data[data['PermissionSet.IsOwnedByProfile']== False][['User.Username', 'PermissionSet.Label', 'Field API Name']]
        
                return pd.merge(profile_violations, permission_set_violations, on=['User.Username','Field API Name'], how='outer')
        else:
                return pd.DataFrame(columns=['User.Username', 'Profile.Name', 'Field API Name', 'PermissionSet.Label'])
