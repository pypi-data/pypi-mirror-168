...
FONT_PATH = path.join(PATH_DIR, "fonts/")
...
def write_icon(text, size=80, bgcolor="white", fontcolor="black"):
    ...
    font_size = 2
    font_family = f"{FONT_PATH}OpenSans-Bold.ttf"
    width_ratio = 0.9
    font_size = find_font_size(text, font_family, image, width_ratio)
    font = ImageFont.truetype(font_family, font_size)
    draw.text((size / 2, size / 2), text, font=font, fill=fontcolor, anchor="mm", align="center")
    return image
