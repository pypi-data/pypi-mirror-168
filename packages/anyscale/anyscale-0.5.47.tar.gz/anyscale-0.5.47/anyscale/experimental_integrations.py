...
def wandb_send_run_info_hook(run: Any) -> None:
    ...
    # `trial_log_path` is only set as a run.config attribute after Ray 2.0.0
    if "trial_log_path" in run.config:
        print(
            "Please set `run_config=air.RunConfig(log_to_file=True)` to view logs "
            "at the following paths within the Anyscale cluster"
            f"{f' at {run.config.anyscale_logs}' if 'anyscale_logs' in run.config else ''}."
        )
        # We currently print a log path that can be accessed within a Ray cluster, but can update this
        # as we increase the functionality of the dashboard log viewer in Anyscale
        print(
            f"Standard out log: {run.config.trial_log_path}/stdout\nStandard error log: {run.config.trial_log_path}/stderr"
        )
