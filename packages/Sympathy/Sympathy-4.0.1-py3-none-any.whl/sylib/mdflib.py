...
def zeroterm(string, env):
    if isinstance(string, str):
        string = string.encode(env[MdfFile.encoding])
...
class Block():
    ...
    def before_write(self, env):
        pass

    def write(self, fsock, env):
        self.before_write(env)
            ...
            if env[Identification.version] >= ver:
                for i in range(len(form)):
                    ...
                    if isinstance(field, str):
                        field = field.encode(env[MdfFile.encoding])
...
class Identification(Block):
    ...
    def get_format(self):
        return [(000, [], lambda: ()),
                ...
                (330, self._format_330, self.get_init_330)]
...
class DataBlock():
    ...
    def read_data_records(self, cgblock, fsock, env):
        ...
        for cnblock in channels:
            ...
            # Excluding the case of multiple 1-bit signals packed in a single
            # byte.
...
class Text(Block):
    ...
    def before_write(self, env):
        self.text = zeroterm(self.text, env)
...
class Program(Block):
    ...
    def before_write(self, env):
        self.program_specific_data = zeroterm(self.program_specific_data, env)
...
class Channel(Block):
    ...
    def before_write(self, env):
        self.short_signal_name = zeroterm(self.short_signal_name, env)
        self.signal_description = zeroterm(self.signal_description, env)
...
class ChannelConversion(Block):
    ...
    def before_write(self, env):
        self.physical_unit = zeroterm(self.physical_unit, env)
            ...
            if conversion['conversion_type'] == 'lookup_dict':
                ...
                if data:
                    ...
                    ccblock.set_init_000x(
                        flatten(
                            [[float(k), zeroterm(v, env)] for k, v in data]))
...
class MdfFile():
    ...
    def __init__(self, file_object, mode='rb',
                 ...
                 allow_partial=False, encoding='latin1'):
        ...
        if mode in ['rb', 'w+b']:
            ...
            if mode == 'rb':
                ...
                self.default_init()
                self.idblock.version_number = version
                if version == 300:
                    pass
                elif version == 320:
                    self.idblock.format_identifier = '3.20'
                elif version == 330:
                    self.idblock.format_identifier = '3.30'
                else:
    ...
    def default_header(self):
        ...
        hdblock.time_stamp = int(now.timestamp() * 10 ** 9)
        hdblock.utc_time_offset = 0
        return hdblock
...
# Code pages known to codecs, corresponding to ones defined by
