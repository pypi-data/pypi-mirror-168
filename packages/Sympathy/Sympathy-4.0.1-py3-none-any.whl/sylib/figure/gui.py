...
from sylib.tree_model import qt_model
...
The tree view
=============
This configuration view consists of a tree view containing all the
...
Multiple axes
=============
There is support in the node for having an extra x or y axis. This
can for example be useful if you want to have two different y ranges for
...
*YAxis* to 'right'. Lastly add one or more plots to each *Axes*.
...
``col_name`` will take on all the column names in the Table and can be used
in e.g. *Y Data* as such: ``arg[col_name]``.

Python expressions as values
============================
To allow the user extra flexibility, all properties can be given either as
a normal value or as a python expression which evaluates to a value for
that configuration item. To switch to Python mode click the grayed out
...
class DataModel:
    def __init__(self, data, backend: Backend, data_table=None):
    ...
    def set_data(self, data):
        self.model = data
        self.root = models.Root(data, self._backend)
        self.root.set_data_table(self._data_table)


class ParameterTreeView(qt_model.BaseParameterTreeView):
    def __init__(self, input_table, backend: Backend, mime=MIME, parent=None):
        super().__init__(input_table, mime, parent)
    ...
    def add(self, item_cls):
        ...
        if item_cls in selected_item.get_available_children():
        ...
        elif item_cls not in selected_item.get_available_children():
            parent_node = selected_item.parent
        else:
            parent_node = selected_item
        parent_index = model.item_to_index(parent_node)

        if (item_cls in parent_node.get_available_children() and
...
class FigureFromTableWidget(ParameterView):
    def __init__(self, input_table, parameters, backend: Backend, parent=None):
    ...
    def _init_gui(self):
        ...
        self._show_home_action = QtGui.QAction(self, 'show_home')
        ...
        self._show_tree_action = QtGui.QAction(self, 'show_tree')
        ...
        self._back_action = QtGui.QAction(self, 'back')
        ...
        self.param_tree = ParameterTreeView(
            self.input_table, backend=self._backend, mime=MIME, parent=self)
        self.param_tree.setModel(self._model)
        toolbar_items = [ti for ti in TOOLBAR_ITEMS
                         if ti == 'separator'
                         or self._backend.matches(ti.backends)]
        self.toolbar = qt_model.BaseItemsToolBar(
            self.param_tree, toolbar_items, mime=MIME, parent=self)
        if self._backend.wizards:
    ...
    def _get_wizard(self, name) -> wizards.Wizard:
        if self._wizards[name] is not None:
            # Already initialized
    ...
    def _wizard_changed(self, name):
        self._model.set_data(self._get_wizard(name).get_model())
        self.expand_axes()
    ...
    def _init_from_parameters(self):
        ...
        if self._model.root_item().has_axes():
            self.stacked.setCurrentKey('tree')
        elif not self._backend.wizards:
            # The model is usually initialized when the user selects one of the
            # wizards on the wizard page, but if the backend supports no
            # wizards that page will not be shown, and so we need to initialize
            # the model here instead. A simple way to do that is with the empty
            # wizard.
            self._model.set_data(self._get_wizard('empty').get_model())
            self.expand_axes()
            self.stacked.setCurrentKey('tree')
        else:
            self.stacked.setCurrentKey('home')
