...
from sylib.figure import colors, common
...
class Backend:
    name: str
    markers: List[str]
    legend_locations: List[str]
    font_sizes: List[str]
    wizards: List[str]

    def matches(self, backends: Sequence[str]):
        """
        Return True if self is represented in the list of backends
        An empty list is seen as matching all backends.
        """
        return (not backends or self.name in backends)
...
class BackendSpecific:
    ...
    def resolve(self, backend: Backend) -> Any:
        return getattr(backend, self.attr)
...
SY_MARKER_PARAMS = {'label': 'Marker',
                    ...
                    'icon': SvgIcon.marker,
                    'default': 'circle',
                    'options': BackendSpecific('markers'),
                    'description': (
...
class FigureBaseNode(BaseNode):
    ...
    def valid_leafs(self):
        ...
        for leaf, params in self.NODE_LEAFS.items():
            if not self.root_node().backend.matches(
                    params.get('backends', [])):
                continue
            valid.append(leaf)
        return valid
    ...
    def get_available_children(self):
        return [c for c in super().get_available_children()
                if self.root_node().backend.matches(c.backends)]
    ...
    def create_leaf(self, leaf, data=None, eval=None, params=None):
        ...
        if isinstance(options, BackendSpecific):
            options = options.resolve(self.root_node().backend)
...
class FigureProperty(Property):
    ...
    def parsed_value(self, extra_vars=None):
        if self.eval == 'py':
            ...
            raise ValueError(f'Unknown evaluation method "{self.eval}"')
        return value
...
class Root(FigureBaseNode):
    ...
    def __init__(self, data, backend: Backend, parent=None):
...
class BasePlot(FigureBaseNode):
    ...
    def init(self, data):
        valid_child_nodes = [c for c in self.get_available_children()]
...
class Iterator(BasePlotContainer):
    ...
    def get_available_children(self):
        if len([c for c in self.children if c.node_type in self.PLOT_TYPES]):
            ...
            return [
                c for c in super().get_available_children()
                if c in self.parent.get_available_children()]
        else:
            return [c for c in super().get_available_children()]
...
class BarContainer(BasePlotContainer):
    ...
    def add_other_nodes(self, data):
        valid_child_nodes = [
            c for c in self.get_available_children()
            if not issubclass(c, (BasePlot, BasePlotContainer))]
...
class Legend(FigureBaseNode):
    ...
    NODE_LEAFS = dict([
        ...
        ('loc', {'label': 'Location',
                 ...
                 'default': 'upper right',
                 'options': BackendSpecific('legend_locations'),
        ...
        ('fontsize', {'label': 'Font Size',
                      ...
                      'default': 'medium',
                      'options': BackendSpecific('font_sizes'),
...
class BaseAxis(FigureBaseNode):
    ...
    NODE_LEAFS = dict([
        ...
        ('inverted', {'label': 'Inverted',
                      ...
                      'options': None,
                      'description': (
                          'Invert the min and max limits of the axis.'),
                      'editor': tree_widgets.SyBoolComboBox,
                      'backends': ['matplotlib']}),
        ('scale', {'label': 'Scale',
                   'type': 'options',
