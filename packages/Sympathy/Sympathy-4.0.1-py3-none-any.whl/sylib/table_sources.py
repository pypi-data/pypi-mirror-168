...
# Copyright (c) 2013, 2016-2017, 2022, Combine Control Systems AB
#
...
def _local_csv_reader(*args, **kwargs):
    res = pd.io.parsers.read_csv(*args, header=None, **kwargs)
    res.rename(
        columns={k: f'X{i}' for i, k in enumerate(res.head())}, inplace=True)
    return res
...
def compute_end_row(offset, no_rows, rows, part=False):
    """Compute end row for table preview."""
    if part:
...
class TableSourceModel(QtCore.QObject):
    ...
    def _init_model_common_parameters(self):
        """Init common parameters for table importers."""
...
class TableSourceCSV(object):
    ...
    def _read_c(self, no_rows, row_offset, encoding,
        ...
        try:
            return _local_csv_reader(self._fq_infilename,
                                     sep=delimiter,
