...
from PySide6 import (QtCore, QtWebChannel, QtWebEngineCore)
    ...
    def add_objects(self, objects, signals=''):
        if self.webChannel() is not None:
            ...
            for name, obj in objects.items():
                ...
                end_script += "{helper} = channel.objects.{helper};".format(
                    helper=name)
            js = (
                initial_script +
...
class GraphJSWebPageView(WebEnginePage):
    ...
    def run_scripts_on_load(self):
        ...
        #     "click", function(){ jshelper.on_clicked(editor.getHtml()) });
        # '''
        # self.runJavaScript(js)
