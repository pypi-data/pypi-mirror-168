...
def color_string_to_tuple(color_string):
    return tuple([int(x, 16) for x in re_color.match(color_string).groups()])


def interpolate_color_tuples(color1_tuple, color2_tuple, weight):
    return tuple([int((c2 - c1) * weight + c1)
                  for c1, c2 in zip(color1_tuple, color2_tuple)])


def tuple_to_color_string(color_tuple):
...
class IdentityScale(object):
    ...
    def __call__(self, domain_value):
        if (isinstance(domain_value, (Sequence, np.ndarray)) and
...
class LinearScale(IdentityScale):
    ...
    def __call__(self, domain_value):
        if self.f == self.color_f:
            result = super().__call__(domain_value)
            return result
