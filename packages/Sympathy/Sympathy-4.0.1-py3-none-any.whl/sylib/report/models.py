...
class Property(AbstractLeaf):
    ...
    def __init__(self, parameters, parent=None):
        ...
        """
        super().__init__(parameters['data'], parent)
    ...
    def init(self):
        ...
        for label_, property_, icon, editor in self.SCALE_PROPERTIES:
            if property_ == 'type':
                ...
                options = _func_value(scales.SCALE_TYPES)
            else:
                options = _func_value(None)
    ...
    def init(self):
        ...
        for label, property_, icon, editor_class in self.GRAPH_NODE_PROPERTIES:
            if property_ == 'projection':
                ...
                options = _func_value(self.projection_options)
            else:
                options = _func_value(None)
    ...
    def init(self):
        for label, property_, icon, editor in self.AXIS_NODE_PROPERTIES:
            if property_ == 'scale_type':
                options = _func_value(['linear', 'log'])
            else:
                options = _func_value(None)
            prop = Property({'label': label,
                             'property': property_,
...
class GraphLayerDataDimension(AbstractNode):
    ...
    def _get_source_property(self):
        return [x['id'] for x in self.root_data()['data']]
    ...
    def init(self):
        for label, property_, icon, editor in self.BINDING_PROPERTIES:
            if property_ == 'source':
                options = self._get_source_property
            elif property_ == 'axis':
                options = self._get_axis_property
            else:
                options = _func_value(None)
            prop = Property({'label': label,
                             'property': property_,
