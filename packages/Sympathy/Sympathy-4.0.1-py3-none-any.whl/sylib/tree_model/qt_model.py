...
class BaseTreeItemModel(QtCore.QAbstractItemModel):
    ...
    def get_data_table(self):
        return self.model.root.get_data_table()
    ...
    def data(self, index, role):
        ...
        if role == QtCore.Qt.DisplayRole:
            ...
            if index.column() == 1 and item.is_leaf:
                return (str(item.data), item.eval)
...
class SyDataTreeCompleterModel(complete.DictTreeModel):
    def __init__(self, data, parent=None):
        self._data = data
        dict_model = data.completions().to_dict(data, name='arg')
        super().__init__(dict_model, parent=parent)
...
class TreeModelCompleter(complete.TreeItemCompleter):
...
class ParameterTreeDelegate(QtWidgets.QStyledItemDelegate):
    def __init__(self, completer_models, parent):
        ...
        self.closeEditor.connect(self._on_closeEditor)
    ...
    def setEditorData(self, editor, index):
        if not isinstance(editor, widgets.SySubEditMixin):
    ...
    def _on_closeEditor(
        ...
        # Ensure completer is removed.
        if self._completer:
            popup = self._completer.popup()
            if popup and popup.isVisible():
                popup.hide()
            editor.set_completer(None)
            self._completer = None
...
class BaseParameterTreeView(QtWidgets.QTreeView):
    ...
    def __init__(self, input_table, mime=MIME, parent=None):
        ...
        self.edit_action = QtGui.QAction(icon, 'Edit', self)
    ...
    def add_child(self, parent_node, child_cls, parent_index):
        child_node = child_cls.create_empty_instance(parent=parent_node)
