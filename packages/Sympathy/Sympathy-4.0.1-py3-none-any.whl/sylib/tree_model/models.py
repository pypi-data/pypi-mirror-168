...
def insert_into_dict(data, node, position=0):
    """Insert a node into a dict ``data`` a ``position``."""
    keys = data.keys()
    if node.is_leaf:
        key = node.name
    else:
    ...
    data = dict([(k, data.get(k, node.data)) for k in keys])
    return data
...
class BaseNode:
    ...
    def init_leafs(self, data):
        for leaf in self.valid_leafs():
            params = self.NODE_LEAFS[leaf]
...
class Property(BaseLeaf):
    ...
    def __init__(self, parameters, parent=None):
        ...
        """
        super().__init__(parameters['data'], parent=parent)
        self._label = parameters['label']
        self.name = parameters['name']
        self.type = parameters.get('type', str)
