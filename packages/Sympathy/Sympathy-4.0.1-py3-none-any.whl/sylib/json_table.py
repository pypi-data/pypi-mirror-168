...
class JsonRow(object):
    ...
    def minimize_keys(self):
        ...
        for depth in range(1, max_depth):
            ...
            for i, (item, trial_key) in enumerate(
                    zip(self._items, trial_keys)):
                if not testable[i]:
                    continue
                count = sum(
                    [trial_key == key_b for key_b in trial_keys if key_b])
                if count > 1:
                    testable[i] = False
        ...
        if all(mk is None for mk in minimal_keys):
            # Nothing was changed, all keys were unique from the beginning
            return
...
class JsonTable(object):
    ...
    def is_group(self):
        return isinstance(self._dict, (list, dict))
