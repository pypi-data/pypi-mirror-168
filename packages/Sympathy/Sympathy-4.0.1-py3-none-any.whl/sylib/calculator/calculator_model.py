            ...
            if self._expr and self._expr.strip():
                ...
                for sub in subs:
                    if (isinstance(sub.value, ast.Name)
                        ...
                        if isinstance(sub.slice.value, ast.Str):
                            cols.append(sub.slice.value.s)
                        elif isinstance(sub.slice.value, str):
                            # Python 3.9.
                            cols.append(sub.slice.value)

            cols.extend(res_col_regex.findall(self._expr))
            self._deps = list(dict.fromkeys(cols).keys())
        return self._deps
    ...
    def valid(self):
        ...
        def check_ast(calc, ctx=None):
            ...
            def check_var(load, store, top_store, ctx):
                for x in load:
                    if (x not in store and
                            x not in [item[0] for item in inspect.getmembers(
                                builtins)] and
                            x not in ctx and
                            x not in top_store):
                        raise NameError(
                            'NameError: {} can\'t be found'.format(x))
...
class CalculatorItemModel(QtGui.QStandardItemModel):
    ...
    def get_item_by_name(self, name):
        items = self.findItems(name, flags=QtCore.Qt.MatchExactly, column=0)
        if items:
            return items[0]
    ...
    def validate(self):
        ...
        valid = all([i.valid and not i.duplicate and not i._forbidden_name
                     for i in items])
        cycles = check_cycles()
