...
class FunctionsTreeWidget(TreeDragWidget):
    ...
    def set_model(self, model):
        ...
        def build_tree(root, content):
            if isinstance(content, list):
                ...
                for tree_text, subcontent in content.items():
                    child = build_and_insert_subroot(
                        root, tree_text, subcontent)
            else:
        ...
        def purge_hidden_items(gui_dict, hidden_items):
            ...
            current_hidden = [
                item[0] for item in hidden_items if len(item) == 1]
...
class TableDragMixin(object):
    def __init__(self, *args, **kwargs):
        ...
        drag_mode : PySide6.QtCore.Qt.DropAction
            Drag mode to use.
        """
...
class CalculatorWidget(ParameterView):
    def __init__(self, in_data, parameters,
        ...
        self.calcs_table_view.current_row_changed.connect(
    ...
    def _undo_state_changed(self, new_state):
        ...
        fail_strategy_event = self._parameter_root[
            'fail_strategy'].value_changed
        ...
        if diff_calc is not None:
            self.calcs_table_view.setCurrentIndex(
                self.model.index(diff_calc, 0))
        if diff_attr:
