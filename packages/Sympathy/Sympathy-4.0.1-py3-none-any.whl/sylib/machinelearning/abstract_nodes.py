...
class ModelParameterWidget(ParameterView):
    ...
    def build_ui(self):
        ...
        Tabbed UIs derive from a list _desc_info (defined in the node_<name>.py
        script).
        ...
        if len(self._desc.info) > 0 and isinstance(self._desc.info[0], list):
            ...
            for info in self._desc.info:
                # Generates tabs (number equal the length ofn the "info"
                # vector) plus sub-layouts for evey element in the tabs.
    ...
    def build_parameter_widget_onetab(self, name):
        ...
        if 'dispname' in parameter.keys():
            ...
            widget.setToolTip("{0}\n\n{1}".format(
                self._desc.descriptions[name].replace('\a', '\n'),
                text_description))
