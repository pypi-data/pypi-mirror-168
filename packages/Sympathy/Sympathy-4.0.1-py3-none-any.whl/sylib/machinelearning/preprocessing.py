...
check10 = sklearn_version >= pversion.Version('1.0.0')
...
class OneHotEncoderDescriptor(Descriptor):
    ...
    def post_fit(self, skl):
        ...
        if sklearn_version <= pversion.Version('0.20.0'):
...
class PolynomialFeaturesDescriptor(Descriptor):
    ...
    def get_attribute(self, skl, attribute):
        if check10 and attribute == "n_input_features_":
            attribute = "n_features_in_"
        return getattr(skl, attribute)

    def post_fit(self, skl):
        y_names = []
        x_names = self.x_names or [
            "X{0}".format(i) for i in range(
                self.get_attribute(skl, "n_input_features_"))]
        for row in range(skl.n_output_features_):
            name = ""
            for col in range(self.get_attribute(skl, "n_input_features_")):
    ...
    def fit(self, X):
        ...
        for col in range(X.shape[1]):
            ...
            if self.max_categories is not None:
                count = np.array([np.sum(X[:, col] == val)
                                  for val in all_values], dtype=int)
                s = np.argsort(-count)
