...
def table_to_array(tbl, unitary=False):
    result = np.array([tbl.get_column_to_array(colname)
...
def data_to_table(X, names, table, default_prefix='x', transpose=False):
    ...
    2D Arrays keep their default shape (first index is table row, second
    column) lists are assumed to be lists of columns.
    """
...
class _PickleablePrefixGeneratingFunction(object):
    """
    Dummy class that pretends to be a function with a lexical enclosure that
    cannot pickled, instead implemented as a class with explit storage of the
    enclosed value
    """
...
def value_to_tables(value, name, cnames=None, rnames=None):
    ...
    if isinstance(value, str) or isinstance(value, numbers.Number):
        ...
        if dimensions == 1 and cnames is not None:
            # Let column vectors become a row vector when column names are
            # given
        ...
        if dimensions == 1:
            ...
            if rnames is not None:
                ...
                tbl.set_column_from_array(
                    'name', np.array(rnames[:value.shape[0]]))
