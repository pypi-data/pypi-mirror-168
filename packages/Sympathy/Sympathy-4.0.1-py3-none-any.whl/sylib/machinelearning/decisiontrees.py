...
class RandomForestDescriptor(DecisionTreeDescriptor):
    ...
    def get_attribute(self, skl, attribute):
        if check_10 and attribute == "n_features_":
    ...
    def get_parameters(self, parameters):
        ...
        for name, info in self._parameters.items():
            ...
            if check_10 and parameters[name].value in ["mse", "mae"]:
                compat_dict = {"mse": "squared_error", "mae": "absolute_error"}
                kwargs[skl_name] = compat_dict[value]
            else:
