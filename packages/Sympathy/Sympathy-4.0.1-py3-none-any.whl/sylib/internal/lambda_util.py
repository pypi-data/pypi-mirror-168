    ...
    def execute(self, json_data, datatype, filenames, **kwargs):
        ...
        for filename, result in extract_results:
            ...
            if status:
                ...
                flowdata = []
                try:
                    flow_vars = flow.effective_vars_context()
                    ...
                    for lambda_ in lambdas:
                        flowdata.append(json.loads(
                            sympathy.app.execore.lambdadata(
                                lambda_, flow_vars)[
                                    'parameters']['data']['flow']['value']))
                except sympathy.app.execore.FlowDataError as e:
                    result = (False, str(e))
                except sympathy.app.flow.functions.ExtractError as e:
                    result = (False, str(e))
                except Exception:
                    import traceback
                    result = (False, 'Could not extract lambdas due to an '
