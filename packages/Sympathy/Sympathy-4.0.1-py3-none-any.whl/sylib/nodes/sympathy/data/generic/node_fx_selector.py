...
The script file can be edited using either the `internal` or the `external`
editor.

The internal editor is a basic code editor which supports syntax highlighting
and has basic support for indentation in python code, using 4 spaces, after
TAB and ENTER. It is included in Sympathy.

The external editor is the global editor in Sympathy, it is normally the
default program configured to open python files (.py).

Script without separate file
============================

For short scripts that are not intended to be shared between different nodes it
can be convenient not having to create an external file. To enable this
feature, the datasource port first needs to be deleted.  Once there is no
datasource port, the node will show an embedded version of the internal editor,
when configured.
