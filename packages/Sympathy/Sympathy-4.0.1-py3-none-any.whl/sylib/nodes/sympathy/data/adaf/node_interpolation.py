...
def resample_file_with_spec(parameter_root, spec, in_adaffile, out_adaffile,
    ...
    if raster_param.selected:
        raster_names = spec._require_column(raster_param, types='U')
    else:
    ...
    for (dt, raster_name), dt_signals in dt_to_signals.items():
        ...
        if raster_name:
            ...
            deprecated_warn(
                'Parameter "Round time steps in raster name"', '5.0.0')
        else:
        ...
        for origin_system_name in in_adaffile.sys.keys():
            ...
            for origin_raster_name in origin_system.keys():
                ...
                for signal_name in signals_in_this_raster:
                    progress(100 * min(
                        1, progress_counter / total_signals_count))
...
def get_new_timebasis(in_adaffile, dt, signals):
    ...
    if sample_count == int(sample_count):
        timebasis_new = np.linspace(
            t_range[0], t_range[1], int(sample_count + 1))
    else:
...
class SuperNode(synode.Node):
    author = 'Helena Olen & Magnus Sandén'
    ...
    parameters.set_float(
        ...
        editor=synode.editors.spinbox_editor(step=1e-3))
    parameters.set_list(
