...
FAILURE_STRATEGIES = dict([('Exception', 0), ('Skip calculation', 1)])
...
To use a signal from the incoming generic data use ``arg`` in a way that fits
the data format as can be seen in the examples below.
...
The data on the input port is available under the name ``arg`` and any
calculated columns are accessable from the Table named ``res``.
...
Some examples::
    ...
    - ADAF          - Time series are accessed as
                      `arg.sys['system0']['raster0']['signal0'].y`
