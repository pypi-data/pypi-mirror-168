...
def _scipy_version():
    return pversion.parse(scipy.__version__)
...
def median_period(data, peak_detector, samples):
    ...
    if peak_detector == LOCAL_MAXIMA:
        ...
        if _scipy_version() < pversion.Version('1.5.0'):
...
class SequenceSplit(node.Node):
    """
    Find peaks in a signal and slice the input Table into a list of Tables,
    where each element contains one peak.
    ...
    parameters.set_list(
        ...
        editor=node.Util.combo_editor(edit=True, placeholder='Required'))
    ...
    def execute(self, node_context):
        ...
        data = in_table._require_column(parameters['select_column'])
