...
def axes_wrapper(axes):
    if isinstance(axes, (collections.abc.Sequence, np.ndarray)):
        wrapped = []
        for ax in axes:
...
class File(filebase.TypeAlias):
    ...
    def _decode(text_data):
        figure = pickle.loads(base64.decodebytes(text_data.encode('ascii')))
    ...
    @staticmethod
    def _encode(fig):
        return base64.encodebytes(pickle.dumps(
