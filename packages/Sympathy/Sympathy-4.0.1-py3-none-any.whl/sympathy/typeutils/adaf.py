...
from .. datasources.hdf5.dstable import array_to_bytes as _array_to_bytes
...
class File(filebase.TypeAlias):
        ...
        return {
            ...
            'sys': {
                system_name: {
                    ...
                    for raster_name, raster in self._index_system_rasters(
                            system)
                }
    ...
    def equal_to(self, other, col_order=True, col_attrs=True, tbl_names=True,
        ...
        if len(self_sys_keys) != len(other_sys_keys):
            return not_equal(
                'ADAFs are not equal. The number of systems differ.')
    ...
    def __get_index(self):
        try:
            data = _array_to_bytes(self.root['gzip_index'].value()[0])
            return json.loads(gzip.decompress(data).decode('ascii'))
        except KeyError:
            return None
...
class Group(filebase.PPrintUnicode):
    ...
    def vjoin(self, other_groups, input_index='', output_index='', fill=False,
              minimum_increment=1):
        """VJoin Group with other Group."""
        input_list = sylist(types.from_string('[sytable]'))
        for other_group in other_groups:
            input_list.append(other_group.__data)
        sybase.vjoin(self.__data, input_list, output_index, fill,
                     minimum_increment)
...
class SystemGroupContainer(NamedGroupContainer):
    ...
    def __init__(self, data, name=None):
        self.name = name
        self._data = data
        self._cache = dict.fromkeys(data.keys())
