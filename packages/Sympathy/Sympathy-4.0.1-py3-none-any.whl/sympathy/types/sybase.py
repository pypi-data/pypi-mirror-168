...
class VJoinVisitor(GroupVisitor):
    ...
    def __init__(self, current, output_index, fill,
                 minimum_increment):
    ...
    def visit_table(self, sytable):
        ...
        def indices_using_group_array(group_array):
            ...
            for i, (v, m), in enumerate(
                    zip(group_array, np.ma.getmaskarray(group_array))):
                if m:
                    indices = lookup.setdefault(m, [])
                else:
                    indices = lookup.setdefault(v, [])

                indices.append(i)

            res = []
            for v in list(lookup):
                res.append(np.array(lookup.pop(v)))
            return res
        ...
        for slice_index in slice_indices:
            ...
            self.output_list.append((None, result))
...
def vjoin(output_table, sylist, output_index, fill,
          minimum_increment):
    """VJoin first and second sygroup using the VJoinVisitor."""
    empty_output = output_table.is_empty()
    if sylist:
        tmp_output_table = output_table
        if not empty_output:
            tmp_output_table = sylist.create()
            tmp_sylist = sylist[:0]
            tmp_sylist.append(output_table)
            tmp_sylist.extend(sylist)
            sylist = tmp_sylist

        visitor = VJoinVisitor(
            tmp_output_table, output_index, fill,
            minimum_increment)
        sylist.visit(visitor)

        if not empty_output:
            output_table.source(tmp_output_table, shallow=True)
...
class symap(sycontainer):
    ...
    def index(self, limit=None):
        ...
        for k, v, ll in self._limited_items(self._cached_items(), limit):
            if ll is False:
                res_items[k] = None
            elif v is None:
                if k in ind_items:
