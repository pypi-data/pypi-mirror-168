...
def _get_none_object_dtype(arr, name):
    ...
    try:
        ...
        if isinstance(first, datetime.datetime):
            ...
            # bug.
            # TODO: Check if this fallback can be removed
        ...
        raise exc.DatasetTypeError(
            'Unsupported object type in column {}'.format(name)) from None
    return arr
...
class Column(object):
    def __init__(self, data=None, source=None, attrs=None):
        ...
        elif isinstance(data, _da().Array):
            source = InMemoryColumnSource(data)
    ...
    def get(self, index=None, kind='numpy'):
        ...
        if kind == 'numpy':
            if isinstance(res, _da().Array):
                res = res.compute()
        elif kind == 'dask':
            if not isinstance(res, _da().Array):
                res = _da().from_array(res, chunks=chunks(res), asarray=False)
        else:
