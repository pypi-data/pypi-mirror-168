...
def format_deprecated_warn(msg, version, repl='', help=False):
    """
    Calling the method will return a warning message about the node
    in question being scheduled for removal.

    If repl (replacement message) is provided that message is included.
    """
    help_msg = ''.join([
...
def is_removed_node(cls):
    res = False
    try:
        res = cls.node_is_removed
    except Exception:
        pass
    return res


def removed_node(version, repl=''):
    """
    Soft remove, to allow keeping implementations for a limited amount of time.
    Arguments are used for annotation and are not directly in use.
    """
    def wrapper(cls):
        cls.node_is_removed = True
        cls.tags = tag.Tags(tag.TagBuilder().Hidden.Deprecated)
        cls.deprecated = (cls.name, version, repl)
        return cls
    return wrapper
