...
from .. platform.port import PortType


def port_structures_to_dict(inputs, outputs):
    port_structure = {}
    group = False

    all_names = {}
    for field, field_data in zip(['inputs', 'outputs'], [inputs, outputs]):
        names = {}
        all_names[field] = names

        if field_data is not None:
            field_data = [port.to_dict() for port in field_data]
            # Temporary restoration of indices.
            for i, port in enumerate(field_data):
                port['index'] = i
                name = port.get('name')
                ...
                if name is not None and name in names:
                    raise ValueError(
                        "{} port names may not be reused".format(field[:-1]))
                ...
                if name is not None:
                    ...
                    if name.startswith('__sy'):
                        raise ValueError(
                            "Port names starting with __sy are "
                            "reserved for internal use.")

                if 'n' in port:
                    if not name:
                        raise ValueError(
                            "When 'n' is used port name needs to be defined")

            port_structure[field] = field_data
        else:
        ...
        if group:
            if set(all_names.get('inputs', {})).intersection(
                ...
                raise ValueError("When template group is used, port names "
                                 "need to be unique")
...
def port_maker(port_information, mode, external=True,
               no_datasource=False):
    """
...
def port_file_maker(port_information, mode, external=True):
    ...
    except types.UndefinedTypeError as e:
        part = ''.join([str(a) for a in e.args])
        raise exceptions.SyNodeError(
            f'Undefined type {part} in port type {alias}')
    ...
    return factory.typefactory.from_dict(data)
