...
class Draggable:
    ...
    def _on_pick(self, event):
        try:
            if (event.artist is self.artist and self._grab.grab(
                    event.mouseevent)):
                self.grab()

                xdata = _num_to_axis(
                    event.mouseevent.xdata, self.ax.get_xaxis())
                ydata = _num_to_axis(
                    event.mouseevent.ydata, self.ax.get_yaxis())
...
class VerticalCursorController(DraggableParent, QtCore.QObject):
    ...
    def __init__(self, ax, snapper, grabber, cursors, data, band=False):
        ...
        self._cursors = [DraggableVerticalLine(
            snapper=snapper, grabber=grabber, ax=ax, x=x, label='xo',
            parent=self)
                         for x in cursors]
        self._children = []
