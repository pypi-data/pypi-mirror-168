...
from . dsstate import hdf5_state
...
def read_header(filepath):
    ...
    with open(filepath, 'rb') as hdf5:
        ...
        header_data = json.loads(
            line.decode('ascii'), object_pairs_hook=dict)
        return header_data
            ...
            if not isinstance(lnk, h5py.ExternalLink):
                ...
                if not file_group:
                    try:
                        ...
                        # Ignore corrupted and missing files that can result
                        # from failures or file cleanup.
                        return NullGroupWrapper(NullGroup(lnk_filename))
...
class NullGroupWrapper(GroupWrapper):
    """
    Represent a group that is unavailable.
    For example after a linked file has been removed.
    """
    def __init__(self, group):
        self.__group = group
...
class Hdf5Group(object):
    ...
    def write_link(self, name, other, other_name):
        ...
        other_name = replace_slash(other_name)
