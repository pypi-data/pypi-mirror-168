...
_numpy_lt_119 = False
if parse_version(np.__version__) < parse_version('1.19.0'):
    _numpy_lt_119 = True


def array_to_bytes(array):
    if not _numpy_lt_119:
...
class Hdf5TableBase(dsgroup.Hdf5Group):
    ...
    def __init__(self, group, can_write, can_link, external):
        ...
        self._length = None
        if ext is not None:
            ext = json.loads(array_to_bytes(ext[...]))
    ...
    def read_column(self, column_name, index=None):
        ...
        column = _da().from_array(
            Column(self, store_name, column),
    ...
    def write_column(self, column_name, column):
        ...
        if isinstance(column, _da().Array):
    ...
    def transferable(self, other):
        return (isinstance(other, Hdf5TableBase) and
    ...
    def transfer(self, name, other, other_name, index=None, attrs=None):
        ...
        if isinstance(other, h5py.Dataset):
            ...
            index = other._col_indices.get(other_name)
            src = other._col_sources.get(other_name)
            if src:
            ...
            if attrs is None:
                attrs = other._col_attrs.get(other_name)
        ...
        self._col_sources[store_name] = [
            src_filename, src_dataset_name, store_name]
    ...
    def write_finished(self):
        ...
        if ext:
            self.group.create_dataset(
                EXT, data=np.array(json.dumps(ext).encode('ascii'),
                                   dtype=np.void))
...
class Hdf5Table(dsgroup.Hdf5Group):
    ...
    def transferable(self, other):
        return self._table.transferable(other._table)

    def transfer(self, name, other, other_name, index=None, attrs=None):
        return self._table.transfer(
            name, other._table, other_name, index, attrs)
    ...
    def write_link(self, name, other, other_name):
        return self._table.write_link(name, other._table, other_name)
