...
class FunctionWizard(SuperWizard):
    def __init__(self, parent=None):
        super().__init__('Function wizard', parent)
...
class FunctionWizardModel(QtCore.QObject):
    ...
    _IMPORTS = {
        'any': 'from sympathy.api import fx',
    ...
    _OTHER_EXAMPLE = _error_text.__func__(1)[0:-1]
    ...
    def set_function_name(self, name):
        self._function_name = name.lower().replace(' ', '_')
    ...
    def set_function(self):
        self._function = '\n\n' + (
            f"@fx.decorator(['{self._type}'])\n"
            f'def {self._function_name}(arg, res):\n'
            f'    """Add function documentation here"""\n')

        try:
            self._function += self._EXAMPLES[self._type]
        except KeyError:
    ...
    def get_code(self):
        if (f'def {self._function_name}(' not in self._code
                and self._function_name):
    ...
    def _set_header(self):
        ...
        key = key.replace('[', '')
        key = key.replace(']', '')
        if self._IMPORTS['any'] not in self._code:
            self._header += self._IMPORTS['any'] + '\n'
        if (self._IMPORTS[key] not in self._code and
                self._IMPORTS[key] not in self._header):
