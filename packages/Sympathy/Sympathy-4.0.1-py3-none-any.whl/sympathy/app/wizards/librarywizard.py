...
class LibraryWizardDirectoryPage(QtWidgets.QWizardPage):
    def __init__(self, title, model, parent=None):
        ...
        self._new_button = QtWidgets.QRadioButton('New (recommended)')
        ...
        self._old_button.setToolTip(
            'Old structure available since version 1.0.0, organizes the '
            'library in a folder structure compatible with older versions.')
    ...
    def _create_base_tree(self):
        ...
        self._add_item_to_tree(test, file_icon, 'test_with_pytest.py')
        self._add_item_to_tree(test, file_icon, 'test_with_nose.py')
...
'''
        ...
        elif path.endswith('test_with_pytest.py'):
            flows_dir = 'flows'
...
from sympathy.test.run_workflow import sy_flow_runner

test_root = os.path.dirname(os.path.abspath(__file__))
test_flows, test_ids = run_workflow.find_flows(
    os.path.join(test_root, '{flows_dir}'))

@pytest.mark.parametrize("flow_path", test_flows, ids=test_ids)
def test_flow(flow_path, sy_flow_runner):
    """
    Test all workflows in library workflow test directories.
    Test workflows using command line tool cli.
    """
    assert sy_flow_runner.run(flow_path) == 0
'''
            with open(path, 'w') as f:
                f.write(test)

        elif path.endswith('test_with_nose.py'):
            flows_dir = 'flows'
            if not self._model.new_version:
                flows_dir = 'Workflow'
            test = f'''import os
...
def test_workflows():
    ...
    Test workflows using command line tool cli.
    """
    for f in run_workflow.run_flows_in_path(
            os.path.join(test_root, '{flows_dir}'),
            prefix='{self._model.library_name} test flow '):
        yield f
'''
            with open(path, 'w') as f:
                f.write(test)
