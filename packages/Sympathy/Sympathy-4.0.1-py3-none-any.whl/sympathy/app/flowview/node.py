...
class NodeInfo(QtWidgets.QDialog):
    ...
    def _init_parameters_tab(self):
        ...
        self._tab_widget.addTab(parameters_tab, "Advanced")
...
class NodeView(BaseNodeView):
    ...
    def _init(self):
        if self._model.has_svg_icon:
            self._icon = QtSvgWidgets.QGraphicsSvgItem(self)
    ...
    def _init_context_menu(self):
        ...
        self._migrate_forward_action = QtGui.QAction(
            'Migrate node')
    ...
    def _edit_node_requested(self):
        ...
        if (settings.instance()['Gui/system_editor'] or not
            ...
            oss.open_url(QtCore.QUrl.fromEncoded(encoded_url).toString())
        else:
    ...
    def contextMenuEvent(self, event):
        ...
        self._migrate_forward_action.setEnabled(self._model.can_migrate())
        text = "Migrate node"
        if self._model.migrations_are_forced():
            text += " (force)"
        self._migrate_forward_action.setText(text)
