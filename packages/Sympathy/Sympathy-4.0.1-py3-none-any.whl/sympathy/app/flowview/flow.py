...
from sympathy.app import keymaps
...
def paint_drop_indicator_border(painter, rect):
    border = 4
    theme = themes.get_active_theme()
    pen = QtGui.QPen(
        theme.drop_flow_indicator_color, border, QtCore.Qt.DashLine)
    painter.setPen(pen)
    diff = border // 2
    assert diff % 2 == 0
...
class MimeFlows:
    @staticmethod
    def is_flow_filename(filename: str) -> bool:
        return filename.endswith(".syx")

    @classmethod
    def filenames(cls, mime_data):
        res = []
        if mime_data.hasUrls():
            res = [url.toLocalFile() for url in mime_data.urls()]
            return res

    @classmethod
    def is_all_flow_filenames(cls, mime_data):
        filenames = cls.filenames(mime_data)
        return bool(filenames) and all(
            cls.is_flow_filename(filename) for filename in filenames)
...
class FlowView(ElementViewInterface, QtWidgets.QGraphicsView):
    ...
    open_named_flow_requested = QtCore.Signal(str)
    ...
    def __init__(self, model, app_core, parent=None):
        ...
        self._keymap = keymaps.get_active_keymap()
    ...
    def _init(self):
        ...
        self._auto_connection_node = None
        self._auto_connection_mode = False
        self._auto_connection_max_distance = 250
        self._transient_auto_connections = {}
        self._drop_flow_active = False
    ...
    def _init_actions(self):
        ...
        self._insert_subflow_action = self._create_action(
            'Insert Subflow as Link...',
            slot=self._handle_insert_linked_subflow)
    ...
    def _init_state_machine(self):
        self._state_machine = QtStateMachine.QStateMachine(parent=self)

        self._normal_edit_state = QtStateMachine.QState(self._state_machine)
        self._insert_text_field_state = QtStateMachine.QState(
            self._state_machine)
    ...
    def _cursor_point(self):
        ...
        return self._center_node_position(top_right)

    def _create_flow(self, factory):
        position = grid.instance().snap_to_grid(self._cursor_point())
        cmd = factory(position=position, flow=self._model)
    ...
    def _create_flowio(self, factory):
        position = grid.instance().snap_to_grid(self._cursor_point())
        cmd = factory(flow=self._model, position=position)
        self._model.undo_stack().push(cmd)

    def _handle_insert_linked_subflow(self):
        ...
        if isinstance(result, tuple):
            ...
            filenames = result
        self._handle_insert_linked_subflow_filenames(
            filenames, self._context_menu_pos)

    def _handle_insert_linked_subflow_filenames(self, filenames, start_pos):
        start_pos = self._center_snap_node_position(start_pos)
        try:
            QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.BusyCursor)
            cmds = []
            layout_items = settings.instance()[
                settings.Keys.gui_layout_items]
            for i, filename in enumerate(filenames):
                if layout_items not in settings.layout_items_choice:
                    layout_items = settings.layout_items_vertical

                if layout_items == settings.layout_items_vertical:
                    offset = QtCore.QPointF(
                        0, flow.Node.default_height)
                else:
                    offset = QtCore.QPointF(
                        flow.Node.default_width, 0)

                pos = start_pos + (2 * offset * i)
                cmd = user_commands.InsertSubflowLinkCommand(
                    filename=filename,
                    flow=self._model, position=pos)
                cmds.append(cmd)
            if cmds:
                number = 's' if len(cmds) > 1 else 's'
                macro = user_commands.MacroCommand(
                    text=f'Inserting subflow{number} as link',
                    commands=cmds,
                    context=user_commands.propagate_flow_ctx(self._model))
                self._model.undo_stack().push(macro)
                created_elements = [
                    cmd.created_element() for cmd in cmds if
                    cmd.create_subflow_cmd and cmd.created_element()]
    ...
    def _common_node_creation(self, node, node_view):
        ...
        self._node_signals.connect(
            node,
            node_view.transient_connection_add_request[object],
            self._handle_auto_connection_add_request)
        self._node_signals.connect(
            node,
            node_view.transient_connection_end_request,
            self._handle_auto_connection_end_request)
        self._node_signals.connect(
    ...
    def _handle_subflow_created(self, subflow):
        ...
        self._node_signals.connect(
            subflow,
            subflow_view.transient_connection_add_request[object],
            self._handle_auto_connection_add_request)
        self._node_signals.connect(
            subflow,
            subflow_view.transient_connection_end_request,
            self._handle_auto_connection_end_request)
        self._node_signals.connect(
    ...
    @QtCore.Slot(object)
    def _handle_auto_connection_add_request(self, node):
        self._auto_connection_node = node
        self._remove_transient_auto_connections()

    @QtCore.Slot()
    def _handle_auto_connection_end_request(self):
        # Create transient auto connections and clean out but keep
        # auto connection mode on
        self._handle_create_connection_from_transient_auto_connection()
        self._auto_connection_node = None
    ...
    def _handle_connection_end_requested(self, port_view):
        ...
        if port_view and port_view.model and self._source_port:
            ...
            cmd = user_commands.CreateConnectionCommand(
                source, destination, self._model, check_cycle=True)
            self._model.undo_stack().push(cmd)
            if cmd.succeeded():
    ...
    def _handle_close_add_context(self, result, start_port, position):
        if isinstance(result, library.LibraryNode):
            ...
            position = self._center_snap_node_position(position)
    ...
    def _node_center(self):
        size = self._model.size
        return QtCore.QPointF(size.height() / 2, size.width() / 2)

    def _center_node_position(self, position):
        return position - self._node_center()

    def _center_snap_node_position(self, position):
        return grid.instance().snap_to_grid(
            self._center_node_position(position))
    ...
    def handle_paste_at_cursor(self):
        ...
        if rect.contains(position):
            self._handle_paste(position)
            self.zoom_fit_paste()

    def _snap_element_list(self, elements):
        snap = grid.instance().snap_to_grid
        ...
        if elements:
            ...
            for element in elements:
                p0 = element.position
                user_commands.MoveElementCommand(
                    element, p0, p0 + diff).redo()
    ...
    def _handle_paste(self, pos):
        ...
        """
        pos = self._center_node_position(pos)
        ...
        if data.hasFormat(appcore.AppCore.mime_type_flow()):
            ...
            self._snap_element_list(created_elements)
            self._scene.clearSelection()
            self.select_movables(created_elements)
        else:
    ...
    def focusOutEvent(self, event):
        self._end_pan()
        self._handle_end_auto_connections()
    ...
    def paintEvent(self, event):
        ...
        if self.drop_flow_active:
            paint_drop_indicator_border(
                QtGui.QPainter(self.viewport()), event.rect())
    ...
    def dragEnterEvent(self, event):
        ...
        elif MimeFlows.is_all_flow_filenames(event.mimeData()):
            self.drop_flow_active = True
            event.acceptProposedAction()
        else:
            event.setAccepted(False)

    def dragLeaveEvent(self, event):
        self.drop_flow_active = False
    ...
    def dropEvent(self, event):
        ...
        if (mime_data.hasFormat(self._app_core.mime_type_node()) and not
           (event.isAccepted() and self._scene_.itemAt(
            ...
            cmd = user_commands.CreateLibraryElementCommand(
                node_id=node_identifier, position=position, flow=self._model)
            self._model.undo_stack().push(cmd)
        elif MimeFlows.is_all_flow_filenames(mime_data):
            event.setAccepted(True)
            if settings.instance().drop_syx_as_link:
                self._handle_insert_linked_subflow_filenames(
                    MimeFlows.filenames(mime_data),
                    position)
            else:
                for filename in MimeFlows.filenames(mime_data):
                    self.open_named_flow_requested.emit(filename)
        else:
            event.setAccepted(False)
    ...
    def auto_connection_mode(self):
        return self._auto_connection_mode

    @auto_connection_mode.setter
    def auto_connection_mode(self, state):
        self._auto_connection_mode = state
        if state:
            self._handle_temporary_auto_connections()
        else:
            self._handle_end_auto_connections()
    ...
    def mousePressEvent(self, event):
        if event.button() == QtCore.Qt.LeftButton:
            ...
            if self._transient_connection:
                self._handle_connection_end_requested(None)
            elif self.auto_connection_mode:
                self._handle_end_auto_connections()
            else:
    ...
    def _handle_end_auto_connections(self):
        self._remove_transient_auto_connections()

    def _remove_transient_auto_connections(self):
        for connection, _ in self._transient_auto_connections.values():
            if connection is not None:
                self.scene().removeItem(connection)
        self._transient_auto_connections = {}

    def _handle_temporary_auto_connections(self):
        if (self._auto_connection_node is None or
                not self._auto_connection_mode):
            return

        selected_node = self._auto_connection_node
        input_ports = [p for p in selected_node.inputs if not p.is_connected()]
        output_ports = selected_node.outputs
        ports = input_ports + output_ports

        if len(ports) > 0:
            self._remove_transient_auto_connections()
            try:
                close_enough_nodes = self._find_close_nodes(
                    ports, selected_node)
            except ValueError as e:
                core_logger.exception(e)
                close_enough_nodes = []

            if len(close_enough_nodes):
                _, port, port_view, other_port_view = close_enough_nodes[0]
                self._add_transient_connection_auto_connect_mode(
                    port, port_view, other_port_view)

    def _add_transient_connection_auto_connect_mode(self, port, port_view,
                                                    other_port_view):
        transient_connection = TransientConnectionView(
            other_port_view.connection_position(), parent=self
        )
        transient_connection.set_end_position(port_view.connection_position())
        self.scene().addItem(transient_connection)
        self._transient_auto_connections[port] = (
            transient_connection, other_port_view.model)

    def _find_close_nodes(self, ports, selected_node):
        allowed_other_nodes_types = {flow.Type.Node, flow.Type.Flow,
                                     flow.Type.FlowInput, flow.Type.FlowOutput}
        close_enough_nodes = []

        for port in ports:
            if port.type == flow.Type.InputPort:
                port_view = self._nodes[selected_node].input_port_view(port)
            elif port.type == flow.Type.OutputPort:
                port_view = self._nodes[selected_node].output_port_view(port)
            else:
                raise ValueError(
                    f'Unsupported port type "{port.type()}" for '
                    f'auto-connection'
                )

            for other_node, other_node_view in self._nodes.items():
                if (selected_node != other_node and
                        other_node.type in allowed_other_nodes_types):

                    if port.type == flow.Type.InputPort:
                        other_port_views = other_node_view.output_port_views()
                    else:
                        other_port_views = other_node_view.input_port_views()

                    for other_port_view in other_port_views:
                        match = self._match_port_type_compute_distance(
                            other_port_view, port, port_view)

                        if match is not None:
                            close_enough_nodes.append(match)

        return sorted(close_enough_nodes, key=lambda v: v[0])

    def _match_port_type_compute_distance(self, other_port_view, port,
                                          port_view):
        def distance(pos1, pos2):
            return math.sqrt(
                (pos1.x() - pos2.x()) ** 2 + (pos1.y() - pos2.y()) ** 2)

        other_port = other_port_view.model

        # Match the port type and compute distance
        if (other_port.type != flow.Type.InputPort or
                not other_port.is_connected()) and port.matches(other_port):

            port_pos = port_view.connection_position()
            other_port_pos = other_port_view.connection_position()

            output_port_to_the_left = (port.type == flow.Type.InputPort and
                                       port_pos.x() >= other_port_pos.x())
            input_port_to_the_right = (port.type == flow.Type.OutputPort and
                                       port_pos.x() <= other_port_pos.x())

            if output_port_to_the_left or input_port_to_the_right:

                dist = distance(port_pos, other_port_pos)
                if dist <= self._auto_connection_max_distance:
                    return dist, port, port_view, other_port_view
    ...
    def _handle_create_connection_from_transient_auto_connection(self):
        connection_items = self._transient_auto_connections.items()
        for port, (connection, other_port) in connection_items:
            if connection is not None:
                self.scene().removeItem(connection)

                if port and other_port:
                    source, destination = flow.connection_direction(
                        other_port, port)
                    cmd = user_commands.CreateConnectionCommand(
                        source, destination, self._model, check_cycle=True)
    ...
    @property
    def drop_flow_active(self):
        return self._drop_flow_active

    @drop_flow_active.setter
    def drop_flow_active(self, value):
        if self.drop_flow_active != value:
            self._drop_flow_active = value
            self._update_view()

    def keyPressEvent(self, event):
        if event.key() == QtCore.Qt.Key_Escape:
            self._handle_connection_end_requested(None)
            self.auto_connection_mode = False
        elif (event.key() == self._keymap.auto_connect_mode and
              event.modifiers() == QtCore.Qt.ShiftModifier and
              not self.auto_connection_mode):
            self.auto_connection_mode = True
        elif (event.key() == self._keymap.auto_connect_make_connection and
              self.auto_connection_mode):
            # Create the transient auto connections and start from new without
            # the need to release the left mouse button
            self._handle_create_connection_from_transient_auto_connection()
            self._handle_temporary_auto_connections()
    ...
    def keyReleaseEvent(self, event):
        if (event.key() == self._keymap.auto_connect_mode and
                self.auto_connection_mode):
            self.auto_connection_mode = False
