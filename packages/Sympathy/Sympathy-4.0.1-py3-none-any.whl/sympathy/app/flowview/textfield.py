...
class TextFieldView(MovableElementViewInterface):
    ...
    def _init_resize_state_machine(self):
        self._resize_state_machine = QtStateMachine.QStateMachine()
        self._state_resizing = QtStateMachine.QState(
            self._resize_state_machine)
        self._state_normal = QtStateMachine.QState(self._resize_state_machine)
    ...
    def set_text(self, value):
        try:
            ...
            for line in value.splitlines():
                ids = (sympathy.app.library_manager.
                       DocsBuilder.node_example_re.findall(line))
                for node_id in ids:
                    try:
                        ...
                        pass
                lines.append(line)
            self._textview.document().setMarkdown(
                '\n'.join(lines),
    ...
    def _select_example_nodes_requested(self):
        ...
        if dialog.exec_():
            for item in widget.selectedItems():
                ...
                texts.append('- {}'.format((
                    sympathy.app.library_manager.
                    DocsBuilder.format_node_example(
...
class ColorAction(QtGui.QAction):
    def __init__(self, color, parent=None):
        ...
        super().__init__(QtGui.QIcon(col), color, parent)
        self._color = color
