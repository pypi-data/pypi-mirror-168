...
from sympathy.utils.context import trim_doc, is_removed_node
...
def check_file_for_classes(node):
    ...
    members = dict(inspect.getmembers(mod_))
...
def get_properties(node, class_name, instance):
    ...
    properties['validate'] = instance._needs_validate()

    try:
        properties['parameters'] = {}
        parameters = members['parameters']
        if not isinstance(parameters, ParameterRoot):
            sywarn(
                'Use sympathy.api.node.parameters to create node parameters.'
                '\n'
                'From version 5.0.0 using dict parameters will result in an '
                'error.'
             )
            parameter_root = members.get('parameter_root')
            if parameter_root:
    ...
    if not has_required or is_removed_node(instance):
        ...
        for flode in flodes:
            ...
            if cached is not None:
                ...
                try:
                    ...
                    sywarn(f'Node error, failed {flode} due to {e}')
...
def create(source_dirs, temp, session):
        ...
        if os.path.exists(library):
            if sympathy.platform.library.is_old_style(library):
                ...
                try:
                    ...
                    sywarn(
                        f'Failed to load library {library} due to {e}.')

    installed_libraries = list(sympathy.platform.library.available_libraries())
    for k, e in sympathy.platform.library.unavailable_libraries():
        sywarn(f'Failed to load library plugin {k} due to {e}.')

    for k in sympathy.platform.library.unsatisfied_libraries():
        sywarn(f'Library plugin {k} has unsatisfied requirements and may not '
               f'work properly.')
    ...
    for library_dict in libraries:
        ...
        for filename in library_dict['plugins']:
            try:
                for k, vs1 in components.get_all_components(
                        filename, no_raise=False).items():
                    for v in vs1:
                        plugins.setdefault(k, []).append(plugin_data_impl(v))
            except Exception as e:
                sywarn(f'Plugin error, failed {filename} due to {e}')

        for k, v in library_dict.get('types', {}).items():
            all_types.setdefault(k,  []).append(v)
    ...
    for name, types in all_types.items():
        if len(types) > 1:
            try:
                type_utils = ' and '.join([t.get('util', '-') for t in types])
            except Exception:
                type_utils = str(types)
            sywarn(
                f'Type conflict for {name} defined by {type_utils}.\n'
                f'This will cause problems on use.')
    return libraries
