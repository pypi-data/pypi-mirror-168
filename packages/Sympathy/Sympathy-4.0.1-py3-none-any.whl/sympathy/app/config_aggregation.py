...
from sympathy.utils.prim import format_display_string, limit_traceback
...
def clean_flow_info(flow_info_with_instances):
    ...
    def clean_node_info(node_info):
        ...
        result.pop('node_context_traceback', None)
...
def _warn_changed(store: dict, key: str, warn: str):
    old_traceback = store.get(key)
    store[key] = warn
    if old_traceback != warn:
...
class WidgetManager(object):
    ...
    def _init_uuid_to_widget(self, info_dict, top_level=True,
        ...
        def lazy_node_context(node_info_dict, node_dict):
            ...
            def inner():
                res = None
                try:
                    library_node = node_info_dict.get('library_node_instance')
                    if library_node:
                        res = self._build_node_context(library_node, node_dict)
                except Exception as e:
                    text = '\n'.join(format_exception(e))
                    _warn_changed(
                        node_info_dict, 'node_context_traceback', text)
                return res
            return inner
    ...
    def _build_node_context(self, library_node_instance, node_dict):
        """Build context for node."""
        objects = {}
        if self._cdm._lock_wizard:
            objects = dict(self._cdm.objects)
        return library_node_instance.build_context(
    ...
    def widget(self, uuid):
        ...
        def node_widget_from_context(item_dict, node_context):
            ...
            try:
                ...
                try:
                    source_file = item_dict['info_dict']['source_file']
                    text = limit_traceback(lines, source_file)
                except Exception:
                    # Unable to get source_file, show all lines.
                    text = '\n'.join(lines)
                _warn_changed(item_dict, 'widget_traceback', text)
                raise
        ...
        def node_widget(item_dict):
            ...
            if node_context is not None:
                node_widget_from_context(item_dict, node_context)
            else:
                item_dict['requires_input_data'] = True
