...
class SyiNode(object):
    ...
    def __configure_widget(self, node_context, return_widget=True):
        """
        Internal method (for testing only).
        """
        return self.__node._exec_parameter_view_with_context(
    ...
    def __configure(self, inputs=None, return_widget=True):
        ...
        with _instantiated_ports(ports, inputs):
            ...
            try:
                ...
                return self.__configure_widget(node_context, return_widget)
            finally:
                if node_context:
                    node_context.close()
                state.node_state().cleardata()
    ...
    def _apply_parameters(self):
        self.__parameters['parameters']['data'] = (
            self.__syiparameters.data.to_dict())
...
class SyiParameters(object):
    def __init__(self, parameters):
        self.__parameters_data = syparameters(parameters)
    ...
    @property
    def data(self):
        return self.__parameters_data

    @data.setter
    def data(self, value):
        self.__parameters_data = value
