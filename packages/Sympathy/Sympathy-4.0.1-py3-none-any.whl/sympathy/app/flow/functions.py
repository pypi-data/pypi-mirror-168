...
class Lambda(node.StatefulNodeMixin, flow.Flow):
    ...
    def to_dict(self, execute=False, stub=False, flow_vars=None):
...
def flow_to_lambda(flow_):
    ...
    try:
        ...
        if not flow_._flow_inputs:
            raise ExtractError(
                'Lambda must have at least one input port.')

        if not flow_._flow_outputs:
            raise ExtractError('Lambda must have one output port.')
