...
_flowio_types = (Type.FlowInput, Type.FlowOutput)
...
class Flow(NodeStatusInterface, NodeInterface):
    ...
    def propagate_ctx(self, focus=None):
        ...
        propagation. For example, execute_done. In such cases, no
    ...
    def _change_connection_state(self, connection):
        if self._in_state_done_locked:
            destination = connection.destination
            if destination:
                node = destination.node
                if node and node.type == Type.FlowOutput:
                    self.update_state()

    def _add_node_state(self, node, changed_done=True):
        abort = False
        if self._locked_operation and node in self._locked_nodes:
            if node.is_queued() or node.is_executing():
                pass
            elif node.is_done_locked():
                pass
            elif node.is_done():
    ...
    def remove_node(self, node):
        self.app_core.node_removed_from_flow(node)
    ...
    def remove_subflow(self, subflow):
        self.app_core.node_removed_from_flow(subflow)
        ...
        for incoming in incoming_connections.values():
            ...
            for port_nr, connection in enumerate(incoming):
                ...
                if port_pair not in created_port_pair_set:
                    created_subflow_connections.append(
                        subflow.create_connection(
                            flow_input.output, connection.destination,
                            emit=False))
    ...
    def _remove_connection(self, connection):
        ...
        self._change_connection_state(connection)
    ...
    def can_self_lock(self):
        return True

    def _in_parent_atom(self):
        flows = self.parent_flows()
        next(flows)
        return any(flow.is_self_atom() for flow in flows)

    def can_lock(self):
        return not self._in_parent_atom() and self.can_self_lock()
    ...
    def is_locked(self):
        ...
        """
        return not self._in_parent_atom() and self.is_self_atom()
    ...
    def source_port_traverse(self, element, previous=None, atom=False):
        if element is not None and element.type == Type.Node:
            ...
            if element.type in _flowio_types:
    ...
    def arm(self, port=None):
        if port and port.node.type in _flowio_types:
            return
    ...
    def _end_nodes(self, atom=True):
        ...
        for element in self._graph.exit_nodes():
            ...
            while not found:
                ...
                if not found:
                    element = next(
                        iter(self._graph.predecessors(element)), None)
                else:
                    found = True
        return end_nodes
    ...
    def _predecessor_node_edges_internal_list(
            self, atom=True
    ) -> List[Tuple[NodeInterface, NodeInterface]]:
        """
        (Predecessor) node edges for nodes nodes internal to the current flow.
        """
        start_ports = {self.flow.source_port(input, atom=atom)
                       for input in self.inputs}
        starts = {p.node for p in start_ports if p}

        traversed = set(starts)
        edge_set = util.OrderedSet()
        edge_set.update(self._predecessor_node_edges(
            self._end_nodes(atom=atom), atom=atom, traversed_nodes=traversed))
        # Remove edges starting with starts since these are external.
        return [(src, dst) for src, dst in edge_set if src not in starts]

    def _predecessor_node_graph(self, node=None, atom=True, starts=None):
        """
        ...
        ----------
        node
            Marks the end of graph. Set node to None to use all exit nodes!
            (caveat: the exit has to be in this).
        atom
            When True, such flows are considered as 1 node.
        starts
            Can be used to limit the beginning of the graph, when a start is
            reached, that branch of predecessor search ends.
        """
        ...
        if node is None:
            ...
            edge_set = util.OrderedSet()
            edge_set.update(self._predecessor_node_edges(
                self._end_nodes(), atom=atom, traversed_nodes=starts))
        else:
            edge_set = node.flow._predecessor_node_edges(
                [node], atom=atom, traversed_nodes=starts)
    ...
    def update_input_state(self):
        ...
        # This would require looking at the actual input, like nodes do
        # by use of _input_event.
        pass
    ...
    def predecessor_node_depths(self, node=None, remove_invalid=False,
                                atom=True) -> List[NodeInterface]:
        """
        Return dependency to order up to `node`, as a list.

        Nodes are sorted by depth in the graph, which is calculated
        from edges starting with `node` when not None and otherwise all the end
        nodes of the current flow.
        """
        return self._predecessor_node_graph_and_depths(
            node, remove_invalid, atom)[0]

    def all_incoming_nodes_are_successfully_executed(self):
        def check(port):
            if port:
                node = port.node
                # FlowIO might implement part of the node interface instead.
                return (self._is_optional_flowio_without_parent_port(node) or
                        node.is_successfully_executed() and
    ...
    def validate(self):
        for node in self._topological_nodes():
            if node.type in [Type.Flow, Type.Node]:
                core_logger.debug(
                    "Calling flow.node.Node.validate for node %s",
                    node.uuid)
                node.validate()
    ...
    def is_partially_executed(self):
        total, executed = self.execution_status()
        return 0 < executed < total
    ...
    def _set_recursive_status(self, setter):
    ...
    def set_queued(self):
        self._set_recursive_status(lambda node: node.set_queued())

    def execute_queued(self):
        self._locked_operation = True
        self.set_queued()

    def set_started(self):
        self._set_recursive_status(lambda node: node.set_started())

    def execute_started(self):
        self._locked_operation = True
        self.set_started()

    def execute_done(self, status):
        self._locked_operation = False
        self.set_done(status)
        self.update_state()
    ...
    def execute_aborting(self, child_nodes=None):
        self.set_aborting(child_nodes)
        self.update_state()
    ...
    def execute_aborted(self, child_nodes=None):
        def visit_predecessors(graph, work, ignore):
            visited = set()
            while work:
                element = work.pop()
                if element not in visited:
                    ...
                    if element not in ignore:
                        yield element
                        work.extend(graph.predecessors(element))

        self.set_aborted(child_nodes)
        if self._locked_operation:
            # Avoid searching previous flows.
            starts = {self.flow.source_port(input, atom=False).node
                      for input in self.inputs}
            graph = self._predecessor_node_graph(atom=False, starts=starts)
            lock_nodes = set()
            keep_starts = []

            for element in visit_predecessors(
                    graph, list(graph.exit_nodes()), starts):
                if element.type in [Type.Flow, Type.Node]:
                    state = element.state()
                    if state in [States.error, States.done]:
                        keep_starts.append(element)
                    elif state == States.done_locked:
                        lock_nodes.add(element)

            keep_nodes = set(visit_predecessors(graph, keep_starts, starts))

            for element in lock_nodes.difference(keep_nodes):
                element.arm()
    ...
    def _new_state(self):
        if self._broken_link:
            ...
            if self._in_state_error:
                ...
                locked_state = (States.executing
                                if self._locked_operation
                                else States.done_locked)
                new_state = (locked_state if
    ...
    def can_migrate_overrides(self):
        for tree_uuid in self.override_parameters.keys():
            ...
            if migration_chain is None:
                continue
            if migration_chain.can_migrate():
                return True
        return False
    ...
    def execution_status(self):
        ...
        total_finished = len({
            node for node in itertools.chain(
                self._in_state_done,
    ...
    def local_vars(self) -> Dict[str, str]:
        """
        Return local variables as stored in the flow, does not include any
        generated variables.
        """
        return (self._parameters or {}).get('environment', {}) or {}

    def vars_context(self, parent_context: Dict[str, str]) -> Dict[str, str]:
        """
        Return local and generated variables built from the current flow
        together with ones inherited from `parent_context`.

        When `parent_context` is the effective context for the parent
        flow, the result is the effective context for the current flow.
        This makes it possible to compute the effective context in steps
        and to save work when processing siblings.
        """
        filename = self.filename
        res = {}
        if self.library_node:
            res['SY_LIBRARY_DIR'] = (
                self.library_node.library)
        elif filename:
            res.update(self.local_vars())
            res['SY_FLOW_FILEPATH'] = filename
            parent_filename = parent_context.get('SY_FLOW_FILEPATH')
            if parent_filename:
                res['SY_PARENT_FLOW_FILEPATH'] = parent_filename
            res['SY_FLOW_DIR'] = os.path.dirname(filename)
        else:
            if self.flow is None:
                res['SY_FLOW_DIR'] = os.getcwd()
                res.update(self.local_vars())
        return res

    def effective_vars_context(self) -> Dict[str, str]:
        """
        Return effective vars context for current flow.
        The context is inherited from parent to child flow, producing
        new contexts in steps.
        """
        parent_context = {}
        child_context = {}
        for parent in reversed(list(self.parent_flows())):
            child_context = parent.vars_context(parent_context)
            parent_context = dict(parent_context)
            parent_context.update(child_context)
        return parent_context

    def _get_default_environment_keys(self):
        return [
            'SY_FLOW_FILEPATH',
            'SY_PARENT_FLOW_FILEPATH',
            'SY_FLOW_DIR',
            'SY_LIBRARY_DIR'
        ]

    def _get_default_environment(self):
        default_keys = self._get_default_environment_keys()
        return {k: v for k, v in self.effective_vars_context().items()
    ...
    def set_environment(self, env):
        default_keys = self._get_default_environment_keys()
        new_env = {k: v for k, v in env.items() if k not in default_keys}
    ...
    def migrate_overrides(self, tree_uuid, macro_cmd):
        migration_chain = self._get_overrides_migration_chain(
            tree_uuid=tree_uuid)
        if migration_chain is not None and migration_chain.can_migrate():
    ...
    def port_index(self, port):
        ...
        elif port.type in _flowio_types:
    ...
    def create_named_input(self, name):
        ...
        pdef = library.PortDefinition(
            self._conf_port_name, 'Configuration port', '<z>', 'hdf5', 0,
            False)
        mapping = {}
    ...
    def _is_optional_flowio_without_parent_port(self, element):

        return (element.type in _flowio_types and
                self._can_create_parent_flowio(element))
