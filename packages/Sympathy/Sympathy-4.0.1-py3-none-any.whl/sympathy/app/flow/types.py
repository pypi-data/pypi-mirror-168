...
class ElementInterface(QtCore.QObject):
    ...
    def parent_flows(self):
        """
        Iterator over all parent flows starting at the immediate parent flow
        and ending at the root flow (last parent).
        """
        parent = self._flow
        while parent:
            yield parent
...
class NodeInterface(PortManagerInterface, ElementInterface):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._size = QtCore.QSizeF(self.default_width, self.default_height)
...
class NodeStatusInterface(object):
    ...
    def is_partially_executed(self):
        """The node is partially executed."""
        raise NotImplementedError('Not implemented for interface')
