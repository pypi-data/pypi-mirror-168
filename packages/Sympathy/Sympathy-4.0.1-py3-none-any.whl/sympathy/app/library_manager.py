...
def libraries_from_creator(output):
    ...
    for x in output or []:
        ...
        inherit['library'] = x['root']
        inherit['library_identifier'] = x['identifier']

        nodes = []
        for node_file, js_nodes in x['js_nodes'].items():
            for js_node in js_nodes:
                try:
                    with open(js_node) as f:
                        node = json.load(
                            f, object_pairs_hook=dict)
...
def library_tags(library_json):
    return LibraryTags.merge([
        LibraryTags.from_dict(x['tags']) for x in library_json]).to_dict()
...
:Project page: `https://www.sympathyfordata.com
...
Privacy Notice
--------------
The Privacy Notice is available `here <{{privacy_url}}>`_.
"""
...
"""
    ...
    def convert_libraries_to_old_library_format(libraries):
        ...
        def tmp_to_old(tree):
            return {
                ...
                'nodes': tree['nodes'],
                'libraries': [
                    tmp_to_old(x) for x in tree['libraries'].values()]
            }
    ...
    def process_platform(self, libraries, outdir, appdir, paths,
        ...
        self._write_cached(os.path.join(src, 'privacy_notice.rst'),
                           jinja2.Template(self.PRIVACY_NOTICE).render(
                               privacy_url=version.privacy_url()))
...
try:
    ...
    argv = ['--no-color', '-b', 'html', '.', os.path.join(
        working_dir, '..', 'html')]
except ImportError:
    import sphinx
    argv = ['', '--no-color', '-b', 'html', '.', os.path.join(
...
class LibraryManager(object):
    ...
    def typealiases(self):
        """Return typeliases."""
        res = {}
        for x in self.library_by_json():
            res.update(x['types'])
        return res

    def plugins(self):
        def plugin_info(plugin_dict):
            return (plugin_dict['class'], plugin_dict['file'])

        res = {}
        for x in self.library_by_json():
            for base, impls in x['plugins_info']:
                for impl in impls:
                    res.setdefault(
                        plugin_info(base), []).append(plugin_info(impl))
