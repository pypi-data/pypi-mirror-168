...
def copy(dt):
    ...
    if t == TypeList:
        ...
        return TypeRecord(dict((k, copy(v))
                               for k, v in dt.items()))
    elif t == TypeDict:
...
def p_error(token):
    raise TypeError('Invalid specification: "{}: {}"'.format(
    ...
    def expansions(dt):
        ...
        if t == TypeList:
            ...
            if isinstance(value, TypeAlias):
                dt[0] = definitions.get(value.name(), value)
            else:
                expansions(value)
            ...
            value = dt.element()
            if isinstance(value, TypeAlias):
                dt.set_element(definitions.get(value.name(), value))
            else:
                expansions(value)
            ...
            for key, value in dt.items():
                if isinstance(value, TypeAlias):
                    dt[key] = definitions.get(value.name(), value)
                else:
                    expansions(value)
            ...
            for i, value in enumerate(dt):
                if isinstance(value, TypeAlias):
                    dt[i] = definitions.get(value.name(), value)
                else:
                    expansions(value)
            ...
            value = dt.get()
            if isinstance(value, TypeAlias):
                dt.set(definitions.get(value.name(), value))
            else:
                expansions(value)
...
def simplify_aliases(typealiases):
    ...
    def traverse(ob, names_in_chain=None):
        ...
        if isinstance(ob, TypeAlias):
            ret = traverse(ob.get(), names_in_chain)

        elif isinstance(ob, TypeList):
            ret = TypeList()
            if isinstance(ob[0], TypeAlias):
...
def expand_type(typealiases, type_hierarchy):
    ...
    if t == TypeTable:
        ...
        return TypeRecord(
            dict((key, expand_type(typealiases, value))
...
class UndefinedTypeError(KeyError):
    pass
...
def from_type_expand(parsed_type):
    try:
        ...
        raise UndefinedTypeError(*e.args)


def from_string_expand(string_type):
    return from_type_expand(from_string(string_type))
