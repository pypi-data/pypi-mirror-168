...
def set_application_id(
        identifier='Combine Control Systems AB.Sympathy.SympathyGUI'):
    if prim.is_windows():
        try:
...
def set_shared_opengl_contexts():
    ...
    PySide6.QtWidgets.QApplication.setAttribute(
        PySide6.QtCore.Qt.AA_ShareOpenGLContexts, True)


def set_use_desktop_opengl():
    import PySide6
    # Other renderers are AA_UseDesktopOpenGL, AA_UseSoftwareOpenGL.
    PySide6.QtWidgets.QApplication.setAttribute(
        QtCore.Qt.AA_UseDesktopOpenGL, True)


def setup_qt_opengl():
    set_shared_opengl_contexts()
    if prim.is_windows():
        set_use_desktop_opengl()
...
def set_high_dpi_unaware():
    # Calling this function does nothing. It will eventually be removed.
    return
...
def open_url(url: str):
    if prim.is_cygwin():
        subprocess.call(['/usr/bin/cygstart', url])
    else:
        from PySide6 import QtGui
        QtGui.QDesktopServices.openUrl(QtCore.QUrl(url))


def _open_local(filename: str):
    if prim.is_cygwin():
        subprocess.call(['/usr/bin/cygstart', filename])
    else:
        from PySide6 import QtGui
        QtGui.QDesktopServices.openUrl(prim.localuri(filename))


def open_directory(directory: str) -> bool:
    res = os.path.isdir(directory)
    if res:
        _open_local(directory)
    return res


def open_filename(filename: str) -> bool:
    res = os.path.isfile(filename)
    if res:
        _open_local(filename)
    return res
