...
class ShadowLegion:
    def __init__(
        ...
        self.ss = SS(host=self.ip, port=7889, silent=True, functions=dict(
            sws_length=self.sws_length,
            sws_info=self.sws_info,
            sws_restart=self.sws_restart,
            sws_started=self.sws_started,
            sws_stopped=self.sws_stopped,
            stop=self._stop,
            restart=self.restart,
        ))

    def sws_started(self, i):
        return not self.sws_stopped(i)

    def sws_stopped(self, i):
        return not self.sws[i].clash
    ...
    def start_sws(self, sws=None):
        sws = sws or self.sws
        tw = threadwrapper.ThreadWrapper(threading.Semaphore(2 ** 11))
        for i in range(0, len(sws)):
            def job(i):
                sws[i].start(quiet=True, proxy=sws[i].prev_proxy)
            tw.add(job=def_template(job, i))
            if i % 100 == 0:
                time.sleep(1)
        time.sleep(math.ceil(len(sws)/100)*5)

    def start(self, sws=None):
        self.start_sws(sws)
        p = threading.Thread(target=lambda: self.ss.start())
        p.daemon = True
        p.start()

    def stop_sws(self, port_offset=None):
        ...
        for _ in psutil.process_iter():
            try:
                ...
                if port.endswith(str(port_offset)):
                    cp.append(_)
            except:
                pass
    ...
    def stop(self, port_offset=None):
        self.ss.stop()
        self.stop_sws(port_offset)
    ...
    def restart(self):
        os.remove("ping_cache.json")
        os.remove("proxies_cache.json")
        port_offset = self.port_offset
        self.port_offset = (self.port_offset+1)%10
        sws = [self.SW(max_ping=998)]
        sws[0].clash_port += self.port_offset
        sws[0].prev_proxy = sws[0].proxies[0]
        for _ in range(1, len(sws[0].proxies)):
            sws.append(sws[-1].clone())
            sws[-1].prev_proxy = sws[-1].proxies[_]
        self.start_sws(sws)
        self.sws = sws
        self.stop_sws(port_offset)
