...
class ReqUtil(object):
    ...
    async def rest_fetch_util(host_uri, method, param=None, behavior='',
        ...
        if method == HTTP_POST:
            ...
            flag = '&' if '?' in host_uri else '?'
            uri_p = host_uri + '%s%s' % (flag, parse.quote(json.dumps(arguments)))
            try:
    ...
    async def call_sync_trans(self, events, creator, s_type="rest", local_sync_coro=None, mock_data=None):
        ...
        p = {
            "Type": s_type, "Creator": creator, "Events": pes
        }
        label = None
        coro = None
        if local_sync_coro:
    ...
    async def call_patch_delay(self, event, creator, delay):
        ...
        return await self.restful_fetch(ServiceConst.patchCore + "/patchCore/patchRegister", "POST", {
            "Event": p_event, "Creator": creator, "Delay": delay
        }, behavior="delay")

    async def call_patch_wait4done(self, event, creator, callback_patch4done, callback_patch4progress=None):
